...
function getDashboardForRole(userRole: string | undefined): string {
  switch (userRole) {
    ...
    case 'warehouse_manager': return '/inventory/items'
    case 'viewer': return '/inventory/items'
...
const router = createRouter({
  ...
  routes: [
    ...
    { 
      ...
      meta: { requiresAuth: true, roles: ['company_manager', 'warehouse_manager', 'viewer'], title: 'الحركات' } 
    },
...
// ✅ Fixed: Remove prefetchRoute as it doesn't exist on Router type
if ('requestIdleCallback' in window) {
  requestIdleCallback(() => {
    // Simple prefetch by importing routes
    ...
    routesToPrefetch.forEach(routeName => {
      try {
        // Use dynamic import for prefetching
        const route = router.getRoutes().find(r => r.name === routeName)
        if (route?.component) {
          // Prefetch the component
          import(/* @vite-ignore */ route.component.toString())
        }
      } catch {
