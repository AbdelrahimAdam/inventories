...
let cachedAuthState: { isAuthenticated: boolean; userRole: string | undefined } | null = null
...
router.beforeEach(async (to: RouteLocationNormalized, _from: RouteLocationNormalized, next: NavigationGuardNext) => {
  ...
  cachedAuthState = currentAuthState

  const isAuthenticated = currentAuthState.isAuthenticated
  const userRole = currentAuthState.userRole
...
if ('requestIdleCallback' in window) {
  requestIdleCallback(() => {
    ...
    routesToPrefetch.forEach(routeName => {
      try {
        router.router?.['prefetchRoute']?.(routeName)
      } catch {
