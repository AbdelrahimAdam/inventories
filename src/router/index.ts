...
router.beforeEach(async (to: RouteLocationNormalized, _from: RouteLocationNormalized, next: NavigationGuardNext) => {
  ...
  // ⭐ FIX: Only check subscription if it has been initialized
  // If subscription status hasn't been fetched yet, fetch it now
  if (isAuthenticated && !isSuperAdmin && !authStore.isSubscriptionInitialized) {
    console.log('⏳ Subscription not initialized, fetching now...')
    try {
      await authStore.refreshSubscriptionStatus(true)
      // Mark as initialized (the store should do this, but just in case)
      // We need to ensure the store has this property
      // If authStore.isSubscriptionInitialized doesn't exist, we'll set it
      // This will be handled in the auth.ts update
    } catch (err) {
      console.error('Failed to fetch subscription status:', err)
      // If we can't fetch subscription, assume it's inactive for safety
      // but don't block the user from seeing the page
    }
  }

  // ⭐ FIX: Only check trial and subscription if subscription is initialized
  if (isAuthenticated && !isSuperAdmin) {
    const isTrialExpired = authStore.tenantTrialExpired || authStore.isUserTrialExpired
    // Only consider subscription inactive if it has been initialized AND is false
    // If not initialized, allow the navigation (will be checked again after initialization)
    ...
    if (isTrialExpired && !isOnExpiredPage) {
      console.log('⏰ Trial expired, redirecting to /trial-expired')
      return next('/trial-expired')
    }

    // ⭐ FIX: Only redirect if subscription is DEFINITELY inactive (initialized and false)
    // If subscription is not yet initialized, let the navigation proceed
    // The App.vue will handle the loading state
    if (isSubscriptionInactive && !isOnExpiredPage) {
      console.log('🚫 Subscription is inactive, redirecting to /subscription-expired')
  ...
  if (isAuthenticated && publicPaths.includes(to.path)) {
    // Don't redirect if on expired pages (they're public)
    if (to.path === '/trial-expired' || to.path === '/subscription-expired') {
      // Allow access to expired pages even if authenticated
      // But check if the user is actually expired
      if (!isSuperAdmin) {
        const isTrialExpired = authStore.tenantTrialExpired || authStore.isUserTrialExpired
        const isSubscriptionInactive = authStore.isSubscriptionInitialized && !authStore.isSubscriptionActive
        
        // If not expired and trying to access expired page, redirect to dashboard
        ...
        // If expired, allow access to the expired page
        return next()
      }
      // Super admin can access expired pages
      return next()
    }
    
    // For other public paths (login, register, etc.)
    if (!isSuperAdmin) {
      ...
      if (isExpired) {
        // If expired, allow access to login page
        return next()
      }
    }
    const dashboard = getDashboardForRole(userRole)
