...
export const useInvoiceStore = defineStore('invoice', () => {
  ...
  // Use inventoryStore.dispatchItem for stock deduction (multi-warehouse support)
    ...
    try {
      ...
      for (const [warehouseId, items] of itemsByWarehouse) {
        for (const item of items) {
          ...
          // Use the same dispatchItem method as the dispatch modal
  ...
  async function returnStockForInvoice(invoice: Invoice, itemsToReturn?: { item_id: string; quantity: number }[]): Promise<{ success: boolean; message?: string }> {
    ...
    try {
      ...
      // Group return items by warehouse
      const itemsByWarehouse = new Map<string, any[]>()
