...
export const useInvoiceStore = defineStore('invoice', () => {
  ...
  // FIXED: Use authStore.canEdit directly - it already handles permissions correctly
  ...
  // FIXED: Create ONE invoice with ALL items (multi-warehouse support)
  async function createInvoice(invoiceData: Partial<Invoice>): Promise<{ success: boolean; message?: string; data?: Invoice }> {
    ...
    // Check each item has warehouse_id
    ...
    try {
      ...
      // Create ONE invoice for ALL items
      ...
      // Insert ALL items into ONE invoice (each with its warehouse_id)
      const itemsPayload = invoiceData.items.map(item => ({
        ...
        warehouse_id: item.warehouse_id, // Each item keeps its warehouse
