...
function clearCache(_key: string, tenantId: string): void {
  try {
    const fullKey = getCacheKey(_key, tenantId)
    localStorage.removeItem(fullKey)
  } catch {
    // Silently fail
  }
...
export const useInvoiceStore = defineStore('invoice', () => {
  ...
  async function deleteInvoice(
    ...
    try {
      ...
      if (options?.returnItems) {
        const returnResult = await returnItemsForInvoice(invoice)
      ...
      return { 
        success: true, 
        message: options?.returnItems 
          ? 'تم حذف الفاتورة وإرجاع المخزون بنجاح' 
  ...
  async function returnItemsForInvoice(
    invoice: Invoice
  ): Promise<{ success: boolean; message?: string; returnedItems?: any[] }> {
    if (!invoice.items || invoice.items.length === 0) {
      return { success: true, message: 'لا توجد أصناف للإرجاع', returnedItems: [] }
    }
    ...
    try {
      for (const [warehouseId, items] of itemsByWarehouse) {
        if (warehouseId === 'unknown') {
          ...
          return { 
            success: false, 
            message: `الأصناف التالية ليس لها مخزن محدد: ${itemNames}` 
          }
        }
        ...
        for (const item of items) {
          ...
          if (fetchError) {
            return { 
              success: false, 
              message: `الصنف ${item.name} غير موجود` 
            }
          }
          ...
          if (updateError) {
            return { 
              success: false, 
              message: `فشل إرجاع مخزون الصنف ${item.name}` 
            }
          }
          ...
          if (txError) {
            console.error('❌ Failed to insert RETURN transaction:', txError)
          }
      ...
      const tenantId = authStore.currentTenantId
      if (tenantId && returnedItems.length > 0) {
        const changedItemIds = returnedItems.map(item => item.item_id)
        await inventoryStore.refreshItems(changedItemIds)
        
        // ⭐ Refresh transactions to show the RETURN transaction
        await transactionStore.fetchTransactions(1, 10000, false)
      }

      return { 
        success: true, 
        message: `تم إرجاع ${returnedItems.length} صنف بنجاح`,
        returnedItems 
      }
    } catch (err: any) {
      console.error('Error returning items for invoice:', err)
      return { 
        success: false, 
        message: err.message || 'حدث خطأ أثناء إرجاع الأصناف' 
      }
    }
  }
