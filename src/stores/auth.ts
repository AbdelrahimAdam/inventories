// stores/auth.ts (updated with cache)
...
export const useAuthStore = defineStore('auth', () => {
  ...
  const isSubscriptionInitialized = ref(false)
  ...
  function clearUserCache(): void {
    ...
    if (tenantId) {
      clearAllAuthCache(tenantId)
    }
  }

  function handleSubscriptionRestored(): void {
    ...
    showToast('تم تجديد اشتراكك بنجاح!', 'success')
    refreshUserProfile()
    refreshSubscriptionStatus(true)
  }

  function setupSubscriptionListeners(): void {
    if (supabaseService && typeof supabaseService.setSubscriptionRestoredHandler === 'function') {
      supabaseService.setSubscriptionRestoredHandler(handleSubscriptionRestored)
    }
  }
  ...
  async function refreshSubscriptionStatus(force: boolean = false): Promise<boolean> {
    ...
    if (!force && lastSubscriptionCheck.value && now - lastSubscriptionCheck.value < 300000) {
      ...
      if (cached) {
        isSubscriptionActive.value = cached.active
        subscriptionExpiryDate.value = cached.expiry ? new Date(cached.expiry) : null
        isSubscriptionInitialized.value = true
    ...
    try {
      ...
      // Update cache
      setCachedSubscription(tenantId, active, paidUntil ? paidUntil.toISOString() : null)
  ...
  async function checkTenantTrialStatus(): Promise<boolean> {
    const tenantId = currentTenantId.value
    if (!tenantId || isSuperAdmin.value) return false

    // Check cache first
    const cached = getCachedTrialStatus(tenantId)
    if (cached) {
      isTenantTrial.value = cached.isTrial
      tenantTrialEndsAt.value = cached.endsAt ? new Date(cached.endsAt) : null
      tenantTrialExpired.value = cached.expired
    ...
    try {
      ...
      // Update cache
      setCachedTrialStatus(tenantId, isTenantTrial.value, expired, data?.trial_ends_at || null)
  ...
  async function fetchUserProfile(userId: string, retries = 5): Promise<UserProfile | null> {
    ...
    // Check cache first (only if we have tenantId)
    if (tenantId) {
      const cached = getCachedUserProfile(tenantId)
    ...
    for (let i = 0; i < retries; i++) {
      try {
        ...
        // Cache the profile
        if (profile.tenantId) {
          setCachedUserProfile(profile.tenantId, profile)
        }
  ...
  async function logout(): Promise<void> {
    ...
    isSubscriptionInitialized.value = false
  ...
  async function updateProfile(updates: { name?: string; phone?: string }): Promise<boolean> {
    ...
    try {
      ...
      const tenantId = currentTenantId.value
      if (tenantId && user.value) {
        setCachedUserProfile(tenantId, user.value)
      }
