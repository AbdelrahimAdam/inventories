...
export const useAuthStore = defineStore('auth', () => {
  ...
  // ⭐ NEW: Track if subscription has been initialized
    ...
    initPromise = (async () => {
      try {
        ...
        if (!session?.user) {
          ...
          // ⭐ Mark as initialized even without user
          isSubscriptionInitialized.value = true
        ...
        // ⭐ FIX: Fetch subscription BEFORE setting isFullyReady
        if (!isSuperAdmin.value) {
          // AWAIT the subscription check
          await refreshSubscriptionStatus(true)
          // isSubscriptionInitialized is set inside refreshSubscriptionStatus
        } else {
          isSubscriptionInitialized.value = true
          isSubscriptionActive.value = true
        }
        ...
        // ⭐ Set isFullyReady AFTER subscription is fetched
        isFullyReady.value = true

        // ⭐ REMOVED: The async call that was causing the race condition
        // if (!isSuperAdmin.value) {
        //   refreshSubscriptionStatus(true).catch(() => {}) // ← REMOVED
        // }
  ...
  async function login(credentials: LoginCredentials): Promise<boolean> {
    ...
    try {
      ...
      // ⭐ REMOVED: Extra async call (already awaited above)
      // if (!isSuperAdmin.value) {
      //   refreshSubscriptionStatus(true).catch(() => {}) // ← REMOVED
      // }
  ...
  async function logout(): Promise<void> {
    ...
    // ⭐ Reset subscription initialized flag
