...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  // OPTIMIZED: Skip expensive database check for initial load
  async function hasDataChanged(tenantId: string): Promise<boolean> {
    // Skip DB check - trust cache for faster initial load
    return false
  }
  ...
  // OPTIMIZED: Single query for all stats using RPC
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        summaryStats.value = {
          ...
          criticalStock: stats.critical_stock || 0,
  ...
  // OPTIMIZED: Parallel loading of items and total count
  async function fetchItemsPage(params: {
    ...
    try {
      ...
      totalCount.value = countResult.data || 0
