...
function getFromCache<T = any>(key: string): CacheData<T> | null {
  try {
...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  const summaryStats = ref<StatsCacheData>({
    totalItems: 0,
    totalQuantity: 0,
    lowStock: 0,
    criticalStock: 0,
  ...
  /**
   * Load items from cache and update the store state.
  ...
  function loadFromCache(): boolean {
    ...
    totalCount.value = cached.data.length

    // CRITICAL FIX: Update lastFetchTime so the store knows this data is fresh
    lastItemsFetchTime = cached.timestamp
    lastItemsFiltersHash = JSON.stringify(cached.filters || {})
  ...
  function updateLocalItem(updatedItem: InventoryItem) {
    ...
    saveToCache<InventoryItem[]>(CACHE_KEY_ITEMS, itemsArray, {
  ...
  async function fetchSummaryStats(params?: { search?: string; warehouseId?: string; color?: string; size?: string }) {
    ...
    const cached = getFromCache<StatsCacheData>(CACHE_KEY_STATS)
    ...
    try {
      ...
      if (data && data.length) {
        ...
        saveToCache<StatsCacheData>(CACHE_KEY_STATS, stats)
      }
    } catch (err) {
  ...
  async function fetchItemsPage(params: {
    ...
    // Check if we have valid cached data in memory (same filters, not expired)
    const memoryCacheValid = !force && 
                            lastItemsFetchTime > 0 && 
                            (now - lastItemsFetchTime) < CACHE_DURATION &&
                            lastItemsFiltersHash === currentHash && 
                            itemsMap.value.size > 0
    ...
    // If not forced and items are empty or expired, try loading from localStorage cache
    if (!force && itemsMap.value.size === 0) {
      if (loadFromCache()) {
        // Check if the cached data matches the current filters
        const cached = getFromCache<InventoryItem[]>(CACHE_KEY_ITEMS)
        if (cached && cached.filters) {
          const cachedFiltersHash = JSON.stringify(cached.filters)
          if (cachedFiltersHash === currentHash) {
            isLoading.value = false
            return
          }
        }
        // Filters don't match, continue to fetch fresh data
      }
    }

    // Check if we have cache but it's expired, still use it while fetching fresh
    const cached = getFromCache<InventoryItem[]>(CACHE_KEY_ITEMS)
    let usedStaleCache = false
    if (!force && cached && cached.data && cached.data.length > 0) {
      const expired = (Date.now() - cached.timestamp) > CACHE_DURATION
      // If it's not too old (within 10 minutes) or we're offline, use it
      if (expired && itemsMap.value.size === 0) {
        const isOffline = !navigator.onLine
        if (isOffline || (Date.now() - cached.timestamp) < CACHE_DURATION * 2) {
          loadFromCache()
          usedStaleCache = true
    ...
    try {
      ...
      const itemsArray = Array.from(newMap.values())
      saveToCache<InventoryItem[]>(CACHE_KEY_ITEMS, itemsArray, {
        page,
        pageSize: pgSize,
        search,
      ...
      if (err.name !== 'AbortError') {
        ...
        // Try to load from cache on error (if not already loaded)
        if (!usedStaleCache && itemsMap.value.size === 0) {
          loadFromCache()
        }
      }
    } finally {
  ...
  async function fetchTransactions(page: number = 1, pageSizeArg: number = 50, append: boolean = false): Promise<{ data: Transaction[]; total: number }> {
    ...
    try {
      ...
      saveToCache<Transaction[]>(CACHE_KEY_TRANSACTIONS, mapped)
  ...
  async function performUpdate(
    ...
    return {
      ...
      type: 'updated',
      id: existingItem.id,
      item: itemsMap.value.get(existingItem.id),
      quantityAdded,
      message: `تم تحديث ${existingItem.name}: أضيف ${quantityAdded} وحدة`
    }
  }
  ...
  async function addItem(itemData: Partial<InventoryItem> & {
    ...
    try {
      ...
      return {
        ...
        message: `تم إنشاء صنف جديد: ${itemData.name}`
      }
    } catch (err: any) {
      itemsMap.value.delete(tempId)
  ...
  async function transferItem(transferData: {
    ...
    try {
      ...
      return {
        ...
        message: `تم نقل ${result?.transferred || 0} وحدة بنجاح`
      }
    } catch (err: any) {
      error.value = err.message
      return { success: false, message: err.message }
