...
export const useInventoryStore = defineStore('inventory', () => {
    ...
    try {
      ...
      if (cachedItems && cachedItems.length > 0) {
        ...
        const isComplete = metadata?.loadType === 'full' || 
                           (metadata?.loadType === 'paginated' && metadata?.totalItems === cachedItems.length)
  ...
  // ============================================
  // NEW: Fast dashboard stats (NO full item load)
  // ============================================
  ...
  // ============================================
  // NEW: Fast alert items (limited to alerts only)
  // ============================================
  async function fetchAlertItemsOptimized(warehouseId?: string): Promise<{
    ...
    try {
      // Get all alert items in ONE query
      let query = supabase
        .from('items')
        .select(`
          *,
      ...
      // Apply allowed warehouses filter
      if (!allowed.includes('all') && allowed.length > 0) {
        query = query.in('warehouse_id', allowed)
      }
  ...
  // ============================================
  // NEW: Quick dashboard load (stats + alerts + first page)
  // ============================================
  async function loadDashboardData(warehouseId?: string): Promise<{
    ...
    try {
      ...
      // Load first page of items (50 items) for the table
      await fetchItemsPage(1, {
        pageSize: 50,
        warehouseId: warehouseId || undefined
      })
  ...
  return {
    ...
    isDataAvailable,
    // NEW: Dashboard optimized methods
    fetchDashboardStats,
