...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  // ✅ STATS CALCULATED FROM LOADED ITEMS - NO API CALLS
  ...
  // ✅ Stats from allItemsForStats (for dashboard)
  ...
  function loadFromCache(tenantId: string): boolean {
    ...
    try {
      ...
      if (cachedItems && cachedItems.length > 0) {
        ...
        // Update allItemsForStats from cache
  ...
  // ✅ fetchSummaryStats now uses computed stats - no API call
  async function fetchSummaryStats(params?: { search?: string; warehouseId?: string; color?: string; size?: string }): Promise<typeof summaryStats.value> {
    // If warehouse filter is applied, return filtered stats
  ...
  return {
    ...
    hybridSearch,
    clearSearchResults,
    // ✅ Stats are now computed from loaded items - no API calls
    summaryStats,
    filteredStats,
    currentFilters,
    transactionFilters,
