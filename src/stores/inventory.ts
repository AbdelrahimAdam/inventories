...
import type { InventoryItem, Transaction, TransferResponse } from '@/types'
...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  function loadFromCache(tenantId: string): boolean {
    ...
    try {
      ...
      if (cachedItems && cachedItems.length > 0) {
        ...
        const cachedStats = getCache<typeof summaryStats.value>(CACHE_KEYS.STATS, tenantId)
        if (cachedStats) {
          summaryStats.value = cachedStats
        }

        const cachedTransactions = getCache<Transaction[]>(CACHE_KEYS.TRANSACTIONS, tenantId)
        if (cachedTransactions) {
          transactions.value = cachedTransactions
          transactionsLoaded.value = true
        }

        const cachedPage = getCache<{ page: number; pageSize: number }>(CACHE_KEYS.PAGE, tenantId)
        if (cachedPage) {
          currentPage.value = cachedPage.page
          pageSize.value = cachedPage.pageSize
        }

        const cachedFilters = getCache<typeof currentFilters.value>(CACHE_KEYS.FILTERS, tenantId)
        if (cachedFilters) {
          currentFilters.value = cachedFilters
        }
  ...
  function saveToCache(tenantId: string) {
    ...
    try {
      const itemsArray = Array.from(itemsMap.value.values())
      setCache(CACHE_KEYS.ITEMS, tenantId, itemsArray)
      setCache(CACHE_KEYS.STATS, tenantId, summaryStats.value)
  ...
  // ============================================================
  // REFRESH SPECIFIC ITEMS ONLY (Optimized)
  // ============================================================
  async function refreshItems(itemIds: string[]): Promise<void> {
    ...
    try {
      // 1. Fetch only the changed items from database
      ...
      // 2. Update data version to trigger cache invalidation
      updateDataVersion(tenantId)

      // 3. Clear search results and filter cache (they might be stale)
      unifiedSearchResults.value = []
      filterCache.value.clear()
  ...
  async function fetchSummaryStats(params?: { search?: string; warehouseId?: string; color?: string; size?: string }) {
    ...
    try {
      ...
      let quantityQuery = supabase
        .from('items')
        .select('remaining_quantity')
        .eq('tenant_id', tenantId)

      if (search && search.trim()) {
        quantityQuery = quantityQuery.or(`name.ilike.%${search}%,code.ilike.%${search}%,size.ilike.%${search}%`)
      }
      if (warehouseId) quantityQuery = quantityQuery.eq('warehouse_id', warehouseId)
      if (color && color.trim()) quantityQuery = quantityQuery.ilike('color', `%${color}%`)
      if (size && size.trim()) quantityQuery = quantityQuery.ilike('size', `%${size}%`)

      if (!allowed.includes('all') && allowed.length > 0) {
        quantityQuery = quantityQuery.in('warehouse_id', allowed)
      }

      const { data: quantityData } = await quantityQuery

      const totalQuantity = quantityData?.reduce((sum, item) => sum + (item.remaining_quantity || 0), 0) || 0
      ...
      if (search && search.trim()) {
        lowStockQuery = lowStockQuery.or(`name.ilike.%${search}%,code.ilike.%${search}%,size.ilike.%${search}%`)
      }
      if (warehouseId) lowStockQuery = lowStockQuery.eq('warehouse_id', warehouseId)
      if (color && color.trim()) lowStockQuery = lowStockQuery.ilike('color', `%${color}%`)
      if (size && size.trim()) lowStockQuery = lowStockQuery.ilike('size', `%${size}%`)

      if (!allowed.includes('all') && allowed.length > 0) {
        lowStockQuery = lowStockQuery.in('warehouse_id', allowed)
      }

      const { count: lowStock } = await lowStockQuery

      let criticalStockQuery = supabase
        .from('items')
        .select('*', { count: 'exact', head: true })
        .eq('tenant_id', tenantId)
        .gt('remaining_quantity', 0)
      ...
      if (search && search.trim()) {
        criticalStockQuery = criticalStockQuery.or(`name.ilike.%${search}%,code.ilike.%${search}%,size.ilike.%${search}%`)
      }
      if (warehouseId) criticalStockQuery = criticalStockQuery.eq('warehouse_id', warehouseId)
      if (color && color.trim()) criticalStockQuery = criticalStockQuery.ilike('color', `%${color}%`)
      if (size && size.trim()) criticalStockQuery = criticalStockQuery.ilike('size', `%${size}%`)

      if (!allowed.includes('all') && allowed.length > 0) {
        criticalStockQuery = criticalStockQuery.in('warehouse_id', allowed)
      }

      const { count: criticalStock } = await criticalStockQuery
      ...
      if (search && search.trim()) {
        outOfStockQuery = outOfStockQuery.or(`name.ilike.%${search}%,code.ilike.%${search}%,size.ilike.%${search}%`)
      }
      if (warehouseId) outOfStockQuery = outOfStockQuery.eq('warehouse_id', warehouseId)
      if (color && color.trim()) outOfStockQuery = outOfStockQuery.ilike('color', `%${color}%`)
      if (size && size.trim()) outOfStockQuery = outOfStockQuery.ilike('size', `%${size}%`)

      if (!allowed.includes('all') && allowed.length > 0) {
        outOfStockQuery = outOfStockQuery.in('warehouse_id', allowed)
      }
      ...
      const tenantIdCache = authStore.currentTenantId
      if (tenantIdCache) {
        setCache(CACHE_KEYS.STATS, tenantIdCache, summaryStats.value)
      }
    } catch (err) {
  ...
  async function fetchItemsPage(params: {
    ...
    if (!force && !cacheLoaded && !search && !warehouseId && !status && !color && !itemSize) {
  ...
  async function updateItem(itemId: string, itemData: Partial<InventoryItem>): Promise<boolean> {
    ...
    try {
      ...
      if (refreshed) updateLocalItem(refreshed)
