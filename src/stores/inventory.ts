...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  async function hybridSearch(searchQuery: string) {
    ...
    try {
      const serverResults = await fetchAllItemsForExport({
        search: query,
  ...
  async function fetchItemsPage(params: {
    ...
    if (dataLoaded.value && itemsMap.value.size > 0 && !force) {
      isLoading.value = false
      return
    }
    ...
    const now = Date.now()
    const cacheValid = !force && lastItemsFetchTime > 0 && (now - lastItemsFetchTime) < CACHE_TTL &&
