...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  // ✅ REMOVED: cacheLoaded - we no longer use this flag
  ...
  // ✅ loadFromCache now only loads if cache exists, but does NOT set any flags
  ...
  async function fetchItemsPage(params: {
    ...
    // ✅ Use filter cache only for non-force requests
  ...
  // ✅ FIXED: Removed automatic cache loading from watch
  // This was the root cause - it was loading 50 cached items before components could fetch fresh data
  watch(
    ...
    (tenantId) => {
      ...
      if (tenantId) {
        ...
        // ✅ DO NOT auto-load cache here - let components decide
      }
    },
    { immediate: true }
  )
