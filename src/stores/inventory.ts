...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  const loadingProgress = ref(0) // 0-100 for progress tracking
  ...
  function updateLocalItem(updatedItem: InventoryItem) {
    ...
    if (tenantId) {
      // Use debounced cache update instead of immediate
  ...
  /**
   * Fetches ALL items from the database using pagination
   * This is the primary function for loading all inventory items
   */
  ...
  /**
   * Fetches items with pagination support
   * For large page sizes (>10000), it automatically uses fetchAllItemsForExport
   */
  async function fetchItemsPage(params: {
    ...
    // ✅ If loading ALL items (large pageSize), use fetchAllItemsForExport
    if (pgSize >= 10000) {
    ...
    // ✅ Normal paginated loading (small page size)
  ...
  /**
   * Fetches ALL items - uses fetchAllItemsForExport for completeness
   */
  async function fetchItems(): Promise<void> {
    ...
    try {
      // ✅ Use the paginated export function to get ALL items
      const allItems = await fetchAllItemsForExport({
  ...
  /**
   * Emergency function to force refresh all data
  ...
  // ✅ Fixed: Watch tenant changes but don't auto-load cache
  watch(
    ...
    (tenantId) => {
      ...
      if (tenantId) {
        ...
        // Components decide when to load - no auto-load here
      }
    },
    { immediate: true }
  )
