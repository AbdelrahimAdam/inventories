...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  let isBackgroundLoading = ref(false)
  ...
  function isCacheStale(tenantId: string): boolean {
    try {
      const fullKey = getCacheKey(CACHE_KEYS.ITEMS, tenantId)
      const cached = localStorage.getItem(fullKey)
  ...
  function loadFromCache(tenantId: string): boolean {
    ...
    try {
      ...
      if (cachedItems && cachedItems.length > 0) {
        ...
        totalItemsCount.value = cachedItems.length
  ...
  async function fetchItemsPage(params: {
    ...
    const { page, pageSize: pgSize = pageSize.value, search, warehouseId, status, color, size: itemSize, force = false, background = false } = params
    const tenantId = authStore.currentTenantId
    ...
    if (!force && !search && !warehouseId && !status && !color && !itemSize && fullDataLoaded.value) {
      if (background) {
        isLoadingMore.value = false
      } else {
        isLoading.value = false
  ...
  async function fetchInitialItems(): Promise<void> {
    ...
    await fetchItemsPage({ page: 1, pageSize: pageSize.value, force: false })
    
    const total = await fetchTotalCount({})
    totalItemsCount.value = total
  }

  async function loadRemainingItems(): Promise<void> {
    ...
    const currentCount = itemsMap.value.size
    const total = await fetchTotalCount({})
    totalItemsCount.value = total
    ...
    const allItems: InventoryItem[] = Array.from(itemsMap.value.values())
    ...
    while (hasMore && !fullDataLoaded.value) {
      try {
        const { data, error: rpcError } = await supabase.rpc('get_items_page', {
          p_tenant_id: tenantId,
          p_limit: batchSize,
        ...
        const mappedItems = data.map((item: any) => mapDbItemToInventoryItem(item))
        const newMap = new Map(itemsMap.value)
        const newUniqueMap = new Map(itemsByUniqueKey.value)
        ...
        loadingProgress.value = Math.min(Math.round((loadedCount / total) * 100), 100)
        
        totalItemsCount.value = total

        offset += batchSize
        hasMore = data.length === batchSize && loadedCount < total
  ...
  async function fetchItems(): Promise<void> {
    ...
    const hasCache = loadFromCache(tenantId)
    if (hasCache && fullDataLoaded.value) {
      return
    }
