...
const CACHE_KEYS = {
  ...
  SCROLL: 'inventory_scroll_cache',
  VIEW_MODE: 'inventory_view_mode_cache',
  SEARCH: 'inventory_search_cache',
  FILTER_STATE: 'inventory_filter_state_cache',
  CACHE_METADATA: 'inventory_cache_metadata',
}
...
interface CacheMetadata {
  totalItems: number
  loadedAt: number
  loadType: 'full' | 'partial' | 'paginated'
  filters: {
    search?: string
    warehouseId?: string
    status?: string
...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  const filterCache = new LRUCache<string, { data: InventoryItem[]; timestamp: number }>(100)
  ...
  function restorePersistedState(tenantId: string) {
    if (!tenantId) return

    const savedFilters = getCache<typeof currentFilters.value>(CACHE_KEYS.FILTERS, tenantId)
    if (savedFilters) {
      currentFilters.value = savedFilters
    }

    const savedViewMode = getCache<'paginated' | 'view-all'>(CACHE_KEYS.VIEW_MODE, tenantId)
    if (savedViewMode) {
      viewMode.value = savedViewMode
    }

    const savedScroll = getCache<Record<string, number>>(CACHE_KEYS.SCROLL, tenantId)
    if (savedScroll) {
      tableScrollPositions.value = savedScroll
    }

    const savedPage = getCache<{ page: number; pageSize: number }>(CACHE_KEYS.PAGE, tenantId)
    if (savedPage) {
      currentPage.value = savedPage.page
      pageSize.value = savedPage.pageSize
    }

    const savedSearch = getCache<string>(CACHE_KEYS.SEARCH, tenantId)
    if (savedSearch) {
      currentFilters.value.search = savedSearch
    }

    const savedTxFilters = getCache<typeof transactionFilters.value>(CACHE_KEYS.FILTER_STATE, tenantId)
    if (savedTxFilters) {
      transactionFilters.value = savedTxFilters
    }
  }

  function saveAllState(tenantId: string) {
    if (!tenantId) return

    setCache(CACHE_KEYS.FILTERS, tenantId, currentFilters.value)
    setCache(CACHE_KEYS.VIEW_MODE, tenantId, viewMode.value)
    setCache(CACHE_KEYS.SCROLL, tenantId, tableScrollPositions.value)
    setCache(CACHE_KEYS.PAGE, tenantId, { page: currentPage.value, pageSize: pageSize.value })
    setCache(CACHE_KEYS.SEARCH, tenantId, currentFilters.value.search)
    setCache(CACHE_KEYS.FILTER_STATE, tenantId, transactionFilters.value)
  }
  ...
  function startAutoRefresh(tenantId: string) {
    if (autoRefreshTimer) {
      clearInterval(autoRefreshTimer)
      autoRefreshTimer = null
    }
    autoRefreshTimer = setInterval(async () => {
      if (tenantId && !isLoading.value && document.hidden === false) {
        try {
  ...
  const filteredStatsComputed = computed(() => {
    if (filteredStats.value.totalItems > 0) {
  ...
  function reset() {
    ...
    filterCache.clear()
    filteredStats.value = { totalItems: 0, totalQuantity: 0, lowStock: 0, criticalStock: 0, outOfStock: 0 }
    ...
    if (tenantId) {
      ...
      clearCache(CACHE_KEYS.CACHE_METADATA, tenantId)
    }
  ...
  async function fetchFilteredStats(params: {
    ...
    const tenantId = authStore.currentTenantId
    if (!tenantId) {
      return { totalItems: 0, totalQuantity: 0, lowStock: 0, criticalStock: 0, outOfStock: 0 }
    }
    ...
    try {
      ...
      return {
        ...
        lowStock: row.low_stock_count || 0,
        criticalStock: row.critical_stock_count || 0,
        outOfStock: row.out_of_stock_count || 0,
      }
    } catch (err) {
  ...
  async function fetchSummaryStats(params?: { search?: string; warehouseId?: string; color?: string; size?: string }): Promise<typeof summaryStats.value> {
    if (params?.search || params?.warehouseId || params?.color || params?.size) {
      const stats = await fetchFilteredStats({
        search: params?.search,
        warehouseId: params?.warehouseId,
  ...
  async function fetchAllItemsForExport(params: {
    ...
    const { search, warehouseId, status, color, size: itemSize, onProgress, isFullLoad = false } = params
    ...
    if (tenantId && isFullLoad) {
      ...
      setCache(CACHE_KEYS.CACHE_METADATA, tenantId, metadata)
    }
  ...
  async function fetchItemsPage(params: {
    ...
    if (!force && filterCache.get(cacheKey)) {
      const cached = filterCache.get(cacheKey)!
    ...
    try {
      ...
      // Update filtered stats after loading page
  ...
  async function fetchItems(): Promise<void> {
    ...
    const cacheResult = loadFromCache(tenantId)

    if (cacheResult.loaded && cacheResult.isComplete) {
      try {
        const hasChanged = await hasDataChanged(tenantId)
        if (hasChanged) {
          isLoading.value = true
          loadingProgress.value = 0
  ...
  async function fetchDashboardStats(warehouseId?: string): Promise<{
    totalItems: number
    totalQuantity: number
    lowStock: number
    criticalStock: number
    ...
    try {
      const { data, error } = await supabase.rpc('get_dashboard_stats', {
        p_tenant_id: tenantId,
        p_warehouse_id: warehouseId || null,
        p_allowed_warehouses: allowed.includes('all') ? null : allowed
      })
      ...
      return {
        ...
        warehouseStats: row.warehouse_stats || []
      }
    } catch (err) {
  ...
  async function fetchAlertItemsOptimized(warehouseId?: string): Promise<{
    ...
    try {
      let query = supabase
        ...
        .select(`
          *,
          warehouses:warehouse_id(name, name_ar)
        `)
        .eq('tenant_id', tenantId)
        .or('remaining_quantity.eq.0,remaining_quantity.lte.500')
  ...
  async function loadDashboardData(warehouseId?: string): Promise<{
    stats: Awaited<ReturnType<typeof fetchDashboardStats>>
    alerts: Awaited<ReturnType<typeof fetchAlertItemsOptimized>>
  }> {
    isLoading.value = true
    error.value = null
    loadingProgress.value = 0
    ...
    try {
      const [stats, alerts] = await Promise.all([
        fetchDashboardStats(warehouseId),
        fetchAlertItemsOptimized(warehouseId)
      ])
  ...
  async function transferItem(transferData: {
    ...
    const transactionId = transferData.transaction_id || `${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
  ...
  watch(
    ...
    async (tenantId) => {
      ...
      if (tenantId) {
        ...
        startAutoRefresh(tenantId)
        restorePersistedState(tenantId)
        
        const cacheResult = loadFromCache(tenantId)
        if (!cacheResult.isComplete && !isLoadingInitial.value && !initialLoadDone.value) {
  ...
  const initialTenantId = authStore.currentTenantId
  if (initialTenantId) {
    restorePersistedState(initialTenantId)
    startAutoRefresh(initialTenantId)
    
    const cacheResult = loadFromCache(initialTenantId)
    if (!cacheResult.isComplete && !isLoadingInitial.value && !initialLoadDone.value) {
      isLoadingInitial.value = true
      fetchItemsPage({
        page: 1,
  ...
  return {
    ...
    clearSearchResults,
    summaryStats,
    filteredStats: filteredStatsComputed,
    currentFilters,
    transactionFilters,
    viewMode,
    setItems,
