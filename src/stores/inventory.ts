...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  // ✅ FIX: searchResults now filters the current page items (client-side)
  // This is used as a fallback when server search is not needed
  const searchResults = computed(() => {
    const search = currentFilters.value.search?.trim().toLowerCase()
    if (!search) return itemsList.value
  ...
  const displayItems = computed(() => {
    // If server search is active and has results, use those
    if (needsServerSearch.value && serverSearchResults.value.length > 0) {
      return serverSearchResults.value
    }

    // If there's a search term, use client-side filtering on current page
  ...
  function updateLocalItem(updatedItem: InventoryItem) {
    ...
    if (tenantId) {
      ...
      // ✅ Clear filter cache when items change
  ...
  // ✅ NEW: Get cache key for filter combination
  ...
  async function hybridSearch(searchQuery: string) {
    ...
    const cachedResults = itemsList.value.filter(item => {
      const searchable = [
        item.name,
        item.code,
        item.color,
    ...
    needsServerSearch.value = true
    isSearchingServer.value = true

    try {
      // ✅ FIX: Use paginated search instead of fetching all items
      const serverResults = await fetchItemsPageForSearch({
        search: query,
        warehouseId: currentFilters.value.warehouseId || undefined,
        limit: 100,
      })
      serverSearchResults.value = serverResults
      needsServerSearch.value = false
      isSearchingServer.value = false
    } catch (error) {
      ...
      needsServerSearch.value = false
      isSearchingServer.value = false
      serverSearchResults.value = cachedResults
    }
  }

  // ✅ NEW: Paginated search for hybrid search
  async function fetchItemsPageForSearch(params: {
    search: string
    warehouseId?: string
    limit?: number
  }): Promise<InventoryItem[]> {
    const { search, warehouseId, limit = 100 } = params
    const tenantId = authStore.currentTenantId
  ...
  // ✅ FIXED: fetchItemsPage with filter-aware caching
  async function fetchItemsPage(params: {
    ...
    if (!force && filterCache.value.has(cacheKey)) {
      ...
      if (Date.now() - cached.timestamp < FILTER_CACHE_TTL) {
        ...
        // Update filters to match cached data
    ...
    // ✅ Also check persistent cache for first load (no filters)
    ...
    try {
      ...
      // ✅ Cache the result with filter combination
      ...
      // Only save to persistent cache for default (no filter) state
