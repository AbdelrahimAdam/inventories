...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  const MIN_CACHE_SIZE = 100

  function isCacheComplete(tenantId: string): boolean {
    if (!tenantId) return false
    const cachedItems = getCache<InventoryItem[]>(CACHE_KEYS.ITEMS, tenantId)
    if (!cachedItems) return false
    return cachedItems.length >= MIN_CACHE_SIZE
  }
  ...
  function loadFromCache(tenantId: string): CacheResult {
    ...
    try {
      ...
      if (cachedItems && cachedItems.length > 0) {
        ...
        const isComplete = cachedItems.length >= MIN_CACHE_SIZE
