...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  async function fetchItemsPage(params: {
    ...
    const shouldForce = force || isFirstLoad.value

    if (!shouldForce && filterCache.value.has(cacheKey)) {
      const cached = filterCache.value.get(cacheKey)!
    ...
    // ✅ On first load, force fresh data - skip cache check
    if (isFirstLoad.value) {
      isFirstLoad.value = false
    }

    if (!shouldForce && !cacheLoaded && !search && !warehouseId && !status && !color && !itemSize) {
