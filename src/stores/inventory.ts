// stores/inventory.ts - Complete file with working fetchItemsBulk
...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  // ✅ FIXED: Use the working get_items_page RPC instead of the broken bulk RPC
  async function fetchItemsBulk(warehouseId?: string): Promise<InventoryItem[]> {
    const tenantId = authStore.currentTenantId
    if (!tenantId) return []

    try {
      // Use the working get_items_page RPC with a large limit
      const { data, error } = await supabase.rpc('get_items_page', {
        p_tenant_id: tenantId,
        p_limit: 100000, // Large enough to get all items
        p_offset: 0,
        p_search: null,
        p_warehouse_id: warehouseId || null,
        p_status: null,
      ...
      // Populate the store with the loaded items
      if (items.length > 0) {
        const newMap = new Map<string, InventoryItem>()
        const newUniqueMap = new Map<string, string>()
      ...
      // Fallback to fetchItems if the RPC fails
      try {
        await fetchItems()
        if (warehouseId) {
