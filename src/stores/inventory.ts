...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  function updateLocalItem(updatedItem: InventoryItem) {
    ...
    // ✅ Update allItemsForStats as well
  ...
  function removeLocalItem(itemId: string) {
    ...
    // ✅ Remove from allItemsForStats as well
  ...
  function loadFromCache(tenantId: string): boolean {
    ...
    try {
      ...
      if (cachedItems && cachedItems.length > 0) {
        ...
        // ✅ Also update allItemsForStats from cache
  ...
  async function fetchItemsPage(params: {
    ...
    const { page, pageSize: pgSize = pageSize.value, search, warehouseId, status, color, size: itemSize, force = false, skipStats = false } = params
    const tenantId = authStore.currentTenantId
    ...
    try {
      ...
      // ✅ Update allItemsForStats with the fetched data
  ...
  async function performUpdate(
    ...
    // ✅ Update allItemsForStats optimistically
  ...
  async function addItem(itemData: Partial<InventoryItem> & { 
    ...
    try {
      ...
      // ✅ Add to allItemsForStats optimistically
      ...
      // ✅ Update allItemsForStats with the real item
      allItemsForStats.value = allItemsForStats.value.filter(item => item.id !== tempId)
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'items', filter: `tenant_id=eq.${authStore.currentTenantId}` }, async (payload) => {
        ...
        if (eventType === 'INSERT' && newRecord) {
          ...
          if (!itemsMap.value.has(newItem.id)) {
            ...
            // ✅ Add to allItemsForStats
            allItemsForStats.value = [...allItemsForStats.value, { ...newItem }]
          ...
          // ✅ Remove from allItemsForStats
          allItemsForStats.value = allItemsForStats.value.filter(item => item.id !== old.id)
