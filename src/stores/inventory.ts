// stores/inventory.ts - Complete file with fixed searchInventorySpark
import { defineStore } from 'pinia'
import { ref, computed, onScopeDispose, watch, shallowRef } from 'vue'
...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  const filterCache = new LRUCache<string, { data: InventoryItem[]; timestamp: number }>(50)
  ...
  async function fetchAllItemsForExport(params: {
    ...
    const { search, warehouseId, status, color, size: itemSize, onProgress, isFullLoad = false } = params    const allItems: InventoryItem[] = []
    let offset = 0
    const limit = 500
    let hasMore = true
