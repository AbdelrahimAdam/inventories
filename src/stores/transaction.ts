  ...
  async function verifyAndFixBalance(
    ...
    try {
      ...
      if (currentBalance !== calculatedBalance) {
        ...
        // ✅ FIXED: Use exposed updateItem method instead of updateLocalItem
        const updatedItem = await inventoryStore.fetchItemById(itemId)
        if (updatedItem) {
          await inventoryStore.updateItem(itemId, {
            remainingQuantity: calculatedBalance,
            cartonsCount: expectedCartons,
            singleBottlesCount: expectedSingles,
            totalAdded: totalIn,
            version: (item.version || 0) + 1
          })
        }
