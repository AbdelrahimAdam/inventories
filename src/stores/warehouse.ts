...
export const useWarehouseStore = defineStore('warehouse', () => {
  ...
  async function fetchWarehouses(force = false): Promise<void> {
    // ✅ Use cache if available and not forced
    const now = Date.now()
    if (!force && warehouses.value.length > 0 && (now - lastFetchTime.value) < CACHE_TTL) {
      return
    }
  ...
  // ✅ Updated: Use inventoryStore for stats instead of direct query
  async function getWarehouseStats(warehouseId: string): Promise<{ itemCount: number; totalStock: number }> {
    try {
      // Use inventory store's data if available
      const { useInventoryStore } = await import('./inventory')
      const inventoryStore = useInventoryStore()
      
      // If inventory data is loaded, use it
      if (inventoryStore.items.length > 0) {
        const items = inventoryStore.items.filter(item => item.warehouseId === warehouseId)
        const totalStock = items.reduce((sum, item) => sum + (item.remainingQuantity || 0), 0)
        return { itemCount: items.length, totalStock }
      }
      
      // Fallback to direct query if inventory not loaded
  ...
  // ✅ Updated: Use cache instead of fetching fresh
