<template>
  <div 
    class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative register-page"
    :dir="languageStore.direction"
  >
    <!-- Back Arrow -->
    <router-link 
      to="/landing" 
      class="absolute top-4 right-4 sm:top-6 sm:right-6 text-gray-600 dark:text-gray-400 hover:text-amber-600 dark:hover:text-amber-400 transition-colors duration-200 p-2 rounded-full hover:bg-white/50 dark:hover:bg-gray-800/50 backdrop-blur-sm z-10"
    ...
    <div class="register-card bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm border border-gray-300 dark:border-gray-600 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 p-6 sm:p-8 w-full max-w-2xl relative">
      <!-- Logo & Header -->
      <div class="text-center mb-6 sm:mb-8">
        ...
        <h1 class="brand-title text-2xl sm:text-3xl font-extrabold bg-gradient-to-r from-amber-600 to-amber-700 bg-clip-text text-transparent">
          P.commerce
        </h1>
      ...
      <form class="mt-6 sm:mt-8 space-y-4 sm:space-y-6" @submit.prevent="handleSubmit">
        ...
        <div class="bg-amber-50/80 dark:bg-amber-950/20 rounded-xl p-3 sm:p-4 border border-amber-200/70 dark:border-amber-800/30 shadow-sm hover:shadow-md transition-all duration-300">
          <div class="flex items-center gap-2 mb-3">
            <svg class="w-5 h-5 text-amber-600 dark:text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
            <h2 class="text-base sm:text-lg font-bold text-gray-800 dark:text-gray-200">
              {{ isRTL ? 'البيانات الأساسية للشركة' : 'Company Basic Information' }}
            </h2>
          </div>

          <div class="mb-3 sm:mb-4">
            <label class="block text-gray-700 dark:text-gray-300 text-sm font-semibold mb-2" :class="isRTL ? 'text-right' : 'text-left'">
            ...
            <input
              ...
              type="text"
              required
              class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition bg-white/90 dark:bg-gray-700/90 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 hover:border-amber-300 dark:hover:border-amber-700"
              :class="[isRTL ? 'text-right' : 'text-left', { 'border-red-500 dark:border-red-500': errors.companyName }]"
        ...
        <!-- Account Information -->
        <div class="bg-gray-50/80 dark:bg-gray-700/30 rounded-xl p-3 sm:p-4 border border-gray-200/70 dark:border-gray-600/30 shadow-sm hover:shadow-md transition-all duration-300">
          <div class="flex items-center gap-2 mb-3">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div>
              ...
              <input
                ...
                required
                class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition bg-white/90 dark:bg-gray-700/90 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 hover:border-amber-300 dark:hover:border-amber-700"
                :class="[isRTL ? 'text-right' : 'text-left', { 'border-red-500 dark:border-red-500': errors.name }]"
            ...
            <div>
              ...
              <input
                ...
                required
                class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition bg-white/90 dark:bg-gray-700/90 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 hover:border-amber-300 dark:hover:border-amber-700"
                :class="[isRTL ? 'text-right' : 'text-left', { 'border-red-500 dark:border-red-500': errors.phone }]"
            ...
            <div>
              ...
              <input
                ...
                required
                class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition bg-white/90 dark:bg-gray-700/90 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 hover:border-amber-300 dark:hover:border-amber-700"
                :class="[isRTL ? 'text-right' : 'text-left', { 'border-red-500 dark:border-red-500': errors.email }]"
            ...
            <div>
              ...
              <div class="relative">
                <input
                  ...
                  required
                  class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition bg-white/90 dark:bg-gray-700/90 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 hover:border-amber-300 dark:hover:border-amber-700"
                  :class="[isRTL ? 'text-right' : 'text-left', { 'border-red-500 dark:border-red-500': errors.password }]"
                  :placeholder="isRTL ? '••••••••' : '••••••••'"
                  dir="ltr"
                />
                <button
            ...
            <div class="sm:col-span-2">
              ...
              <input
                ...
                required
                class="w-full px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent transition bg-white/90 dark:bg-gray-700/90 text-gray-900 dark:text-white border-gray-300 dark:border-gray-600 hover:border-amber-300 dark:hover:border-amber-700"
                :class="[isRTL ? 'text-right' : 'text-left', { 'border-red-500 dark:border-red-500': errors.confirmPassword }]"
                :placeholder="isRTL ? '••••••••' : '••••••••'"
                dir="ltr"
              />
        ...
        <div class="flex items-start gap-2">
          <input 
            type="checkbox" 
            v-model="agreeTerms" 
            class="mt-1 rounded border-gray-300 dark:border-gray-600 text-amber-600 focus:ring-amber-500 dark:bg-gray-700 w-4 h-4 flex-shrink-0 cursor-pointer"
          />
          <span class="text-sm text-gray-600 dark:text-gray-400">
            {{ isRTL ? 'أوافق على' : 'I agree to the' }} 
            <a href="#" class="text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 hover:underline font-medium">{{ isRTL ? 'الشروط والأحكام' : 'Terms & Conditions' }}</a> 
            {{ isRTL ? 'و' : 'and' }}
            <a href="#" class="text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 hover:underline font-medium">{{ isRTL ? 'سياسة الخصوصية' : 'Privacy Policy' }}</a>
          </span>
        </div>

        <!-- Error / Success Messages -->
        <div v-if="errorMessage" class="rounded-lg bg-red-50 dark:bg-red-950/30 p-3 sm:p-4 border border-red-300 dark:border-red-700 shadow-sm">
          <div class="flex items-start gap-2">
            <svg class="h-5 w-5 text-red-400 dark:text-red-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div v-if="successMessage" class="rounded-lg bg-green-50 dark:bg-green-950/30 p-3 sm:p-4 border border-green-300 dark:border-green-700 shadow-sm">
          <div class="flex items-start gap-2">
            <svg class="h-5 w-5 text-green-400 dark:text-green-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          ...
          class="w-full flex justify-center items-center gap-2 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white py-3 rounded-xl font-semibold transition-all shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed text-base border border-amber-500/20"
        >
        ...
        <div class="space-y-2">
          <p class="text-sm text-gray-500 dark:text-gray-400 text-center">
            ...
            <router-link to="/login" class="text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 font-medium hover:underline">
              {{ isRTL ? 'تسجيل الدخول' : 'Login' }}
            </router-link>
          </p>
          <p class="text-sm text-gray-500 dark:text-gray-400 text-center">
            <router-link to="/landing" class="text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 font-medium hover:underline">
...
<style scoped>
.register-page {
  background: linear-gradient(135deg, #fbbf24 0%, #86b386 50%, #6b8c6b 100%);
}
...
.logo-wrapper {
  ...
  border-radius: 100%;
  background: linear-gradient(135deg, #d4a574 0%, #b8915a 100%);
  padding: 4px;
