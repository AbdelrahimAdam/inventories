<template>
  <div class="p-4 sm:p-6" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <!-- Loading Progress -->
    <div v-if="isLoading || (inventoryStore.isLoading && inventoryStore.loadingProgress < 100)" class="mb-4">
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          <div class="flex justify-between text-xs font-semibold text-gray-600 dark:text-gray-400 mb-0.5">
            ...
            <span>{{ Math.round(inventoryStore.loadingProgress || 0) }}%</span>
          </div>
          <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
            <div class="bg-gradient-to-r from-amber-500 to-amber-600 h-1.5 rounded-full transition-all duration-500" :style="{ width: Math.min(inventoryStore.loadingProgress || 0, 100) + '%' }"></div>
          </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
              {{ isLoading ? '...' : formatNumber(totalItems) }}
            </p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 sm:p-6 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
              {{ isLoading ? '...' : formatNumber(totalUnits) }}
            </p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 sm:p-6 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
              {{ isLoading ? '...' : formatNumber(totalWarehouses) }}
            </p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 sm:p-6 border border-gray-200 dark:border-gray-700 hover:shadow-md transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">
              {{ isLoading ? '...' : formatNumber(totalTransactions) }}
            </p>
          </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
      <div class="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        ...
        <span v-if="!isLoading" class="text-xs text-gray-500 dark:text-gray-400">{{ recentItems.length }} صنف</span>
      </div>

      <!-- Loading State -->
      <div v-if="isLoading && inventoryStore.items.length === 0" class="p-8 text-center">
        <div class="animate-spin rounded-full h-8 w-8 border-2 border-amber-500 border-t-transparent mx-auto mb-2"></div>
        <p class="text-sm text-gray-500 dark:text-gray-400">جاري تحميل الأصناف...</p>
      </div>

      <!-- Desktop Table View -->
      <div v-else-if="recentItems.length > 0" class="hidden sm:block overflow-x-auto">
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="item in recentItems" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              <td class="px-4 sm:px-6 py-3 sm:py-4 text-sm font-medium text-gray-900 dark:text-white">{{ item.name }}</td>
              <td class="px-4 sm:px-6 py-3 sm:py-4 text-sm text-gray-600 dark:text-gray-400">{{ item.code }}</td>
              <td class="px-4 sm:px-6 py-3 sm:py-4 text-sm text-gray-600 dark:text-gray-400">{{ getWarehouseName(item.warehouseId) }}</td>
      ...
      <!-- Mobile Card View -->
      <div v-else-if="recentItems.length > 0" class="sm:hidden divide-y divide-gray-200 dark:divide-gray-700">
      ...
      <div v-else-if="!isLoading" class="p-8 text-center">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">لا توجد أصناف ضمن المستودعات المسموح بها</p>
      </div>
    </div>
  </div>
</template>
...
const totalUnits = computed(() => restrictedItems.value.reduce((sum, i) => sum + (i.remainingQuantity || 0), 0))
...
async function loadDashboardData(force = false) {
  ...
  try {
    ...
    if (!force) {
      ...
      if (cacheLoaded && inventoryStore.items.length > 0) {
