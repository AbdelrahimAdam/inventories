...
// ✅ Optimized: Load inventory with cache check
async function loadInventory(force = false) {
  ...
  if (dataAvailable && !force) {
    ...
    console.log('📦 ItemsList: Using cached data')
  ...
  try {
    ...
    syncFiltersFromStore()
    console.log('📦 ItemsList: Fresh data loaded')
  } catch (error) {
...
// ✅ Optimized onActivated - check cache first
onActivated(async () => {
  const tenantId = authStore.currentTenantId
  if (!tenantId) return
  ...
  if (inventoryStore.items.length > 0) {
    ...
    console.log('📦 ItemsList: Using existing data')
    return
  }
  ...
  if (dataAvailable) {
    ...
    console.log('📦 ItemsList: Loaded from cache')
    return
  }
...
// ✅ Optimized watches - removed unnecessary dataVersion watch
