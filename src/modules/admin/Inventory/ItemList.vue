...
// ✅ FIXED: Use searchInventorySpark properly
async function performSearch(searchTerm: string) {
  ...
  try {
    ...
    // Update the itemsMap using the store's internal method
    const newMap = new Map<string, InventoryItem>()
    for (const item of results) {
      newMap.set(item.id, item)
    }
    
    // Use a type assertion to access the ref's value
    ;(inventoryStore.itemsMap as any).value = newMap
    totalCount.value = results.length
    currentPage.value = 1
    
    // Update summary stats to reflect search results
