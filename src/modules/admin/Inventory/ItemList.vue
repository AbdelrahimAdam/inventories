    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-3 sm:p-4 mb-4">
      <div class="relative mb-3">
        ...
        <input
          ...
          class="w-full pl-9 pr-10 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[40px]"
        ...
        <button 
          ...
          @click="clearSearch"
          class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 min-h-[36px] px-2"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function goToPage(page: number) {
  if (page >= 1 && page <= totalPages.value) {
    ...
    if (inventoryStore.viewMode === 'paginated') fetchPage(false) // ✅ Use cache, don't force
    nextTick(() => {
...
function resetFilters() {
  ...
  // ✅ Don't force fetch - let cache handle it!
  // The store will use cached data for the default (no filter) state
