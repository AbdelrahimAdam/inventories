...
// ✅ FIXED: Server-side search - Use the store's method to update items
async function performSearch(searchTerm: string) {
  ...
  try {
    ...
    // ✅ FIX: Use the store's internal map directly
    // Since itemsMap is a ref, we need to access .value
    const itemsMap = inventoryStore.itemsMap
    if (itemsMap && typeof itemsMap === 'object' && 'value' in itemsMap) {
      const newMap = new Map<string, InventoryItem>()
      for (const item of results) {
        newMap.set(item.id, item)
      }
      // This is the correct way to set a ref's value
      ;(itemsMap as any).value = newMap
    } else {
      // Fallback: if itemsMap is not a ref, try to assign directly
      // This shouldn't happen, but just in case
      console.warn('itemsMap is not a ref, trying direct assignment')
    }
