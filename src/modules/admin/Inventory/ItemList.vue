<template>
  <div class="container mx-auto px-3 sm:px-4 py-4 sm:py-8" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <!-- Items Table - Scrollable Container (taller, shows ~10 rows) -->
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      <div class="overflow-x-auto">
        <!-- Scrollable table body with max-height for ~10 rows -->
        <div class="overflow-y-auto" style="max-height: 550px; scroll-behavior: smooth;">
...
const filteredItems = computed(() => {
  ...
  if (filters.value.status === 'in_stock') {
    ...
    items = items.filter(item => item.remainingQuantity >= 10 && item.remainingQuantity <= 500)
...
const exportToExcel = () => {
  const exportData = filteredItems.value.map(item => ({
    ...
    'Per Carton': item.perCartonCount,
    'Singles': item.singleBottlesCount,
    'Total Quantity': item.remainingQuantity,
    'Supplier': item.supplier || '—',
    'Status': getStatusText(item.remainingQuantity)
  }))
