...
const hasMoreToShow = computed(() => {
  return inventoryStore.items.length < inventoryStore.totalCount
})
...
// ============================================================
// FIXED: onActivated - Use cached data, no skeleton on navigation
// ============================================================
onActivated(async () => {
  // If data exists, just show it immediately
...
// ============================================================
// FIXED: onMounted - Only fetch if store is empty
// ============================================================
onMounted(async () => {
  ...
  // If data exists, show it immediately
  if (inventoryStore.items.length > 0) {
    tableLoading.value = false
    dataLoaded = true
  }
  
  // Only fetch if store has no items
  if (inventoryStore.items.length === 0 && authStore.currentTenantId) {
    await fetchPage()
  }
