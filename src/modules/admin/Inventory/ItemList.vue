  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-3 sm:p-4 mb-4">
      <div class="relative mb-3">
        ...
        <button v-if="localSearchInput.length > 0 && !isSearchingServer" @click="clearSearch" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 min-h-[44px] px-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="localSearchInput.length > 0" class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
        <span>{{ displayItems.length }} نتيجة <span v-if="isSearchingServer">(جارٍ البحث في الخادم...)</span></span>
        <span v-if="displayItems.length === 0 && localSearchInput.length >= 2 && !isSearchingServer" class="text-amber-600 dark:text-amber-400">لا توجد نتائج مطابقة</span>
      </div>
      ...
      <div v-if="colorFilterToggle === 'specific' || sizeFilterToggle === 'specific'" class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
        <div v-if="colorFilterToggle === 'specific'" class="relative">
          <input type="text" v-model="filters.color" @change="onFilterChange" placeholder="اكتب اللون..." class="w-full pl-10 pr-3 py-1.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px]" />
        ...
        <div v-if="sizeFilterToggle === 'specific'" class="relative">
          <input type="text" v-model="filters.size" @change="onFilterChange" placeholder="اكتب المقاس..." class="w-full px-4 py-1.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px]" />
        </div>
      </div>
    </div>
    ...
    <div class="flex justify-end mb-3">
      <div class="inline-flex rounded-xl shadow-sm" role="group">
        <button @click="setViewMode('paginated')" :class="['px-4 py-2 text-xs sm:text-sm font-bold rounded-r-xl border min-h-[44px]', inventoryStore.viewMode === 'paginated' ? 'bg-amber-600 text-white border-amber-600' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700']">عرض بالصفحات</button>
        <button @click="setViewMode('view-all')" :disabled="isLoadingAll" :class="['px-4 py-2 text-xs sm:text-sm font-bold rounded-l-xl border min-h-[44px]', inventoryStore.viewMode === 'view-all' ? 'bg-amber-600 text-white border-amber-600' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700']">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      <div class="overflow-x-auto">
        <div ref="tableContainerRef" class="relative" :style="tableContainerStyle" @scroll="onTableScroll">
                ...
                <tr v-for="item in displayItems" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  ...
                  <td class="px-3 py-3 text-center align-middle w-24">
                    <div class="action-menu-container relative inline-block">
                      <button @click.stop="toggleActionMenu(item.id, $event)" class="px-3 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl transition-all text-xs font-bold inline-flex items-center justify-center gap-1 shadow-md whitespace-nowrap min-h-[44px]" aria-label="إجراءات">
    ...
    <div v-if="inventoryStore.viewMode === 'paginated' && inventoryStore.summaryStats.totalItems > inventoryStore.pageSize" class="flex flex-col sm:flex-row justify-between items-center gap-3 mt-4">
      <div class="text-xs font-semibold text-gray-600 order-2 sm:order-1">عرض {{ ((currentPage - 1) * inventoryStore.pageSize) + 1 }} إلى {{ Math.min(currentPage * inventoryStore.pageSize, inventoryStore.summaryStats.totalItems) }} من {{ formatNumber(inventoryStore.summaryStats.totalItems) }} صنف</div>
...
function onFilterChange() {
  const oldWarehouseId = inventoryStore.currentFilters.warehouseId
  ...
  const warehouseChanged = oldWarehouseId !== filters.value.warehouseId
  fetchPage(false, !warehouseChanged)
}
...
async function fetchPage(force: boolean = false, skipStats: boolean = false) {
