  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
      ...
      <div class="flex gap-2 w-full sm:w-auto flex-wrap">
        ...
        <button
          ...
          class="flex-1 sm:flex-none bg-amber-700 hover:bg-amber-800 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-2 shadow-md text-xs sm:text-sm font-semibold min-h-[44px]"
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-3 sm:p-4 mb-4">
      <div class="relative mb-3">
        ...
        <input
          ...
          class="w-full pl-9 pr-10 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px]"
          :class="{ 'border-amber-500 ring-2 ring-amber-500': isSearchingServer }"
        />
        <div v-if="isSearchingServer" class="absolute right-3 top-1/2 transform -translate-y-1/2">
          <div class="animate-spin rounded-full h-4 w-4 border-2 border-amber-500 border-t-transparent"></div>
        </div>
        <button 
          v-if="localSearchInput.length > 0 && !isSearchingServer"
          @click="clearSearch"
          class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 min-h-[44px] px-2"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="localSearchInput.length > 0" class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
        <span>
          ...
          <span v-if="isSearchingServer">(جارٍ البحث في الخادم...)</span>
        </span>
        <span v-if="displayItems.length === 0 && localSearchInput.length >= 2 && !isSearchingServer" class="text-amber-600 dark:text-amber-400">
      ...
      <div class="flex justify-end">
        <button @click="resetFilters" class="px-4 py-1.5 border border-gray-300 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-sm font-semibold text-gray-700 dark:text-gray-300 min-h-[44px]">
          إعادة تعيين
        </button>
      </div>
      ...
      <div v-if="colorFilterToggle === 'specific' || sizeFilterToggle === 'specific'" class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
        <div v-if="colorFilterToggle === 'specific'" class="relative">
          <input
            ...
            class="w-full pl-10 pr-3 py-1.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px]"
          />
        ...
        <div v-if="sizeFilterToggle === 'specific'" class="relative">
          <input
            ...
            class="w-full px-4 py-1.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px]"
          />
        </div>
      </div>
    </div>
    ...
    <div class="flex justify-end mb-3">
      <div class="inline-flex rounded-xl shadow-sm" role="group">
        <button
          ...
          :class="[
            'px-4 py-2 text-xs sm:text-sm font-bold rounded-r-xl border min-h-[44px]',
        ...
        <button
          ...
          :class="[
            'px-4 py-2 text-xs sm:text-sm font-bold rounded-l-xl border min-h-[44px]',
        ...
        >
          <table class="w-full min-w-[1000px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <template v-if="tableLoading && displayItems.length === 0">
              ...
              <template v-else>
                  ...
                  <td class="px-3 py-3 text-center align-middle w-24">
                    <div class="action-menu-container relative inline-block">
                      <button 
                        ...
                        class="px-3 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl transition-all text-xs font-bold inline-flex items-center justify-center gap-1 shadow-md whitespace-nowrap min-h-[44px]"
                        aria-label="إجراءات"
                      >
                ...
                <tr v-if="displayItems.length === 0 && !inventoryStore.isLoading && !tableLoading && !isSearchingServer">
          ...
          <div v-if="isSearchingServer" class="text-center py-3">
    ...
    <div v-if="inventoryStore.viewMode === 'paginated' && inventoryStore.summaryStats.totalItems > inventoryStore.pageSize" class="flex flex-col sm:flex-row justify-between items-center gap-3 mt-4">
      ...
      <div class="flex items-center gap-2 order-3 sm:order-2">
        ...
        <select v-model="inventoryStore.pageSize" @change="changePageSize" class="px-2 py-1 border border-gray-300 rounded-xl text-xs font-medium bg-white dark:bg-gray-700 text-gray-900 dark:text-white min-h-[44px]">
    ...
    <Teleport to="body">
      <div 
        ...
        ref="dropdownRef"
        class="fixed z-[10000] bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-y-auto min-w-[200px] max-w-[280px]"
        :style="dropdownStyle"
        @click.stop
        role="menu"
      ...
      >
        <div class="py-1">
          <router-link 
            :to="`/inventory/items/${activeActionMenu}`" 
            class="w-full px-4 py-3 text-right text-sm font-semibold hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-3 min-h-[48px] transition-colors"
          ...
          <router-link 
            v-if="selectedItemForAction && authStore.canEditItem(selectedItemForAction.warehouseId)" 
            :to="`/inventory/items/${activeActionMenu}?edit=true`" 
            class="w-full px-4 py-3 text-right text-sm font-semibold hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-3 min-h-[48px] transition-colors"
            :class="languageStore.isRTL ? 'justify-end' : 'justify-start'"
            @click="closeActionMenu"
            role="menuitem"
          ...
          <button 
            @click="selectedItemForAction && exportSingleCard(selectedItemForAction); closeActionMenu()" 
            class="w-full px-4 py-3 text-right text-sm font-semibold hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-3 min-h-[48px] transition-colors"
            :class="languageStore.isRTL ? 'justify-end' : 'justify-start'"
            :disabled="isExporting"
            role="menuitem"
          ...
          <button 
            v-if="selectedItemForAction && authStore.canEditItem(selectedItemForAction.warehouseId)" 
            @click="selectedItemForAction && openAddTransactionModal(selectedItemForAction); closeActionMenu()" 
            class="w-full px-4 py-3 text-right text-sm font-semibold hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-3 min-h-[48px] transition-colors"
            :class="languageStore.isRTL ? 'justify-end' : 'justify-start'"
            role="menuitem"
          >
          ...
          <button 
            v-if="selectedItemForAction && authStore.canEditItem(selectedItemForAction.warehouseId)" 
            @click="selectedItemForAction && openTransferModal(selectedItemForAction); closeActionMenu()" 
            class="w-full px-4 py-3 text-right text-sm font-semibold hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-3 min-h-[48px] transition-colors"
            :class="languageStore.isRTL ? 'justify-end' : 'justify-start'"
            role="menuitem"
          >
          ...
          <button 
            v-if="selectedItemForAction && authStore.canEditItem(selectedItemForAction.warehouseId)" 
            @click="selectedItemForAction && openDispatchModal(selectedItemForAction); closeActionMenu()" 
            class="w-full px-4 py-3 text-right text-sm font-semibold hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-3 min-h-[48px] transition-colors"
            :class="languageStore.isRTL ? 'justify-end' : 'justify-start'"
            role="menuitem"
          >
          ...
          <button 
            @click="selectedItemForAction && openBalanceVerification(selectedItemForAction); closeActionMenu()" 
            class="w-full px-4 py-3 text-right text-sm font-semibold hover:bg-gray-100 dark:hover:bg-gray-700 flex items-center gap-3 min-h-[48px] transition-colors"
            :class="languageStore.isRTL ? 'justify-end' : 'justify-start'"
            role="menuitem"
          >
          ...
          <button 
            ...
            @click="selectedItemForAction && confirmDelete(selectedItemForAction); closeActionMenu()" 
            class="w-full px-4 py-3 text-right text-sm font-semibold text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 flex items-center gap-3 min-h-[48px] transition-colors"
            :class="languageStore.isRTL ? 'justify-end' : 'justify-start'"
            role="menuitem"
          >
            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const displayItems = computed(() => inventoryStore.displayItems)
...
function getCurrentFiltersFromStore() {
  return {
...
function debouncedSearch() {
  ...
  inventoryStore.currentFilters.search = searchTerm
  ...
  searchDebounceTimer = setTimeout(() => {
    inventoryStore.hybridSearch(searchTerm)
    currentPage.value = 1
    isSearchingServer.value = false
  }, 400)
}
...
function syncFiltersFromStore() {
  const storeFilters = getCurrentFiltersFromStore()
...
async function fetchPage(force: boolean = false) {
  ...
  try {
    ...
    await inventoryStore.fetchItemsPage({
      ...
      warehouseId: storeFilters.warehouseId || undefined,
      status: storeFilters.status || undefined,
      color: storeFilters.color || undefined,
      size: storeFilters.size || undefined,
      force
    })
...
// ⭐ FIXED: Use inventoryStore.refreshItems() for specific items
async function refreshInventory() {
  ...
  try {
    ...
    // ⭐ Use the optimized refresh method - only fetches changed items
    // If we don't have specific item IDs, fetch the current page
    const tenantId = authStore.currentTenantId
    if (tenantId) {
      // Get the IDs of items currently displayed
      const currentItemIds = displayItems.value.map(item => item.id)
      if (currentItemIds.length > 0) {
        // Refresh only the items currently visible
        await inventoryStore.refreshItems(currentItemIds)
      } else {
        // If no items visible, fetch the page normally
...
// ⭐ NEW: Watch for inventory data version changes
watch(
  () => inventoryStore.dataVersion,
  (newVersion, oldVersion) => {
    if (newVersion !== oldVersion && dataLoaded) {
      console.log('🔄 Inventory data version changed, refreshing view...')
...
function goToPage(page: number) {
  if (page >= 1 && page <= totalPages.value) {
    ...
    if (inventoryStore.viewMode === 'paginated') fetchPage(false)
    nextTick(() => {
...
const positionDropdown = (buttonRect: DOMRect) => {
...
// ============================================================
// REST OF THE COMPUTED AND FUNCTIONS
// ============================================================
...
const exportItemCount = computed(() => displayItems.value.length)
...
const onTransferSuccess = async () => { await refreshInventory() }
const onDispatchSuccess = async () => { await refreshInventory() }
const onTransactionSuccess = async () => { await refreshInventory() }

const imagePreviewUrl = ref<string | null>(null)
const openImagePreview = (url: string) => { imagePreviewUrl.value = url }

// ============================================================
// WINDOW EVENT LISTENERS FOR DROPDOWN POSITIONING
// ============================================================
...
onActivated(async () => {
  if (displayItems.value.length > 0 && dataLoaded) {
    tableLoading.value = false
    isSearchingServer.value = false
  ...
  if (displayItems.value.length === 0 && authStore.currentTenantId) {
    await fetchPage()
  } else {
    tableLoading.value = false
  }
