  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <Teleport to="body">
      <div 
        ...
        class="fixed z-[1000] bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden min-w-[200px] max-w-[280px]"
        :style="dropdownStyle"
        @click.stop
        role="menu"
...
const toggleActionMenu = (itemId: string, event: MouseEvent) => {
  ...
  requestAnimationFrame(() => {
    ...
    const enoughBelow = spaceBelow >= dropdownHeight + margin
    const enoughAbove = spaceAbove >= dropdownHeight + margin
    
    if (enoughBelow || (!enoughAbove && spaceBelow >= spaceAbove)) {
    ...
    const maxWidth = Math.min(windowWidth - 32, 280)
    const actualWidth = Math.min(dropdownWidth, maxWidth)
    
    if (languageStore.isRTL) {
      let rightPos = windowWidth - rect.right
      if (rightPos + actualWidth > windowWidth - 16) {
        rightPos = windowWidth - actualWidth - 16
      }
      if (rightPos < 16) rightPos = 16
      right = rightPos
    } else {
      let leftPos = rect.left + window.scrollX
      if (leftPos + actualWidth > windowWidth - 16) {
        leftPos = windowWidth - actualWidth - 16
      }
