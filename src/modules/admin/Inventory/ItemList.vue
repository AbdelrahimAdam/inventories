...
// ✅ FIXED: Use fetchItemsPage with search parameter instead of direct Map manipulation
async function performSearch(searchTerm: string) {
  if (inventoryStore.isLoading) return
  
  if (viewMode.value === 'view-all') {
    // For view-all mode, load all matching items
    isLoadingData.value = true
    try {
      await inventoryStore.fetchAllItemsForExport({
        search: searchTerm,
    ...
    // For paginated mode, use fetchItemsPage with search
    isLoadingData.value = true
    try {
      await inventoryStore.fetchItemsPage({
        page: 1,
