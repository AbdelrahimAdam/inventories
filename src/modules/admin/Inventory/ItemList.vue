  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-2 sm:gap-3 mb-4">
      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(displayStats.totalItems)">{{ formatNumber(displayStats.totalItems) }}</div>
      ...
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(displayStats.totalQuantity)">{{ formatNumber(displayStats.totalQuantity) }}</div>
      ...
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(displayStats.lowStock)">{{ formatNumber(displayStats.lowStock) }}</div>
      ...
      <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(displayStats.criticalStock)">{{ formatNumber(displayStats.criticalStock) }}</div>
      ...
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(displayStats.outOfStock)">{{ formatNumber(displayStats.outOfStock) }}</div>
    ...
    <div v-if="inventoryStore.isLoading" class="mb-3 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-lg p-2.5">
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          ...
          <div class="text-xs text-blue-600 dark:text-blue-400 mt-1 font-semibold">
            جاري تحميل الأصناف... {{ Math.round(inventoryStore.loadingProgress) }}%
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      <div class="overflow-x-auto">
        <div ref="tableContainerRef" class="relative overflow-y-auto" :style="tableContainerStyle" @scroll="onTableScroll">
          ...
          <div v-if="inventoryStore.isLoading && displayItems.length > 0" class="text-center py-3">
            <div class="animate-spin rounded-full h-5 w-5 border-2 border-amber-500 border-t-transparent mx-auto"></div>
            <p class="text-xs font-semibold text-gray-500 mt-1">جاري التحميل...</p>
          </div>
        </div>
      </div>
    </div>
