...
const loadItemsWithRetry = async (): Promise<boolean> => {
  const maxRetries = 3
  let retries = 0
  
  while (retries < maxRetries) {
    try {
      console.log(`🔄 Loading items attempt ${retries + 1}/${maxRetries}...`)
      ...
      if (inventoryStore.items.length > 0) {
        if (inventoryStore.items.length >= 100) {
          console.log(`✅ Loaded ${inventoryStore.items.length} items successfully`)
          return true
        } else {
          retries++
          if (retries < maxRetries) {
            const delay = 1000 * Math.pow(2, retries)
            console.warn(`⚠️ Only ${inventoryStore.items.length} items loaded, retrying in ${delay}ms...`)
        ...
        if (retries < maxRetries) {
          const delay = 1000 * Math.pow(2, retries)
          console.warn(`⚠️ No items loaded, retrying in ${delay}ms...`)
          await new Promise(resolve => setTimeout(resolve, delay))
        }
      }
    } catch (error) {
      retries++
      console.warn(`❌ Load attempt ${retries} failed:`, error)
      if (retries < maxRetries) {
        const delay = 1000 * Math.pow(2, retries)
        await new Promise(resolve => setTimeout(resolve, delay))
      }
  ...
  console.error('❌ Failed to load items after multiple attempts')
  return false
}
...
async function loadInventory() {
  ...
  try {
    ...
    const hasEnoughItems = inventoryStore.items.length >= 100
    const shouldForceRefresh = inventoryStore.items.length > 0 && !hasEnoughItems
    
    if (inventoryStore.items.length === 0 || shouldForceRefresh) {
      if (shouldForceRefresh) {
        console.warn(`⚠️ Only ${inventoryStore.items.length} items in cache, forcing refresh...`)
        inventoryStore.filterCache.value.clear()
      }
      
      const loaded = await loadItemsWithRetry()
      
      if (!loaded) {
        try {
          console.log('🔄 Final attempt using fetchItems...')
...
async function refreshInventory() {
  ...
  try {
    ...
    if (inventoryStore.items.length < 100 && inventoryStore.items.length > 0) {
      console.warn(`⚠️ Only ${inventoryStore.items.length} items loaded after refresh, retrying...`)
