  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="flex justify-between items-center mb-3">
      <div class="text-sm font-semibold text-gray-600 dark:text-gray-400">
        إجمالي الأصناف: {{ formatNumber(totalItemsCount) }}
        <span v-if="filteredItems.length < totalItemsCount" class="text-amber-600 dark:text-amber-400">
          ({{ formatNumber(filteredItems.length) }} بعد التصفية)
        </span>
      </div>
      <div class="flex gap-2">
        <button @click="setViewMode('paginated')" :class="['px-4 py-2 text-xs sm:text-sm font-bold rounded-xl border min-h-[44px]', viewMode === 'paginated' ? 'bg-amber-600 text-white border-amber-600' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700']">
          عرض بالصفحات
        </button>
        <button @click="setViewMode('view-all')" :class="['px-4 py-2 text-xs sm:text-sm font-bold rounded-xl border min-h-[44px]', viewMode === 'view-all' ? 'bg-amber-600 text-white border-amber-600' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700']">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      <div class="overflow-x-auto">
        <div ref="tableContainerRef" class="relative" :style="tableContainerStyle" @scroll="onTableScroll">
          ...
          <div v-if="viewMode === 'view-all'" class="relative" :style="virtualScrollStyle">
            <div :style="{ height: totalHeight + 'px' }"></div>
            <table class="w-full min-w-[1000px] absolute top-0 left-0">
              ...
              <tbody>
                <template v-if="isLoading && filteredItems.length === 0">
...
const itemHeight = 65 // Approximate height per row
const overscan = 10
