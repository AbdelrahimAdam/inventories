...
const displayStats = computed(() => {
  return inventoryStore.filteredStats
})
...
async function refreshFilteredStats() {
  ...
  try {
    await inventoryStore.fetchFilteredStats({
      search: isSearchActive.value ? localSearchInput.value : undefined,
...
function onFilterChange() {
  ...
  // Refresh stats immediately when filters change
...
function debouncedSearch() {
  ...
  searchDebounceTimer = setTimeout(() => {
    ...
    // Refresh stats when search changes
    refreshFilteredStats()
...
async function performHybridSearch(searchTerm: string) {
  ...
  try {
    ...
    if (viewMode.value === 'view-all' && inventoryStore.items.length >= inventoryStore.totalItems) {
      ...
      inventoryStore.setItems(Array.from(newMap.values()))
    ...
    if (cachedMatches.length > 0) {
      ...
      inventoryStore.setItems(Array.from(mergedMap.values()))
    ...
    inventoryStore.setItems(results)
...
async function loadInventory(force = false) {
  ...
  if (inventoryStore.items.length > 0 && !force && !isSearchActive.value) {
...
function confirmExportAllCards() {
  if (inventoryStore.filteredStats.totalItems === 0) {
...
onActivated(async () => {
  if (authStore.isAuthenticated) {
    ...
    if (tenantId) {
      ...
      if (inventoryStore.items.length > 0 && !isSearchActive.value) {
        syncFiltersFromStore()
        totalCount.value = inventoryStore.totalCount || inventoryStore.items.length
        await inventoryStore.fetchSummaryStats({
          warehouseId: inventoryStore.currentFilters.warehouseId || undefined
