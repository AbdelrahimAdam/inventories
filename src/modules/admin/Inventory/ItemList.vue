  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-3 sm:p-4 mb-4">
      ...
      <div v-if="localSearchInput.length > 0" class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
        <span>
          {{ inventoryStore.items.length }} نتيجة 
          <span v-if="isSearchingServer">(جارٍ البحث في الخادم...)</span>
        </span>
        <span v-if="inventoryStore.items.length === 0 && localSearchInput.length >= 2 && !isSearchingServer" class="text-amber-600 dark:text-amber-400">
          ...
          <table class="w-full min-w-[1000px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <template v-else>
                ...
                <tr v-if="inventoryStore.items.length === 0 && !inventoryStore.isLoading && !tableLoading && !isSearchingServer">
...
const displayItems = computed(() => inventoryStore.items)
