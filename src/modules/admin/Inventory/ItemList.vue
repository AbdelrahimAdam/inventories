<template>
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <!-- Teleported Dropdown - Fixed Positioning -->
    <Teleport to="body">
      <div 
        v-if="activeActionMenu" 
        class="fixed z-[10000] bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-y-auto"
        :style="dropdownStyle"
        @click.stop
        role="menu"
...
const toggleActionMenu = (itemId: string, event: MouseEvent) => {
  ...
  const actualWidth = Math.min(240, viewportWidth - (margin * 2))
  
  if (languageStore.isRTL) {
    let rightPos = viewportWidth - rect.right
    if (rightPos + actualWidth > viewportWidth - margin) {
      rightPos = viewportWidth - actualWidth - margin
    }
    if (rightPos < margin) {
      rightPos = margin
    }
    ...
    if (leftPos + actualWidth > viewportWidth - margin) {
      leftPos = viewportWidth - actualWidth - margin
    }
    if (leftPos < margin) {
      leftPos = margin
    }
  ...
  dropdownPosition.value = { top, left: left ?? 0, right: right ?? 0 }
