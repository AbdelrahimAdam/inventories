  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="flex justify-end mb-3">
      <div class="inline-flex rounded-xl shadow-sm" role="group">
        <button @click="setViewMode('paginated')" :class="['px-4 py-2 text-xs sm:text-sm font-bold rounded-r-xl border min-h-[44px]', viewMode === 'paginated' ? 'bg-amber-600 text-white border-amber-600' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700']">عرض بالصفحات</button>
        <button @click="setViewMode('view-all')" :class="['px-4 py-2 text-xs sm:text-sm font-bold rounded-l-xl border min-h-[44px]', viewMode === 'view-all' ? 'bg-amber-600 text-white border-amber-600' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700']">
          عرض الكل ({{ totalItemsCount }})
        </button>
      </div>
    </div>
        ...
        <div ref="tableContainerRef" class="relative" :style="tableContainerStyle" @scroll="onTableScroll">
          <table class="w-full min-w-[1000px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <template v-if="isLoading && displayItems.length === 0">
              ...
              <template v-else>
                ...
                <tr v-if="displayItems.length === 0 && !isLoading && !isSearching">
    ...
    <div v-if="viewMode === 'paginated' && totalItemsCount > pageSize" class="pagination-container">
      <div class="text-xs font-semibold text-gray-600 order-2 sm:order-1">
        عرض {{ ((currentPage - 1) * pageSize) + 1 }} إلى {{ Math.min(currentPage * pageSize, totalItemsCount) }} من {{ formatNumber(totalItemsCount) }} صنف
      </div>
...
const filteredItems = computed(() => {
  ...
  if (filters.value.size) {
    items = items.filter(item => 
      item.size?.toLowerCase().includes(filters.value.size.toLowerCase())
    )
  }
...
const hasMoreToShow = computed(() => {
  return viewMode.value === 'view-all' && searchResults.value.length < filteredItems.value.length
})
...
async function refreshInventory() {
  ...
  try {
    const warehouseId = filters.value.warehouseId || undefined
...
watch(() => inventoryStore.dataVersion, () => {
  if (inventoryStore.items.length > 0) {
    syncFiltersFromStore()
  }
})
