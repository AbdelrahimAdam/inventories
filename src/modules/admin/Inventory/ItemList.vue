  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-3 sm:p-4 mb-4">
      <div class="relative mb-3">
        ...
        <input type="text" v-model="localSearchInput" placeholder="بحث بالاسم أو الكود أو المورد أو الموقع..." class="w-full pl-9 pr-10 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px]" :class="{ 'border-amber-500 ring-2 ring-amber-500': inventoryStore.isSearching }" />
        <div v-if="inventoryStore.isSearching" class="absolute right-3 top-1/2 transform -translate-y-1/2">
          <div class="animate-spin rounded-full h-4 w-4 border-2 border-amber-500 border-t-transparent"></div>
        </div>
        <button v-if="localSearchInput.length > 0 && !inventoryStore.isSearching" @click="clearSearch" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 min-h-[44px] px-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="localSearchInput.length > 0" class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
        <span>{{ displayItems.length }} نتيجة <span v-if="inventoryStore.isSearching">(جارٍ البحث...)</span></span>
        <span v-if="displayItems.length === 0 && localSearchInput.length >= 2 && !inventoryStore.isSearching" class="text-amber-600 dark:text-amber-400">لا توجد نتائج مطابقة</span>
      </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      <div class="overflow-x-auto">
        <div ref="tableContainerRef" class="relative" :style="tableContainerStyle" @scroll="onTableScroll">
          <table class="w-full min-w-[1000px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <template v-else>
                ...
                <tr v-if="displayItems.length === 0 && !inventoryStore.isLoading && !tableLoading && !inventoryStore.isSearching">
          ...
          <div v-if="inventoryStore.isSearching" class="text-center py-3">
            <div class="animate-spin rounded-full h-5 w-5 border-2 border-amber-500 border-t-transparent mx-auto"></div>
            <p class="text-xs font-semibold text-gray-500 mt-1">جاري البحث...</p>
          </div>
    ...
    <div v-if="inventoryStore.viewMode === 'paginated' && inventoryStore.summaryStats.totalItems > inventoryStore.pageSize" class="pagination-container">
...
function onFilterChange() {
  ...
  fetchPage(true, !warehouseChanged)
}
...
function debouncedSearch() {
  ...
  // Update store's isSearching state
  if (searchTerm.length >= 2) {
    inventoryStore.isSearching = true
  }

  searchDebounceTimer = setTimeout(async () => {
    if (searchTerm.length >= 2) {
      await inventoryStore.hybridSearch(searchTerm)
    } else {
      inventoryStore.clearSearchResults()
      fetchPage(false, true)
    }
    inventoryStore.isSearching = false
  }, 400)
}
