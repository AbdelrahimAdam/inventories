...
// ✅ Use summaryStats from store for stats (server-side calculated)
const displayStats = computed(() => {
  // If we have search results, use filtered stats from the store
...
// filteredItems uses server data (inventoryStore.items is already paginated or searched)
const filteredItems = computed(() => {
  let items = inventoryStore.items

  // Additional client-side filtering for color/size if needed
  // (These are already handled server-side, but keep for safety)
...
// Server-side load with pagination
async function loadItemsPage(page: number = 1) {
  ...
  try {
    // Clear any search results when loading a regular page
...
// ✅ FIXED: Server-side search - Properly handle the itemsMap
async function performSearch(searchTerm: string) {
  ...
  try {
    ...
    // ✅ FIX: Update the itemsMap with search results
    ...
    const itemsMap = inventoryStore.itemsMap as any
    itemsMap.value = newMap
    
    totalCount.value = results.length
    currentPage.value = 1
    
    // Update summary stats with search results
