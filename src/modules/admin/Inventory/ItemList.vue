...
async function loadAllItemsForAlerts() {
  if (isLoadingAllItems.value) return
  
  const warehouseId = selectedWarehouseId.value || 'all'
  const cacheKey = warehouseId
  
  if (isWarehouseLoaded.value.get(cacheKey)) {
    const cachedItems = warehouseDataCache.value.get(cacheKey)
  ...
  try {
    ...
    warehouseDataCache.value.set(cacheKey, items)
    isWarehouseLoaded.value.set(cacheKey, true)
    inventoryStore.allItemsForStats = items
    isAllItemsLoaded.value = true
    clientFilteredItems.value = filterItemsClientSide(items)
...
function resetFilters() {
  ...
  if (isWarehouseLoaded.value.get('all')) {
    ...
    inventoryStore.items = cachedItems
    inventoryStore.allItemsForStats = cachedItems
    isAllItemsLoaded.value = true
    clientFilteredItems.value = filterItemsClientSide(cachedItems)
    tableLoading.value = false
