...
// Use server-side stats for display
const displayStats = computed(() => {
  return inventoryStore.summaryStats
})

// filteredItems uses server data (inventoryStore.items is already paginated)
const filteredItems = computed(() => {
  // When in paginated mode, itemsMap already contains the current page
  // When in view-all mode, itemsMap contains all items
...
function debouncedSearch() {
  ...
  searchDebounceTimer = setTimeout(() => {
    ...
    // Use server-side search when search term exists
    if (searchTerm.length >= 2) {
...
async function performSearch(searchTerm: string) {
  ...
  try {
    ...
    // Replace itemsMap with search results
...
// Load inventory with pagination by default
async function loadInventory(force = false) {
  ...
  // If we already have items and not forcing refresh, just sync filters
  ...
  // Load first page only (not all items)
...
function goToPage(page: number) {
  if (page >= 1 && page <= totalPages.value) {
    if (viewMode.value === 'view-all') {
      // In view-all mode, just scroll to top
      scrollTop.value = 0
