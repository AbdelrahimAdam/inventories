<template>
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    <div v-if="authStore.isViewOnly" class="mb-3 bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 rounded-lg p-2.5 sm:p-3 shadow-sm hover:shadow-md transition-all duration-300">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
      ...
      <div class="flex gap-2 w-full sm:w-auto flex-wrap">
        <button @click="exportToExcel" class="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-2 shadow-md hover:shadow-lg text-xs sm:text-sm font-semibold min-h-[44px] border border-blue-400/20">
        ...
        <button @click="confirmExportAllCards" class="flex-1 sm:flex-none bg-amber-700 hover:bg-amber-800 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-2 shadow-md hover:shadow-lg text-xs sm:text-sm font-semibold min-h-[44px] border border-amber-500/20" :disabled="isExporting">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button v-if="authStore.canEdit" @click="openGlobalTransferModal" class="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-2 shadow-md hover:shadow-lg text-xs sm:text-sm font-semibold min-h-[44px] border border-blue-400/20">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button v-if="authStore.canEdit" @click="openGlobalDispatchModal" class="flex-1 sm:flex-none bg-red-600 hover:bg-red-700 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-2 shadow-md hover:shadow-lg text-xs sm:text-sm font-semibold min-h-[44px] border border-red-400/20">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <router-link v-if="authStore.canEdit" to="/inventory/items/new" class="flex-1 sm:flex-none bg-green-600 hover:bg-green-700 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-2 shadow-md hover:shadow-lg text-xs sm:text-sm font-semibold min-h-[44px] border border-green-400/20">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-1.5 sm:gap-2 mb-4">
      ...
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
      ...
      <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md p-3 sm:p-4 mb-4 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
      <div class="relative mb-3">
        <svg class="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
        </svg>
        <input type="text" v-model="localSearchInput" placeholder="بحث بالاسم أو الكود أو المورد أو الموقع..." class="w-full pl-9 pr-10 py-2 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors" :class="{ 'border-amber-500 ring-2 ring-amber-500': isSearching }" />
      ...
      <div v-if="localSearchInput.length > 0" class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
        <span>{{ totalCount }} نتيجة <span v-if="isSearching">(جارٍ البحث...)</span></span>
        <span v-if="totalCount === 0 && localSearchInput.length >= 2 && !isSearching" class="text-amber-600 dark:text-amber-400">لا توجد نتائج مطابقة</span>
      </div>

      <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-2">
        <select v-model="filters.warehouseId" @change="onFilterChange" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
        ...
        <select v-model="filters.status" @change="onFilterChange" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
        ...
        <select v-model="colorFilterToggle" @change="onColorFilterToggleChange" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
          <option value="">تصفية باللون</option>
          <option value="specific">لون محدد</option>
        </select>
        <select v-model="sizeFilterToggle" @change="onSizeFilterToggleChange" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
      ...
      <div class="flex justify-end">
        <button @click="resetFilters" class="px-4 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-sm font-semibold text-gray-700 dark:text-gray-300 min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700">إعادة تعيين</button>
      </div>

      <div v-if="colorFilterToggle === 'specific' || sizeFilterToggle === 'specific'" class="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3 pt-3 border-t border-gray-300 dark:border-gray-600">
        <div v-if="colorFilterToggle === 'specific'" class="relative">
          <input type="text" v-model="filters.color" @input="onFilterChange" placeholder="اكتب اللون..." class="w-full pl-10 pr-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors" />
        ...
        <div v-if="sizeFilterToggle === 'specific'" class="relative">
          <input type="text" v-model="filters.size" @input="onFilterChange" placeholder="اكتب المقاس..." class="w-full px-4 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-white font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors" />
        </div>
      </div>
    </div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-3">
      <div class="text-sm font-semibold text-gray-600 dark:text-gray-400">
        إجمالي الأصناف: {{ formatNumber(totalCount) }}
        <span v-if="inventoryStore.totalItems > 0 && totalCount < inventoryStore.totalItems" class="text-amber-600 dark:text-amber-400">
          (تم تصفية {{ formatNumber(inventoryStore.totalItems - totalCount) }} صنف)
        </span>
      </div>
      <div class="flex gap-2 w-full sm:w-auto">
        <button @click="setViewMode('paginated')" :class="['flex-1 sm:flex-none px-4 py-2 text-xs sm:text-sm font-bold rounded-xl border min-h-[44px] transition-all', viewMode === 'paginated' ? 'bg-amber-600 text-white border-amber-600 shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-amber-300 dark:hover:border-amber-600']">
          عرض بالصفحات
        </button>
        <button @click="setViewMode('view-all')" :class="['flex-1 sm:flex-none px-4 py-2 text-xs sm:text-sm font-bold rounded-xl border min-h-[44px] transition-all', viewMode === 'view-all' ? 'bg-amber-600 text-white border-amber-600 shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-amber-300 dark:hover:border-amber-600']">
          عرض الكل ({{ formatNumber(totalCount) }})
        </button>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md overflow-hidden hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
        ...
        <div ref="tableContainerRef" class="relative overflow-y-auto" :style="tableContainerStyle" @scroll="onTableScroll">
          <table class="w-full min-w-[1000px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <template v-if="inventoryStore.isLoading && visibleItems.length === 0">
    ...
    <div v-if="viewMode === 'paginated' && totalCount > pageSize" class="flex flex-col sm:flex-row justify-between items-center gap-3 mt-3 p-3 bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-sm">
      <div class="text-xs font-semibold text-gray-600 dark:text-gray-400 order-2 sm:order-1">
        عرض {{ ((currentPage - 1) * pageSize) + 1 }} إلى {{ Math.min(currentPage * pageSize, totalCount) }} من {{ formatNumber(totalCount) }} صنف
      </div>
      <div class="flex items-center gap-2 order-3 sm:order-2">
        <span class="text-xs font-semibold text-gray-600 dark:text-gray-400">عرض:</span>
        <select v-model="pageSize" @change="changePageSize" class="px-2 py-1 border border-gray-300 dark:border-gray-600 rounded-xl text-xs font-medium bg-white dark:bg-gray-700 text-gray-900 dark:text-white min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
      ...
      <div class="flex gap-1.5 order-1 sm:order-3">
        <button @click="goToPage(1)" :disabled="currentPage === 1" class="px-2.5 py-1 border border-gray-300 dark:border-gray-600 rounded-xl disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-xs font-semibold min-h-[44px]">««</button>
        <button @click="prevPage" :disabled="currentPage === 1" class="px-2.5 py-1 border border-gray-300 dark:border-gray-600 rounded-xl disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-xs font-semibold min-h-[44px]">السابق</button>
        <div class="hidden sm:flex gap-1">
          <template v-for="page in visiblePages" :key="page">
            <button v-if="page !== '...'" @click="goToPage(Number(page))" :class="['px-2.5 py-1 rounded-xl text-xs font-bold transition-colors min-h-[44px]', currentPage === page ? 'bg-gradient-to-r from-amber-600 to-green-600 text-white shadow-md' : 'border border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 hover:border-amber-300 dark:hover:border-amber-600']">{{ page }}</button>
            <span v-else class="px-2 py-1 text-gray-500 font-semibold text-xs">...</span>
          </template>
        </div>
        <button @click="nextPage" :disabled="currentPage === totalPages" class="px-2.5 py-1 border border-gray-300 dark:border-gray-600 rounded-xl disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-xs font-semibold min-h-[44px]">التالي</button>
        <button @click="goToPage(totalPages)" :disabled="currentPage === totalPages" class="px-2.5 py-1 border border-gray-300 dark:border-gray-600 rounded-xl disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-xs font-semibold min-h-[44px]">»»</button>
      </div>
    </div>

    <div class="mt-3 text-sm font-semibold text-gray-600 dark:text-gray-400 text-center">
      إجمالي الأصناف: {{ formatNumber(totalCount) }}
      <span v-if="inventoryStore.totalItems > 0 && totalCount < inventoryStore.totalItems" class="text-amber-600 dark:text-amber-400">
        (تم تصفية {{ formatNumber(inventoryStore.totalItems - totalCount) }} صنف)
      </span>
    ...
    <Teleport to="body">
      <div v-if="activeActionMenu" ref="dropdownRef" class="fixed z-[10000] bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-300 dark:border-gray-600 overflow-y-auto min-w-[200px] max-w-[280px]" :style="dropdownStyle" @click.stop role="menu" aria-label="قائمة الإجراءات">
        <div class="py-1">
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 max-w-md w-full border border-gray-300 dark:border-gray-600">
        ...
        <div class="flex justify-end gap-3">
          <button @click="showDeleteModal = false" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors font-semibold min-h-[44px]">إلغاء</button>
          <button @click="deleteItem" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl shadow-md font-bold min-h-[44px] border border-red-400/20">حذف</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showExportConfirmModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl p-6 max-w-md w-full border border-gray-300 dark:border-gray-600">
        ...
        <div class="flex justify-end gap-3">
          <button @click="showExportConfirmModal = false" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors font-semibold min-h-[44px]">إلغاء</button>
          <button @click="proceedExportAllCards" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-xl shadow-md font-bold min-h-[44px] border border-amber-500/20">تأكيد التصدير</button>
        </div>
      </div>
    </div>
...
const visibleItems = computed(() => {
  ...
  const end = Math.min(start + Math.ceil((tableContainerRef.value?.clientHeight || 600) / itemHeight) + overscan * 2, items.length)
  return items.slice(Math.max(0, start - overscan), end)
})

const virtualSpacerHeight = computed(() => {
  const items = inventoryStore.items
  const totalHeight = items.length * itemHeight
  const visibleCount = Math.min(visibleItems.value.length, items.length)
  const offset = Math.max(0, Math.floor(scrollTop.value / itemHeight) - overscan)
  return Math.max(0, totalHeight - visibleCount * itemHeight - offset * itemHeight)
})
...
async function loadItemsPage(page: number = 1) {
  ...
  try {
    await inventoryStore.fetchItemsPage({
      ...
      search: localSearchInput.value.length >= 2 ? localSearchInput.value : undefined,
    ...
    totalCount.value = inventoryStore.totalCount || 0
  ...
  try {
    await inventoryStore.fetchAllItemsForExport({
      ...
      onProgress: (loaded, total) => {
        loadingProgress.value = Math.min(100, Math.round((loaded / total) * 100))
      }
    })
    totalCount.value = inventoryStore.items.length
  } catch (error) {
...
function debouncedSearch() {
  ...
  searchDebounceTimer = setTimeout(() => {
    ...
    if (searchTerm.length >= 2) {
      performHybridSearch(searchTerm)
    } else {
      if (viewMode.value === 'view-all') {
        loadAllItems()
      } else {
...
async function performHybridSearch(searchTerm: string) {
  if (inventoryStore.isLoading) return
  
  isLoadingData.value = true
  try {
    // First, check if we have cached items that match
    const cachedMatches = inventoryStore.items.filter(item => {
      const searchable = [
        item.name, item.code, item.color,
        item.size, item.supplier, item.location, item.notes
      ].join(' ').toLowerCase()
      return searchable.includes(searchTerm.toLowerCase())
    })

    // If we have enough cached matches or we're in view-all mode with all items loaded
    if (viewMode.value === 'view-all' && inventoryStore.items.length >= inventoryStore.totalItems) {
      // We have all items, just use cached results
      // Use type assertion for itemsMap
      const newMap = new Map<string, InventoryItem>()
      for (const item of cachedMatches) {
        newMap.set(item.id, item)
      }
      ;(inventoryStore.itemsMap as any).value = newMap
      totalCount.value = cachedMatches.length
      currentPage.value = 1
      return
    }

    // If we have matches but not all items loaded, supplement with server search
    if (cachedMatches.length > 0) {
      // Get server results for the search term
      const serverResults = await inventoryStore.searchInventorySpark({
        searchQuery: searchTerm,
        warehouseId: filters.value.warehouseId || null,
        limit: viewMode.value === 'view-all' ? 10000 : pageSize.value
      })

      // Merge cached and server results (deduplicate by ID)
      ...
      // Add server results (will override cached if newer)
      for (const item of serverResults) {
        mergedMap.set(item.id, item)
      }

      ;(inventoryStore.itemsMap as any).value = mergedMap
      totalCount.value = mergedMap.size
      currentPage.value = 1
      return
    }

    // No cached matches, use server search
...
async function loadInventory(force = false) {
  ...
  if (inventoryStore.items.length > 0 && !force) {
    syncFiltersFromStore()
    totalCount.value = inventoryStore.totalCount || inventoryStore.items.length
    isLoadingData.value = false
    await nextTick()
    return
  }
  ...
  try {
    ...
    if (success) {
      ...
      if (viewMode.value === 'view-all') {
        ...
        await loadItemsPage(currentPage.value)
      }
    } else {
