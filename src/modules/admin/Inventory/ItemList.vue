...
const displayItems = computed(() => {
  return inventoryStore.displayItems
})
...
watch(localSearchInput, async (newVal) => {
  if (searchDebounceTimer) {
    clearTimeout(searchDebounceTimer)
  }

  inventoryStore.currentFilters.search = newVal || ''
  ...
  searchDebounceTimer = setTimeout(async () => {
    await inventoryStore.hybridSearch(newVal)
    tableLoading.value = false
  }, 400)
})
