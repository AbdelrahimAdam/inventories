  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-2 sm:gap-3 mb-4">
      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(summaryStats.totalItems)">{{ formatNumber(summaryStats.totalItems) }}</div>
      ...
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(summaryStats.totalQuantity)">{{ formatNumber(summaryStats.totalQuantity) }}</div>
      ...
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(summaryStats.lowStock)">{{ formatNumber(summaryStats.lowStock) }}</div>
      ...
      <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(summaryStats.criticalStock)">{{ formatNumber(summaryStats.criticalStock) }}</div>
      ...
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <div class="text-lg sm:text-xl lg:text-2xl font-bold break-words max-w-full truncate" :title="String(summaryStats.outOfStock)">{{ formatNumber(summaryStats.outOfStock) }}</div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-3">
      <div class="text-sm font-semibold text-gray-600 dark:text-gray-400">
        ...
        <span v-if="filteredItems.length < totalItemsCount" class="text-amber-600 dark:text-amber-400">
          (تم تصفية {{ formatNumber(totalItemsCount - filteredItems.length) }} صنف)
        </span>
      </div>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <template v-else>
                <template v-if="viewMode === 'view-all'">
                  ...
                  <tr v-if="visibleItems.length === 0 && !isLoading && !isSearching">
...
const totalItemsCount = computed(() => inventoryStore.totalItemsCount || inventoryStore.items.length)
...
async function loadInventory() {
  ...
  try {
    ...
    if (inventoryStore.items.length > 0 && inventoryStore.dataLoaded) {
