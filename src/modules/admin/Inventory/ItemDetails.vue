<template>
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'" class="w-full px-2 sm:px-4 py-3 sm:py-6 pb-24">
    <!-- Header -->
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
      ...
      <div class="flex gap-2 w-full sm:w-auto">
        <router-link to="/inventory/items" class="flex-1 sm:flex-none px-3 sm:px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 font-bold shadow-sm inline-flex items-center justify-center gap-1.5 text-xs sm:text-sm min-h-[44px]">
        ...
        <button
          ...
          @click="openEditModal"
          class="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white rounded-xl transition-all shadow-md font-bold inline-flex items-center justify-center gap-1.5 text-xs sm:text-sm min-h-[44px]"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div v-else-if="item" class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      <div class="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-gray-200 dark:divide-gray-700">
        ...
        <div class="p-3 sm:p-5 space-y-4">
          <!-- Basic Information - 3 columns on mobile -->
          <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
            ...
            <!-- 3 columns on mobile, 3 columns on desktop -->
            <div class="grid grid-cols-3 gap-1.5 sm:gap-2">
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">الاسم</span>
                <p class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white truncate" :title="item.name">{{ item.name }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">الكود</span>
                <p class="text-[10px] sm:text-sm font-mono font-black text-gray-900 dark:text-white">{{ item.code }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">اللون</span>
                <div class="flex items-center justify-center gap-1 mt-0.5">
                  <div class="w-3 h-3 sm:w-5 sm:h-5 rounded-full border-2 border-gray-300 shadow-sm flex-shrink-0" :style="{ backgroundColor: item.color }"></div>
                  <span class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white">{{ item.color || '—' }}</span>
                </div>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">المقاس</span>
                <span class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white inline-block mt-0.5 px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 rounded-lg text-[8px] sm:text-xs">{{ item.size || '—' }}</span>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">المخزن</span>
                <p class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white truncate" :title="getWarehouseName(item.warehouseId)">{{ getWarehouseName(item.warehouseId) }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">المورد</span>
                <p class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white truncate" :title="item.supplier || '—'">{{ item.supplier || '—' }}</p>
              </div>
            </div>
          </div>
          ...
          <div v-if="item.location || item.notes" class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
            ...
            <div class="space-y-2">
              <div v-if="item.location" class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">الموقع</span>
                <p class="text-sm font-black text-gray-900 dark:text-white break-words">{{ item.location }}</p>
              </div>
              <div v-if="item.notes" class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">ملاحظات</span>
                <p class="text-xs text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{{ item.notes }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="p-3 sm:p-5 space-y-4">
          ...
          <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
            ...
            <div v-if="item.perCartonCount === 1 && item.singleBottlesCount === 0" class="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-xl p-4 text-center">
              ...
              <p class="text-3xl sm:text-4xl font-black mt-1 text-center" :class="getStockTextClass(item.remainingQuantity)">{{ formatNumber(item.remainingQuantity) }}</p>
            ...
            <div v-else class="space-y-3">
              <div class="grid grid-cols-2 gap-3">
                <div class="bg-gray-100 dark:bg-gray-700/50 rounded-xl p-3 text-center">
                  ...
                  <p class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mt-0.5 text-center">{{ formatNumber(item.cartonsCount) }}</p>
                ...
                <div class="bg-gray-100 dark:bg-gray-700/50 rounded-xl p-3 text-center">
                  ...
                  <p class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mt-0.5 text-center">{{ formatNumber(item.singleBottlesCount) }}</p>
          ...
          <!-- System Information - 3 columns on mobile -->
          <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
            <h2 class="text-base sm:text-lg font-black text-amber-600 dark:text-amber-400 mb-3 pb-2 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
              معلومات النظام
            </h2>
            <!-- 3 columns on mobile, 2 columns on desktop -->
            <div class="grid grid-cols-3 gap-1.5 sm:gap-2">
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">تاريخ الإنشاء</span>
                <p class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white mt-0.5 text-center">{{ formatDate(item.createdAt) }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">آخر تحديث</span>
                <p class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white mt-0.5 text-center">{{ formatDate(item.updatedAt) }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">تم الإنشاء بواسطة</span>
                <p class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white mt-0.5 text-center">{{ item.created_by_name || '—' }}</p>
              </div>
              <div v-if="item.updated_by_name" class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-1.5 sm:p-2 text-center">
                <span class="text-[8px] sm:text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide block truncate">آخر تحديث بواسطة</span>
                <p class="text-[10px] sm:text-sm font-black text-gray-900 dark:text-white mt-0.5 text-center">{{ item.updated_by_name || '—' }}</p>
              </div>
            </div>
          </div>
        </div>
