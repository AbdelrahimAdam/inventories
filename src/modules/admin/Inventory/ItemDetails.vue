...
const item = ref<InventoryItem | null>(null)          // ✅ store returns null, not undefined
...
const handleUpdate = async () => {
  ...
  try {
    ...
    // ✅ after update, the store already updated its items array via updateLocalItem()
    // so we can simply reassign from the store
...
// ✅ Watch store.items for any real‑time updates (e.g., from other sessions)
watch(
...
onMounted(async () => {
  ...
  // ✅ Store returns InventoryItem | null – assign directly
