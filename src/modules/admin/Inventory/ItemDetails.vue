    ...
    <div v-else-if="item" class="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
      <div class="grid grid-cols-1 lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-gray-200 dark:divide-gray-700">
        ...
        <div class="p-3 sm:p-5 space-y-4">
          <!-- Basic Information - Redesigned with grid layout -->
          <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">الاسم</span>
                <p class="text-sm font-black text-gray-900 dark:text-white truncate" :title="item.name">{{ item.name }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">الكود</span>
                <p class="text-sm font-mono font-black text-gray-900 dark:text-white">{{ item.code }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">اللون</span>
                <div class="flex items-center gap-2 mt-0.5">
                  <div class="w-5 h-5 rounded-full border-2 border-gray-300 shadow-sm flex-shrink-0" :style="{ backgroundColor: item.color }"></div>
                  <span class="text-sm font-black text-gray-900 dark:text-white">{{ item.color || '—' }}</span>
                </div>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">المقاس</span>
                <span class="text-sm font-black text-gray-900 dark:text-white inline-block mt-0.5 px-2 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 rounded-lg text-xs">{{ item.size || '—' }}</span>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">المخزن</span>
                <p class="text-sm font-black text-gray-900 dark:text-white truncate" :title="getWarehouseName(item.warehouseId)">{{ getWarehouseName(item.warehouseId) }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">المورد</span>
                <p class="text-sm font-black text-gray-900 dark:text-white truncate" :title="item.supplier || '—'">{{ item.supplier || '—' }}</p>
              </div>
            </div>
          </div>
        ...
        <div class="p-3 sm:p-5 space-y-4">
            ...
            <div v-else class="space-y-3">
              <div class="grid grid-cols-2 gap-3">
                <div class="bg-gray-100 dark:bg-gray-700/50 rounded-xl p-3 text-center">
                  ...
                  <p class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mt-0.5" dir="ltr">{{ formatNumber(item.cartonsCount) }}</p>
                ...
                <div class="bg-gray-100 dark:bg-gray-700/50 rounded-xl p-3 text-center">
                  ...
                  <p class="text-2xl sm:text-3xl font-black text-gray-900 dark:text-white mt-0.5" dir="ltr">{{ formatNumber(item.singleBottlesCount) }}</p>
              ...
              <div class="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-xl p-4 text-center">
                ...
                <p class="text-3xl sm:text-4xl font-black mt-1" :class="getStockTextClass(item.remainingQuantity)" dir="ltr">{{ formatNumber(item.remainingQuantity) }}</p>
                <span :class="getStatusBadgeClass(item.remainingQuantity)" class="inline-block mt-2 px-3 py-1 text-xs font-black rounded-full shadow-sm">{{ getStatusText(item.remainingQuantity) }}</span>
              </div>
            </div>
          </div>
          ...
          <!-- System Information - Redesigned with grid layout -->
          <div class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-800 dark:to-gray-900 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4">
            <h2 class="text-base sm:text-lg font-black text-amber-600 dark:text-amber-400 mb-3 pb-2 border-b border-gray-200 dark:border-gray-700 flex items-center gap-2">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2 text-center">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">تاريخ الإنشاء</span>
                <p class="text-sm font-black text-gray-900 dark:text-white mt-0.5" dir="ltr">{{ formatDate(item.createdAt) }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2 text-center">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">آخر تحديث</span>
                <p class="text-sm font-black text-gray-900 dark:text-white mt-0.5" dir="ltr">{{ formatDate(item.updatedAt) }}</p>
              </div>
              <div class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2 text-center">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">تم الإنشاء بواسطة</span>
                <p class="text-sm font-black text-gray-900 dark:text-white mt-0.5">{{ item.created_by_name || '—' }}</p>
              </div>
              <div v-if="item.updated_by_name" class="bg-gray-50 dark:bg-gray-700/30 rounded-lg p-2 text-center">
                <span class="text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide">آخر تحديث بواسطة</span>
                <p class="text-sm font-black text-gray-900 dark:text-white mt-0.5">{{ item.updated_by_name || '—' }}</p>
              </div>
            </div>
          </div>
        </div>
              ...
              <form @submit.prevent="handleUpdate" class="space-y-4">
                ...
                <div class="bg-amber-50 dark:bg-amber-900/20 rounded-xl p-3">
                  <div class="flex justify-between items-center">
                    ...
                    <span class="text-xl font-black text-amber-700 dark:text-amber-400" dir="ltr">{{ editTotalQuantity.toLocaleString() }} وحدة</span>
                  </div>
                </div>
                <div>
