...
const showToast = (message: string, type: 'success' | 'error' | 'warning' | 'info' = 'success') => {
  if (window.toast) {
    window.toast.addToast(message, type, 5000)
  }
}
...
const validateForm = (): boolean => {
  ...
  if (itemType.value === 'unit' && (unitQuantity.value === undefined || unitQuantity.value <= 0)) {
    showToast('الكمية الإجمالية مطلوبة ويجب أن تكون أكبر من 0', 'error')
    isValid = false
...
const saveItem = async () => {
  ...
  try {
    if (isUploadingImage.value) {
      showToast('⏳ جاري رفع الصورة، يرجى الانتظار...', 'info')
    ...
    if (isEdit.value) {
      ...
      if (success) {
        showToast(`✅ تم تحديث الصنف "${form.name}" بنجاح`, 'success')
        setTimeout(() => {
          router.push('/inventory/items')
        }, 3000)
      } else {
        showToast(inventoryStore.error || 'حدث خطأ أثناء تحديث الصنف', 'error')
      }
    } else {
      ...
      if (result.success) {
        if (result.type === 'updated') {
          showToast(`📝 تم تحديث صنف موجود: "${form.name}" - تمت إضافة ${result.quantityAdded} وحدة (إجمالي الكمية الآن: ${totalQuantity.value.toLocaleString()})`, 'info')
          ...
          showToast(`✅ تم إضافة الصنف "${form.name}" بنجاح`, 'success')
          const currentWarehouse = form.warehouseId
        ...
        showToast(result.message || 'حدث خطأ أثناء إضافة الصنف', 'error')
      }
    }
  } catch (error: any) {
    ...
    if (!navigator.onLine) {
      ...
      showToast('⚠️ انقطع الاتصال بالإنترنت. سيتم حفظ البيانات عند استعادة الاتصال.', 'warning')
    } else {
      showToast(error.message || 'حدث خطأ أثناء حفظ الصنف', 'error')
    }
  } finally {
...
const handleSubmit = async () => {
  ...
  if (!navigator.onLine) {
    pendingSaveData = { form: { ...form }, isEdit: isEdit.value }
    showToast('⚠️ لا يوجد اتصال بالإنترنت. سيتم حفظ البيانات عند استعادة الاتصال.', 'warning')
    return
  }
...
const handleOnline = () => {
  isOnline.value = true
  if (pendingSaveData) {
    showToast('🔄 استعادة الاتصال. جاري حفظ البيانات...', 'info')
