<template>
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
      ...
      <form @submit.prevent="handleSubmit" class="p-3 sm:p-4 space-y-4">
            ...
            <div class="flex flex-wrap items-center gap-2">
              ...
              <div class="flex gap-1.5 flex-wrap">
                <button
                  ...
                  :class="[
                    'px-3 py-1.5 text-xs sm:text-sm rounded-lg transition-all duration-200 min-w-[44px] active:scale-95 whitespace-nowrap',
                    itemType === 'carton' 
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div class="touch-manipulation">
            ...
            <input
              ...
              type="text"
              v-model="form.name"
              class="w-full px-3 py-2 text-sm border-2 rounded-lg focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 dark:focus:ring-green-800 transition-all bg-white dark:bg-gray-700 text-gray-900 dark:text-white min-h-[44px]"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div class="touch-manipulation">
            ...
            <div class="flex gap-2">
              <input
                type="text"
                v-model="form.color"
                class="flex-1 px-3 py-2 text-sm border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 dark:focus:ring-green-800 transition-all bg-white dark:bg-gray-700 text-gray-900 dark:text-white min-h-[44px]"
          ...
          <div class="touch-manipulation">
            ...
            <input
              ...
              v-model="form.size"
              list="size-options"
              class="w-full px-3 py-2 text-sm border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 dark:focus:ring-green-800 transition-all bg-white dark:bg-gray-700 text-gray-900 dark:text-white min-h-[44px]"
            ...
            <div class="flex justify-end mb-2">
              <div class="flex gap-1 bg-white dark:bg-gray-800 rounded-lg p-0.5 shadow-sm border border-gray-200 dark:border-gray-600">
                <button
                  ...
                  :class="['px-3 py-1.5 rounded min-w-[44px] text-xs transition-all active:scale-95', inputMode === 'detailed' ? 'bg-green-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700']"
                >
                  تفصيلي
                </button>
                <button
                  ...
                  :class="['px-3 py-1.5 rounded min-w-[44px] text-xs transition-all active:scale-95', inputMode === 'simple' ? 'bg-green-500 text-white shadow-md' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700']"
                >
                  بسيط
                </button>
              </div>
        ...
        <div class="touch-manipulation">
          ...
          <textarea
            v-model="form.notes"
            rows="2"
            class="w-full px-3 py-2 text-sm border-2 border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-200 dark:focus:ring-green-800 transition-all bg-white dark:bg-gray-700 text-gray-900 dark:text-white resize-none min-h-[80px]"
        ...
        <div class="flex flex-col sm:flex-row gap-2 justify-end pt-3 border-t-2 border-gray-200 dark:border-gray-700">
          <button
            type="button"
            @click="goBack"
            class="order-2 sm:order-1 text-center px-5 py-2 border-2 border-gray-300 dark:border-gray-600 rounded-lg text-gray-700 dark:text-gray-300 font-semibold hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-400 dark:hover:border-gray-500 transition-all text-sm min-w-[80px] active:scale-95 min-h-[44px]"
          >
            إلغاء
          </button>
          <button
    ...
    <!-- Success Modal -->
    <Teleport to="body">
      <div v-if="showSuccessModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4" @click.self="closeSuccessModal">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
          ...
          <div class="bg-gradient-to-r from-green-600 to-green-700 dark:from-green-700 dark:to-green-800 px-6 py-4">
            <div class="flex justify-between items-center">
              <div class="flex items-center gap-3">
                ...
                <h2 class="text-xl font-bold text-white">تم بنجاح!</h2>
              </div>
              <button @click="closeSuccessModal" class="text-white hover:text-gray-200 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
          ...
          <div class="p-6 space-y-4">
            <p class="text-gray-700 dark:text-gray-300 text-center text-base font-medium">
              {{ successModalMessage }}
            </p>
            ...
            <div class="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg p-4">
              <div class="flex flex-col gap-2">
                <div class="flex justify-between">
                  ...
                  <span class="text-sm font-semibold text-gray-800 dark:text-white">{{ successModalData?.name || '' }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-sm text-gray-600 dark:text-gray-400">الكود:</span>
                  <span class="text-sm font-semibold text-gray-800 dark:text-white">{{ successModalData?.code || '' }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-sm text-gray-600 dark:text-gray-400">الكمية:</span>
                  <span class="text-sm font-semibold text-gray-800 dark:text-white">{{ successModalData?.quantity || 0 }}</span>
                </div>
                <div class="flex justify-between">
                  <span class="text-sm text-gray-600 dark:text-gray-400">المخزن:</span>
                  <span class="text-sm font-semibold text-gray-800 dark:text-white">{{ successModalData?.warehouse || '' }}</span>
                </div>
              </div>
            </div>
            ...
            <div class="flex gap-3 mt-4">
              <button
                v-if="!isEdit"
                @click="addAnother"
                class="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800 text-white rounded-xl transition-colors font-medium shadow-md min-h-[48px]"
              >
                ➕ إضافة صنف آخر
              </button>
              <button
                @click="goBack"
                class="flex-1 px-4 py-3 bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 text-white rounded-xl transition-colors font-medium shadow-md min-h-[48px]"
              >
                {{ isEdit ? '📋 العودة للقائمة' : '📋 العودة للقائمة' }}
              </button>
            </div>
          </div>
        </div>
    ...
    <!-- Error Modal -->
    <Teleport to="body">
      <div v-if="showErrorModal" class="fixed inset-0 bg-black/50 flex items-center justify-center z-[9999] p-4" @click.self="closeErrorModal">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
          <!-- Header -->
          <div class="bg-gradient-to-r from-red-600 to-red-700 dark:from-red-700 dark:to-red-800 px-6 py-4">
            <div class="flex justify-between items-center">
              ...
              <button @click="closeErrorModal" class="text-white hover:text-gray-200 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                </svg>
              </button>
          ...
          <div class="p-6">
            <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
              <p class="text-red-700 dark:text-red-400 text-center text-base font-medium">
                {{ errorModalMessage }}
              </p>
            </div>
            ...
            <button
              ...
              class="w-full mt-4 px-4 py-3 bg-red-600 hover:bg-red-700 dark:bg-red-700 dark:hover:bg-red-800 text-white rounded-xl transition-colors font-medium shadow-md min-h-[48px]"
            >
              حسناً
            </button>
          </div>
    ...
    <!-- Upload Success Toast (silent, non-modal) -->
    <Teleport to="body">
      <div v-if="showUploadToast" class="fixed bottom-24 left-1/2 transform -translate-x-1/2 z-[9998] animate-slide-up">
        <div class="bg-green-50 dark:bg-green-900/95 border-2 border-green-400 dark:border-green-600 rounded-xl px-6 py-3 shadow-2xl">
          <div class="flex items-center gap-3">
            <svg class="w-5 h-5 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <span class="text-sm font-medium text-green-800 dark:text-green-300">✅ تم رفع الصورة بنجاح</span>
          </div>
        </div>
      </div>
    </Teleport>
...
const successModalData = ref<{ name: string; code: string; quantity: number; warehouse: string } | null>(null)

// Upload toast timeout
let uploadToastTimeout: ReturnType<typeof setTimeout> | null = null
let isUploadToastShown = ref(false)
...
const colorNameToHex: Record<string, string> = {
  ...
  'بيج': '#F5F5DC', 'نحاسي': '#B87333', 'كحلي': '#2C3E50',
  'فيروزي': '#40E0D0', 'مرجاني': '#FF7F50', 'كريمي': '#FFFDD0',
  'عنابي': '#800020', 'زيتوني': '#556B2F', 'خوخي': '#FFDAB9',
  'نبيتي': '#722F37', 'تركواز': '#40E0D0', 'لافندر': '#E6E6FA',
  'red': '#FF0000', 'green': '#00FF00', 'blue': '#0000FF',
  'black': '#000000', 'white': '#FFFFFF', 'yellow': '#FFFF00',
  'purple': '#800080', 'orange': '#FFA500', 'pink': '#FFC0CB',
  'brown': '#A52A2A', 'gray': '#808080', 'gold': '#FFD700',
  'silver': '#C0C0C0', 'bronze': '#CD7F32',
  'beige': '#F5F5DC', 'copper': '#B87333', 'navy': '#2C3E50',
  'turquoise': '#40E0D0', 'coral': '#FF7F50', 'cream': '#FFFDD0',
  'maroon': '#800020', 'olive': '#556B2F', 'peach': '#FFDAB9',
  'wine': '#722F37', 'teal': '#008080', 'lavender': '#E6E6FA',
}
...
const colorPickerValue = computed(() => {
  const color = form.color.toLowerCase().trim()
...
const showSuccessToast = (message: string, data?: { name: string; code: string; quantity: number; warehouse: string }) => {
  successModalMessage.value = message
  successModalData.value = data || null
...
const showUploadSuccessToast = () => {
  ...
  if (isUploadToastShown.value) return
  isUploadToastShown.value = true
  ...
  uploadToastTimeout = setTimeout(() => {
    ...
    isUploadToastShown.value = false
    uploadToastTimeout = null
  }, 3000)
}

const selectPerCartonText = () => {
  // Select the text in the input so the user can easily type over it
  nextTick(() => {
...
const onColorInput = () => {
  // The color picker value will auto-update via computed
}
  ...
  if (input.files && input.files[0]) {
    ...
    try {
      ...
      if (uploadedUrl) {
        ...
        // Show a non-modal toast for upload success
        showUploadSuccessToast()
      }
    } catch (err) {
      console.error('Error handling image:', err)
      showErrorToast('حدث خطأ أثناء معالجة الصورة')
    }
  }
}
...
const validateForm = (): boolean => {
  ...
  if (itemType.value === 'unit' && (unitQuantity.value === undefined || unitQuantity.value <= 0)) {
    showErrorToast('الكمية الإجمالية مطلوبة ويجب أن تكون أكبر من 0')
    isValid = false
    ...
    if (inputMode.value === 'detailed' && (form.perCartonCount === undefined || form.perCartonCount <= 0)) {
      showErrorToast('يجب تحديد عدد القطع لكل صندوق')
      isValid = false
    }
  }
  return isValid
...
const saveItem = async () => {
  ...
  try {
    if (isUploadingImage.value) {
      // Wait for image upload to complete if in progress
      await new Promise(resolve => setTimeout(resolve, 500))
    }
    
    // Ensure perCartonCount has a valid value
    if (form.perCartonCount === undefined || form.perCartonCount <= 0) {
      form.perCartonCount = 12
    }
    ...
    if (isEdit.value) {
      ...
      if (success) {
        const warehouse = warehouseStore.warehouses.find(w => w.id === form.warehouseId)
        const warehouseName = warehouse?.name_ar || warehouse?.name || form.warehouseId
        ...
        showErrorToast(inventoryStore.error || 'حدث خطأ أثناء تحديث الصنف')
      }
    } else {
      ...
      if (result.success) {
        ...
        if (result.type === 'updated') {
          showSuccessToast(`📝 تم تحديث صنف موجود: "${form.name}" - تمت إضافة ${result.quantityAdded} وحدة`, {
            name: form.name,
            code: form.code,
            quantity: totalQuantity.value,
            warehouse: warehouseName
          ...
          showSuccessToast(`✅ تم إضافة الصنف "${form.name}" بنجاح`, {
            name: form.name,
            code: form.code,
            quantity: totalQuantity.value,
            warehouse: warehouseName
        ...
        showErrorToast(result.message || 'حدث خطأ أثناء إضافة الصنف')
      }
    }
  } catch (error: any) {
    ...
    if (!navigator.onLine) {
      ...
      showErrorToast('⚠️ انقطع الاتصال بالإنترنت. سيتم حفظ البيانات عند استعادة الاتصال.')
    } else {
      showErrorToast(error.message || 'حدث خطأ أثناء حفظ الصنف')
    }
  } finally {
...
const handleSubmit = async () => {
  ...
  if (!navigator.onLine) {
    pendingSaveData = { form: { ...form }, isEdit: isEdit.value }
    showErrorToast('⚠️ لا يوجد اتصال بالإنترنت. سيتم حفظ البيانات عند استعادة الاتصال.')
    return
  }
...
const handleOnline = () => {
  isOnline.value = true
  if (pendingSaveData) {
    showSuccessToast('🔄 استعادة الاتصال. جاري حفظ البيانات...')
