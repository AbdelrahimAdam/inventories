...
let isUploadingImage = ref(false)
...
const saveItem = async () => {
  ...
  try {
    // If there's a pending image upload, wait for it
    ...
    if (!navigator.onLine) {
      ...
      showToast('⚠️ انقطع الاتصال بالإنترنت. سيتم حفظ البيانات عند استعادة الاتصال.', 'error')
    } else {
...
const handleSubmit = async () => {
  ...
  if (!navigator.onLine) {
    pendingSaveData = { form: { ...form }, isEdit: isEdit.value }
    showToast('⚠️ لا يوجد اتصال بالإنترنت. سيتم حفظ البيانات عند استعادة الاتصال.', 'error')
    return
  }
...
const handleOnline = () => {
  isOnline.value = true
  if (pendingSaveData) {
    showToast('🔄 استعادة الاتصال. جاري حفظ البيانات...', 'success')
