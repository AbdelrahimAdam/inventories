<template>
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'" class="pb-32 sm:pb-20">
    <div class="max-w-7xl mx-auto">
      ...
      <!-- Toast Notifications -->
      <div v-if="toastMessage" class="fixed top-4 left-1/2 transform -translate-x-1/2 z-[9999] w-full max-w-md px-4">
        <div :class="[
          'rounded-xl shadow-lg p-4 flex items-center gap-3 animate-slide-down',
          toastType === 'success' ? 'bg-green-50 dark:bg-green-900/30 border border-green-500 dark:border-green-600' : 'bg-red-50 dark:bg-red-900/30 border border-red-500 dark:border-red-600'
        ]">
          <div :class="[
          ...
          <button @click="clearToast" class="flex-shrink-0 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300">
...
// Load warehouse items using the inventory store - with cache
const loadWarehouseItems = async () => {
  ...
  try {
    // First try to get from inventory store's cached items
...
// Perform search using the inventory store
...
const saveInvoice = async () => {
  ...
  try {
    ...
    if (result.success) {
      localStorage.removeItem(DRAFT_KEY)
      hasDraft.value = false
      showToast(`تم إنشاء الفاتورة #${result.data?.invoice_number} بنجاح`, 'success')
      setTimeout(() => {
