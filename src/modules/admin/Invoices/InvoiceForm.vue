...
const getRealTimeStock = async (itemId: string): Promise<number> => {
  try {
    // Use the same method as dispatch modal - get from itemsMap
...
const updateItemTotal = async (index: number) => {
  ...
  // Get real-time stock for this specific item
...
const getCurrentStock = async (itemId: string): Promise<number> => {
  return await getRealTimeStock(itemId)
}
...
const validateStockLevels = async (): Promise<boolean> => {
  ...
  for (const item of form.items) {
    try {
      // Get real-time stock using the same method as dispatch modal
      const currentStock = await getRealTimeStock(item.item_id)
...
const saveInvoice = async () => {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Refresh warehouse items to show updated stock
      await loadWarehouseItems()
