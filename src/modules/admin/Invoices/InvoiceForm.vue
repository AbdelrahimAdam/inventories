...
// ============================================================
// CACHE PERSISTENCE - Keep form state when navigating away
// ============================================================
...
// Save form state to cache when navigating away
...
// ============================================================
// DEBOUNCED SAVE - Save form state on changes
// ============================================================
...
onActivated(() => {
  ...
  if (!isEdit.value && form.items.length === 0 && !isCacheRestored) {
...
const getStockForItem = async (itemId: string, warehouseId: string): Promise<number> => {
  try {
    // First check the itemsMap cache for the specific warehouse
...
const validateStockLevels = async (): Promise<boolean> => {
  ...
  // Group items by warehouse to check stock per warehouse
  ...
  // Check stock for each item in its respective warehouse
...
onMounted(async () => {
  ...
  if (id) {
    ...
    // Try to restore from cache first, then draft
