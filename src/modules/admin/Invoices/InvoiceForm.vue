  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'" class="pb-32 sm:pb-20">
      ...
      <div class="mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        ...
        <div class="flex gap-2 w-full sm:w-auto">
          <button 
            ...
            class="flex-1 sm:flex-none px-3 sm:px-4 py-1.5 sm:py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors text-center shadow-md text-sm min-h-[44px]"
          >
              ...
              <div class="border border-gray-200 dark:border-gray-700 rounded-lg max-h-48 overflow-y-auto">
                <div v-for="item in displayItems" :key="item.id" @click="addItemToInvoice(item)" class="p-2.5 border-b border-gray-100 dark:border-gray-700 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
                  <div class="flex justify-between items-start">
                    ...
                    <button class="px-2.5 py-1 text-xs bg-gradient-to-r from-amber-600 to-green-600 hover:from-amber-700 hover:to-green-700 text-white rounded ml-1 transition-colors shadow-sm min-h-[32px]">إضافة</button>
                  </div>
                </div>
              </div>
            </div>
                ...
                <table class="w-full text-xs sm:text-sm min-w-[500px]">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in form.items" :key="index" class="border-b border-gray-200 dark:border-gray-700">
                      ...
                      <td class="px-2 py-1.5">
                        <input type="number" v-model.number="item.unit_price" @change="updateItemTotal(index)" min="0" step="0.01" class="w-20 sm:w-24 px-1 py-1 text-center border border-gray-300 dark:border-gray-600 rounded text-xs bg-white dark:bg-gray-700 dark:text-white min-h-[36px]" />
                       </td>
                      ...
                      <td class="px-2 py-1.5 text-center">
                        <button @click="removeItem(index)" class="text-red-500 hover:text-red-700 transition-colors p-1 min-h-[36px] min-w-[36px]">
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-lg z-50 px-3 sm:px-4 py-3 sm:py-4" style="padding-bottom: env(safe-area-inset-bottom, 0.5rem);">
      <div class="max-w-7xl mx-auto flex flex-wrap gap-2">
        <button @click="saveInvoice" :disabled="isSaving" class="flex-1 min-w-[80px] px-3 py-2 sm:py-2.5 bg-gradient-to-r from-amber-600 to-green-600 hover:from-amber-700 hover:to-green-700 text-white rounded-lg transition-colors disabled:opacity-50 shadow-md text-sm font-semibold min-h-[48px] touch-manipulation">
          {{ isSaving ? 'جاري الحفظ...' : (isEdit ? 'تحديث' : 'حفظ') }}
        </button>
        <button v-if="form.items.length > 0 && !isEdit" @click="saveAsDraft" :disabled="isSaving" class="flex-1 min-w-[80px] px-3 py-2 sm:py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 text-sm font-medium min-h-[48px] touch-manipulation">مسودة</button>
        <button v-if="hasDraft && !isEdit" @click="clearDraft" class="flex-1 min-w-[80px] px-3 py-2 sm:py-2.5 border border-red-300 dark:border-red-600 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-sm font-medium min-h-[48px] touch-manipulation">مسح</button>
...
// Load warehouse items using the inventory store - same pattern as transfer modal
const loadWarehouseItems = async () => {
  ...
  try {
    // Use the inventory store's getItemsByWarehouse method (cached)
...
// Perform search using the inventory store - same pattern as transfer modal
const performSearch = async () => {
  ...
  try {
    // Use the inventory store's searchInventorySpark method (same as transfer modal)
...
const getCurrentStock = (_itemId: string): number => {
  const warehouseItem = displayItems.value.find(item => item.id === _itemId)
  if (warehouseItem) {
...
const validateStockLevels = async (): Promise<boolean> => {
  // Refresh warehouse items to get latest stock
  await loadWarehouseItems()
  
  for (const item of form.items) {
    const currentStock = getCurrentStock(item.item_id)
...
onMounted(async () => {
  ...
  // Load inventory items into the store
...
/* Ensure bottom bar stays above everything */
.fixed.bottom-0 {
  z-index: 999;
}
</style>