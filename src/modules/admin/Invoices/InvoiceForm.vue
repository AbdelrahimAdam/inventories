      ...
      <div v-if="toastMessage" class="fixed bottom-20 left-0 right-0 z-[9999] px-4 pointer-events-none">
        <div class="max-w-md mx-auto pointer-events-auto">
          <div :class="[
            ...
            toastType === 'success' ? 'bg-green-50 dark:bg-green-900/95 border border-green-500 dark:border-green-600' : 'bg-red-50 dark:bg-red-900/95 border border-red-500 dark:border-red-600'
          ]">
            <div :class="[
            ...
            <div class="flex-1 min-w-0">
              <p :class="[
                ...
                toastType === 'success' ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300'
              ]">{{ toastMessage }}</p>
            </div>
      ...
      <div v-else class="flex flex-col lg:flex-row gap-4">
        ...
        <div class="flex-1 space-y-4">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-3 sm:p-4 lg:p-5 border border-gray-200 dark:border-gray-700">
            ...
            <div v-if="selectedWarehouseId && displayItems.length === 0 && !searchQuery && !isSearching" class="text-center py-6 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 rounded-lg mb-3 text-sm">لا توجد أصناف في هذا المخزن</div>
            <div v-if="selectedWarehouseId && displayItems.length === 0 && searchQuery && !isSearching" class="text-center py-6 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 rounded-lg mb-3 text-sm">لا توجد أصناف مطابقة للبحث "{{ searchQuery }}"</div>
            <div v-if="selectedWarehouseId && isSearching" class="text-center py-6 text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900/50 rounded-lg mb-3 text-sm">
...
// Load warehouse items from cache - identical to dispatch modal
const loadWarehouseItems = async () => {
  ...
  try {
    // Use the same method as dispatch modal - searchInventorySpark with empty query
...
const performSearch = async () => {
  ...
  try {
    // Use the same method as dispatch modal - searchInventorySpark
    const results = await inventoryStore.searchInventorySpark({
      searchQuery: query,
      warehouseId: selectedWarehouseId.value,
      limit: 50
...
// Get real-time stock from cache - identical to dispatch modal
const getRealTimeStock = async (itemId: string): Promise<number> => {
  try {
    // First check the itemsMap cache (like dispatch modal)
    const key = `${selectedWarehouseId.value}-${itemId}`
    ...
    // If not in cache, search for it (will populate cache)
    await performSearch()
    const foundItem = displayItems.value.find(i => i.id === itemId)
    return foundItem?.remainingQuantity || 0
  } catch (error) {
    console.error('Failed to get real-time stock:', error)
    return 0
  }
}

const addItemToInvoice = async (item: any) => {
  const currentStock = await getRealTimeStock(item.id)
  ...
  if (existingIndex !== -1) {
    ...
    if (form.items[existingIndex].quantity < maxQty) {
      ...
      await updateItemTotal(existingIndex)
    } else {
...
const updateItemTotal = async (index: number) => {
  const item = form.items[index]
  
  const currentStock = await getRealTimeStock(item.item_id)
  
  item.maxQuantity = currentStock
  
  if (item.quantity > currentStock) {
    showToast(`الكمية المطلوبة (${item.quantity}) أكبر من المخزون المتاح (${currentStock})`, 'error')
...
// Validate stock levels using cached data - identical to dispatch modal
const validateStockLevels = async (): Promise<boolean> => {
  ...
  for (const item of form.items) {
    try {
      ...
      if (item.quantity > currentStock) {
        showToast(
          `الكمية المطلوبة للصنف ${item.name} (${item.quantity}) أكبر من المخزون المتاح (${currentStock})`,
          'error'
        )
        return false
      }
    } catch (error) {
      console.error('Failed to check stock for item:', item.name, error)
      showToast(`فشل في التحقق من المخزون للصنف ${item.name}`, 'error')
      return false
    }
  }
...
// Perform dispatch deduction - IDENTICAL to dispatch modal
const performDispatchDeduction = async (): Promise<boolean> => {
  try {
    // Process each item in the invoice
    for (const item of form.items) {
      // Find the item in displayItems to get its details
      const inventoryItem = displayItems.value.find(i => i.id === item.item_id)
      if (!inventoryItem) {
        showToast(`الصنف ${item.name} غير موجود في المخزن`, 'error')
        return false
      }

      // Get per carton count or default to 12 (same as dispatch modal)
      const perCarton = inventoryItem.perCartonCount || 12
      const cartonsToDispatch = Math.floor(item.quantity / perCarton)
      const singlesToDispatch = item.quantity % perCarton
      
      // Generate unique voucher number (same format as dispatch modal)
      const now = new Date()
      ...
      // Use the EXACT same dispatchItem method as the dispatch modal
      const result = await inventoryStore.dispatchItem({
        item_id: item.item_id,
        from_warehouse_id: selectedWarehouseId.value,
        destination: `فاتورة #${form.invoice_number || 'INV'}`,
        destination_id: voucherNumber,
        quantity: item.quantity,
        cartons_count: cartonsToDispatch,
        single_bottles_count: singlesToDispatch,
        notes: `صرف للفاتورة ${form.invoice_number || 'INV'} - العميل: ${form.customer.name}`
      })

      if (!result.success) {
        showToast(result.message || `فشل في صرف الصنف ${item.name}`, 'error')
        return false
      }
    }
    
    // Refresh the warehouse items to show updated stock (cache is updated by the store)
    await loadWarehouseItems()
    
    return true
  } catch (error: any) {
    console.error('Dispatch deduction error:', error)
    showToast(error.message || 'فشل في عملية صرف المخزون', 'error')
    return false
  }
}
...
const saveInvoice = async () => {
  ...
  // Check stock with real-time data
  ...
  try {
    ...
    if (result.success) {
      // Only perform dispatch deduction if status is not 'draft'
      // This matches the dispatch modal behavior where only confirmed invoices deduct stock
      if (form.status !== 'draft' && !isEdit.value) {
        const dispatchSuccess = await performDispatchDeduction()
        if (!dispatchSuccess) {
          showToast('تم إنشاء الفاتورة ولكن فشل في صرف المخزون', 'warning')
          // Proceed with invoice creation but warn user
        }
      }
      ...
      // Refresh warehouse items to show updated stock (cache is automatically updated by the store)
      await loadWarehouseItems()
