...
async function loadWarehouseItems() {
  ...
  try {
    const cachedItems = inventoryStore.itemsList.filter(
      item => item.warehouseId === selectedWarehouseId.value && item.remainingQuantity > 0
    )
...
onMounted(async () => {
  ...
  if (id) {
    ...
    if (invoice) {
      ...
      const cachedItems = inventoryStore.itemsList.filter(
        item => item.warehouseId === selectedWarehouseId.value
      )
      if (cachedItems.length > 0) {
        displayItems.value = cachedItems
      } else {
