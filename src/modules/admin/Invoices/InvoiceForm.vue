<template>
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'" class="pb-32 sm:pb-20">
    <div class="max-w-7xl mx-auto">
      ...
      <div v-if="!authStore.isFullyReady" class="space-y-4">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 animate-pulse">
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4 border border-gray-200 dark:border-gray-700 animate-pulse">
          <div class="h-8 bg-gray-200 dark:bg-gray-700 rounded w-24 mb-4"></div>
      ...
      <div v-if="hasDraft && !isEdit" class="mb-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-200 dark:border-amber-700 rounded-lg p-2.5 flex flex-col sm:flex-row justify-between items-center gap-2">
        ...
        <div class="flex gap-2">
          <button @click="restoreDraftFromCache" class="px-3 py-1 bg-amber-600 hover:bg-amber-700 text-white text-xs sm:text-sm rounded transition-colors min-h-[36px]">استعادة</button>
      ...
      <!-- Main Form -->
      <div v-if="authStore.isFullyReady" class="flex flex-col lg:flex-row gap-4">
        <!-- Main form content -->
        ...
        <div class="w-full lg:w-80 space-y-4">
          ...
          <!-- Status Selection - Only draft option available for creation -->
          <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-3 sm:p-4 border border-gray-200 dark:border-gray-700">
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">سيتم إنشاء الفاتورة كمسودة. يمكنك تغيير الحالة لاحقاً من قائمة الفواتير.</p>
            <div class="flex flex-wrap gap-3">
              <label class="flex items-center gap-1.5 cursor-pointer text-sm min-h-[40px]">
                <input type="radio" v-model="form.status" value="draft" class="text-amber-600" checked />
                <span class="dark:text-gray-300">مسودة</span>
              </label>
              <label class="flex items-center gap-1.5 cursor-not-allowed text-sm min-h-[40px] opacity-60">
                <input type="radio" value="issued" disabled class="text-amber-600" />
                <span class="dark:text-gray-400">صادرة</span>
                <span class="text-[10px] text-gray-400 dark:text-gray-500">(تغيير من القائمة)</span>
              </label>
              <label class="flex items-center gap-1.5 cursor-not-allowed text-sm min-h-[40px] opacity-60">
                <input type="radio" value="paid" disabled class="text-amber-600" />
                <span class="dark:text-gray-400">مدفوعة</span>
                <span class="text-[10px] text-gray-400 dark:text-gray-500">(تغيير من القائمة)</span>
              </label>
            </div>
          </div>
    ...
    <!-- Sticky Bottom Bar - CONTAINER HAS NO v-if, ALWAYS VISIBLE -->
    <div class="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 shadow-lg z-50 px-3 sm:px-4 py-3 sm:py-4" style="padding-bottom: env(safe-area-inset-bottom, 0.5rem);">
      <div class="max-w-7xl mx-auto flex flex-wrap gap-2">
        <button 
          v-if="authStore.isFullyReady && (canCreateInvoice || canEditInvoice)"
          @click="saveInvoice" 
          :disabled="isSaving" 
          class="flex-1 min-w-[80px] px-3 py-2 sm:py-2.5 bg-gradient-to-r from-amber-600 to-green-600 hover:from-amber-700 hover:to-green-700 text-white rounded-lg transition-colors disabled:opacity-50 shadow-md text-sm font-semibold min-h-[48px] flex items-center justify-center touch-manipulation"
        >
          {{ isSaving ? 'جاري الحفظ...' : (isEdit ? 'تحديث' : 'حفظ') }}
        </button>

        <button 
          v-if="form.items.length > 0 && !isEdit && authStore.isFullyReady && (canCreateInvoice || canEditInvoice)"
          @click="saveAsDraft" 
          :disabled="isSaving" 
          class="flex-1 min-w-[80px] px-3 py-2 sm:py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 text-sm font-medium min-h-[48px] flex items-center justify-center touch-manipulation"
        >
          مسودة
        </button>
        ...
        <button 
          ...
          @click="clearDraft" 
          class="flex-1 min-w-[80px] px-3 py-2 sm:py-2.5 border border-red-300 dark:border-red-600 text-red-600 dark:text-red-400 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors text-sm font-medium min-h-[48px] flex items-center justify-center touch-manipulation"
        >
          مسح
        </button>
        ...
        <router-link 
          to="/invoices" 
          class="flex-1 min-w-[80px] px-3 py-2 sm:py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-center text-gray-700 dark:text-gray-300 text-sm font-medium min-h-[48px] flex items-center justify-center touch-manipulation"
        >
          إلغاء
        </router-link>
      </div>
...
import { ref, reactive, computed, onMounted, watch, onBeforeUnmount, onActivated, onDeactivated } from 'vue'
...
const FORM_CACHE_KEY = 'invoice_form_cache'
...
function saveFormToCache() {
  ...
  const cacheData = {
    form: {
      ...
      items: form.items.map((item: any) => ({ ...item })),
...
function restoreFormFromCache(): boolean {
  if (isEdit.value) return false
  if (isCacheRestored) return false
  ...
  try {
    ...
    if (!cache.form) return false
    
    isRestoringDraft = true
    Object.assign(form, cache.form)
    form.items = cache.form.items.map((item: any) => ({ 
      ...item, 
      maxQuantity: item.maxQuantity || 0 
    }))
    selectedWarehouseId.value = cache.selectedWarehouseId || ''
    selectedCurrency.value = cache.selectedCurrency || 'EGP'
    ...
    console.error('Failed to restore form cache:', e)
    return false
  }
}

function restoreDraftFromCache() {
  if (restoreFormFromCache()) {
    calculateTotals()
...
onActivated(() => {
  if (!isEdit.value && form.items.length === 0 && !isCacheRestored && !isInitialLoad) {
    restoreFormFromCache()
  }
  
  if (selectedWarehouseId.value && displayItems.value.length === 0) {
    loadWarehouseItems()
  }
...
async function loadWarehouseItems() {
  ...
  try {
    const cachedItems = inventoryStore.items.filter(
      (item: any) => item.warehouseId === selectedWarehouseId.value && item.remainingQuantity > 0
    )
    
    if (cachedItems.length > 0) {
      displayItems.value = cachedItems
...
const getWarehouseName = (warehouseId: string): string => {
  ...
  return warehouse?.name_ar || warehouse?.name || warehouseId || 'مخزن غير محدد'
}

const getStockForItem = async (itemId: string, warehouseId: string): Promise<number> => {
  try {
    const key = `${warehouseId}-${itemId}`
    const cachedItem = inventoryStore.itemsMap.get(key)
    if (cachedItem && cachedItem.remainingQuantity !== undefined) {
      return cachedItem.remainingQuantity
    }
    
    const item = await inventoryStore.fetchItemById(itemId)
    ...
    console.error('Failed to get stock for item:', error)
    return 0
  }
}
...
const validateStockLevels = async (): Promise<boolean> => {
  ...
  for (const item of form.items) {
    const warehouseId = item.warehouse_id || selectedWarehouseId.value
  ...
  for (const [warehouseId, items] of itemsByWarehouse) {
    for (const item of items) {
      const currentStock = await getStockForItem(item.item_id, warehouseId)
      
      if (item.quantity > currentStock) {
        const warehouseName = getWarehouseName(warehouseId)
        showToast(
          `الكمية المطلوبة للصنف ${item.name} (${item.quantity}) أكبر من المخزون المتاح في ${warehouseName} (${currentStock})`,
          'error'
        )
        return false
      }
...
const saveInvoice = async () => {
  ...
  // Always create as draft - status changes happen in InvoiceList
  ...
  const invoiceData = {
    ...
    items: form.items.map(item => ({
      ...
      warehouse_id: item.warehouse_id || selectedWarehouseId.value,
      per_carton_count: 12
    })),
    warehouse_id: selectedWarehouseId.value,
    country: form.country,
  ...
  try {
    ...
    if (result.success) {
      ...
      // If user selected 'issued' or 'paid', update the status after creation
      // This ensures stock deduction happens correctly through the status update flow
      if (!isEdit.value && result.data && (form.status === 'issued' || form.status === 'paid')) {
        const statusResult = await invoiceStore.updateInvoiceStatus(result.data.id, form.status as 'issued' | 'paid')
        if (!statusResult.success) {
          showToast(`تم إنشاء الفاتورة ولكن فشل تحديث الحالة: ${statusResult.message}`, 'warning')
        }
      }
...
onMounted(async () => {
  ...
  if (id) {
    ...
    if (invoice) {
      ...
      const cachedItems = inventoryStore.items.filter(
        (item: any) => item.warehouseId === selectedWarehouseId.value
      )
      if (cachedItems.length > 0) {
        displayItems.value = cachedItems
      } else {
    ...
    const cacheRestored = restoreFormFromCache()
    if (!cacheRestored) {
