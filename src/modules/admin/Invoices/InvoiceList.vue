<template>
  <div class="w-full px-2 sm:px-4 py-3 sm:py-6 pb-20 sm:pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    <div v-if="authStore.isViewOnly" class="mb-3 bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-300 dark:border-yellow-700 rounded-lg p-2.5 shadow-sm hover:shadow-md transition-all duration-300">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-4">
      ...
      <div class="flex gap-1.5 w-full sm:w-auto flex-wrap">
        <button @click="exportToExcelSimple" class="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-1.5 shadow-md hover:shadow-lg text-xs sm:text-sm font-semibold min-h-[44px] border border-blue-400/20">
        ...
        <button 
          ...
          class="flex-1 sm:flex-none bg-teal-600 hover:bg-teal-700 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-1.5 shadow-md hover:shadow-lg text-xs sm:text-sm font-semibold min-h-[44px] border border-teal-400/20"
        ...
        <router-link 
          ...
          to="/invoices/new" 
          class="flex-1 sm:flex-none bg-green-600 hover:bg-green-700 text-white px-3 sm:px-4 py-2 rounded-xl transition-all inline-flex items-center justify-center gap-1.5 shadow-md hover:shadow-lg text-xs sm:text-sm font-semibold min-h-[44px] border border-green-400/20"
        >
          <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-1.5 sm:gap-2 mb-3">
      <div v-if="invoiceStore.isLoading" class="bg-gradient-to-br from-slate-500 to-slate-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white animate-pulse border border-white/10">
        <div class="h-3 w-16 bg-white/30 rounded mb-1"></div>
        <div class="h-5 w-10 bg-white/30 rounded"></div>
      </div>
      <div v-else class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-[10px] sm:text-xs font-bold text-blue-100 truncate">إجمالي الفواتير</p>
        <p class="text-sm sm:text-base lg:text-lg font-black truncate" :title="String(invoiceStore.totalInvoices)">{{ formatNumber(invoiceStore.totalInvoices) }}</p>
      </div>
      ...
      <div v-else class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-[10px] sm:text-xs font-bold text-green-100 truncate">إجمالي المبالغ</p>
        <p class="text-xs sm:text-sm lg:text-base font-black truncate" :title="String(invoiceStore.totalAmount)">{{ formatCurrencyShort(invoiceStore.totalAmount) }}</p>
      </div>
      ...
      <div v-else class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-[10px] sm:text-xs font-bold text-yellow-100 truncate">المبالغ المستحقة</p>
        <p class="text-xs sm:text-sm lg:text-base font-black truncate" :title="String(invoiceStore.pendingAmount)">{{ formatCurrencyShort(invoiceStore.pendingAmount) }}</p>
      </div>
      ...
      <div v-else class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-[10px] sm:text-xs font-bold text-purple-100 truncate">الفواتير النشطة</p>
        <p class="text-sm sm:text-base lg:text-lg font-black truncate" :title="String(activeInvoices)">{{ formatNumber(activeInvoices) }}</p>
      </div>
    </div>

    <div class="card-warm p-3 sm:p-4 mb-4">
      <div class="relative mb-3">
        ...
        <button v-if="invoiceStore.invoiceFilters.search.length > 0 && !isSearching" @click="clearSearch" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 min-h-[44px] px-2">
      ...
      <div v-if="invoiceStore.invoiceFilters.search.length > 0" class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
        <span>{{ filteredInvoices.length }} نتيجة <span v-if="isSearching">(جارٍ البحث...)</span></span>
        <span v-if="filteredInvoices.length === 0 && invoiceStore.invoiceFilters.search.length >= 2 && !isSearching" class="text-amber-600 dark:text-amber-400">لا توجد نتائج مطابقة</span>
      </div>

      <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-2">
        <select v-model="invoiceStore.invoiceFilters.status" @change="applyFilters" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
        ...
        <select v-model="invoiceStore.invoiceFilters.type" @change="applyFilters" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
        ...
        <input
          ...
          @change="applyFilters"
          type="month"
          class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors"
        />
      </div>
      ...
      <div class="flex justify-between items-center mt-2">
        ...
        <div class="text-xs font-medium text-gray-500 dark:text-gray-400">
          إجمالي {{ formatNumber(filteredInvoices.length) }} فاتورة
        </div>
      </div>
    </div>
      ...
      <div class="table-container" style="max-height: 65vh; min-height: 300px; overflow-y: auto; overflow-x: auto;">
        <table class="w-full min-w-[700px]">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <template v-if="invoiceStore.isLoading && invoiceStore.invoices.length === 0">
              ...
              <tr v-for="invoice in paginatedInvoices" :key="invoice.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                ...
                <td class="px-2 py-2.5 text-center whitespace-nowrap">
                  <button 
                    ...
                    class="status-button"
                    :class="[
                      getStatusButtonClass(invoice.status),
                      isUpdatingStatus && updatingInvoiceId === invoice.id ? 'status-loading' : ''
                    ]"
                    :disabled="isUpdatingStatus && updatingInvoiceId === invoice.id"
                  >
                    <span class="status-dot" :class="getStatusDotClass(invoice.status)"></span>
                    <span class="status-text">{{ getStatusText(invoice.status) }}</span>
                    <svg v-if="!(isUpdatingStatus && updatingInvoiceId === invoice.id)" class="status-chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                  ...
                  <span v-else :class="getStatusBadge(invoice.status)" class="px-2 py-0.5 text-[10px] sm:text-xs font-bold rounded-full border">
                ...
                <td class="px-2 py-2.5 text-center">
                  <div class="flex items-center justify-center gap-0.5 sm:gap-1">
                    ...
                    <button 
                      ...
                      @click="openDeleteConfirmation(invoice)" 
                      class="p-1.5 sm:p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors min-h-[40px] min-w-[40px] sm:min-h-[44px] sm:min-w-[44px] flex items-center justify-center" 
                      title="حذف"
                    >
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <tr v-if="!invoiceStore.isLoading && filteredInvoices.length === 0">
    ...
    <div v-if="filteredInvoices.length > invoiceStore.invoicePagination.pageSize" class="flex flex-col sm:flex-row justify-between items-center gap-2 mt-3">
      ...
      <div class="flex gap-1.5 order-1 sm:order-2">
        <button @click="prevPage" :disabled="invoiceStore.invoicePagination.currentPage === 1" class="px-3 py-1.5 text-xs sm:text-sm border border-gray-300 dark:border-gray-600 rounded-xl disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 font-bold min-h-[40px] hover:border-amber-300 dark:hover:border-amber-600">السابق</button>
        ...
        <button @click="nextPage" :disabled="invoiceStore.invoicePagination.currentPage === totalPages" class="px-3 py-1.5 text-xs sm:text-sm border border-gray-300 dark:border-gray-600 rounded-xl disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 font-bold min-h-[40px] hover:border-amber-300 dark:hover:border-amber-600">التالي</button>
      </div>
    </div>
  ...
  <DeleteConfirmationModal
    ...
    :invoice-id="invoiceToDelete?.id || null"
...
const updatingInvoiceId = ref<string | null>(null)
...
const canDeleteInvoice = computed(() => authStore.isCompanyManager)
...
const invoiceCalculations = computed(() => {
  if (!selectedInvoice.value) {
    return { subtotal: 0, discountAmount: 0, vatAmount: 0, totalAmount: 0 }
  }
...
const activeInvoices = computed(() => {
  return invoiceStore.invoices.filter((i: any) => i.status !== 'paid' && i.status !== 'cancelled').length
})
...
const filteredInvoices = computed(() => {
  ...
  if (status) filtered = filtered.filter((inv: any) => inv.status === status)
  if (type) filtered = filtered.filter((inv: any) => inv.type === type)
  if (dateRange) {
...
const totalPages = computed(() => {
  return Math.ceil(filteredInvoices.value.length / invoiceStore.invoicePagination.pageSize)
})
...
const debounceSearch = (): void => {
  ...
  const searchTerm = invoiceStore.invoiceFilters.search
...
const clearSearch = (): void => {
  invoiceStore.invoiceFilters.search = ''
  isSearching.value = false
...
const resetFilters = (): void => {
  invoiceStore.invoiceFilters.search = ''
  invoiceStore.invoiceFilters.status = ''
  invoiceStore.invoiceFilters.type = ''
  invoiceStore.invoiceFilters.dateRange = ''
  isSearching.value = false
...
const getTypeText = (type: string): string => {
  const types: Record<string, string> = { B2B: 'أعمال', B2C: 'فرد', simplified: 'مبسط' }
...
const viewInvoice = (invoice: any): void => {
...
const openDeleteConfirmation = (invoice: any): void => {
...
const confirmDeleteInvoice = async (data: { 
  invoiceId: string; 
  returnItems: boolean 
}): Promise<void> => {
  try {
    const result = await invoiceStore.deleteInvoice(data.invoiceId, {
    ...
    if (result.success) {
      showDeleteModal.value = false
      invoiceToDelete.value = null
      await invoiceStore.fetchInvoices(true)
      alert(result.message || 'تم حذف الفاتورة بنجاح')
    } else {
      alert(result.message || 'فشل حذف الفاتورة')
    }
  } catch (error: any) {
...
const getWarehouseName = (warehouseId: string): string => {
  ...
  const warehouse = warehouseStore.warehouses.find((w: any) => w.id === warehouseId)
...
const performDispatchDeductionForInvoice = async (invoice: any): Promise<boolean> => {
  ...
  for (const [warehouseId, items] of itemsByWarehouse) {
    if (warehouseId === 'unknown') {
      console.warn('⚠️ Items with unknown warehouse:', items.map((i: any) => i.name).join(', '))
      failedItems.push(...items.map((i: any) => `${i.name} (مخزن غير محدد)`))
      allSuccess = false
      continue
    }
...
const updateStatus = async (invoice: any): Promise<void> => {
  ...
  try {
    ...
    if (result.success) {
      if (shouldDeductStock) {
        const itemsWithoutWarehouse = (invoice.items || []).filter((item: any) => !item.warehouse_id)
        ...
        alert(`✅ تم تحديث حالة الفاتورة إلى "${statusText}" بنجاح`)
      }
      
      await invoiceStore.fetchInvoices()
    } else {
      alert(result.message || '❌ فشل في تحديث حالة الفاتورة')
    }
  } catch (error) {
...
const printSingleInvoice = (invoice: any): void => {
  const companyName = invoiceStore.companyInfo?.name || ''
  const companyTaxNumber = invoiceStore.companyInfo?.taxNumber || ''
  const companyAddress = invoiceStore.companyInfo?.address || ''
  const companyPhone = invoiceStore.companyInfo?.phone || ''
  const companyEmail = invoiceStore.companyInfo?.email || ''
...
const exportToExcelSimple = (): void => {
  const exportData = filteredInvoices.value.map((inv: any) => ({
    ...
    'اسم الشركة': invoiceStore.companyInfo?.name || '',
    'السجل الضريبي للشركة': invoiceStore.companyInfo?.taxNumber || '',
    'عنوان الشركة': invoiceStore.companyInfo?.address || '',
    'هاتف الشركة': invoiceStore.companyInfo?.phone || '',
    'بريد الشركة': invoiceStore.companyInfo?.email || ''
  }))
  const ws = XLSX.utils.json_to_sheet(exportData)
  const wb = XLSX.utils.book_new()
  XLSX.utils.book_append_sheet(wb, ws, 'الفواتير')
  XLSX.writeFile(wb, `invoices_${(invoiceStore.companyInfo?.name || '').replace(/\s/g, '_')}_${new Date().toISOString().split('T')[0]}.xlsx`)
}

const exportToExcelAdvanced = async (): Promise<void> => {
  ...
  try {
    await InvoiceExportService.exportToExcel(invoicesToExport, {
      ...
      companyInfo: {
        name: invoiceStore.companyInfo?.name || '',
        taxNumber: invoiceStore.companyInfo?.taxNumber || '',
        address: invoiceStore.companyInfo?.address || '',
        phone: invoiceStore.companyInfo?.phone || '',
        email: invoiceStore.companyInfo?.email || ''
      }
    })
  } catch (error) {
...
onMounted(async () => {
  ...
  if (!invoiceStore.dataLoaded || invoiceStore.invoices.length === 0) {
    await invoiceStore.fetchInvoices()
  }
})
...
.status-draft:hover:not(:disabled) {
...
.status-issued:hover:not(:disabled) {
...
.status-paid:hover:not(:disabled) {
...
.dark .status-draft:hover:not(:disabled) {
...
.dark .status-issued:hover:not(:disabled) {
...
.dark .status-paid:hover:not(:disabled) {
