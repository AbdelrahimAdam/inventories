...
// Perform dispatch deduction for invoice - IDENTICAL to dispatch modal with warehouse respect
const performDispatchDeductionForInvoice = async (invoice: any): Promise<boolean> => {
  ...
  console.log('🏢 Warehouse ID:', invoice.warehouse_id)
  
  // CRITICAL: Check if warehouse_id exists
  if (!invoice.warehouse_id) {
    console.error('❌ No warehouse_id found in invoice!')
    alert('⚠️ لا يمكن خصم المخزون: لم يتم تحديد المخزن في الفاتورة')
    return false
  }
  
  try {
    for (const item of invoice.items || []) {
      console.log(`📦 Processing item: ${item.name} (${item.item_id}) - Quantity: ${item.quantity}`)
      console.log(`🏢 Using warehouse: ${invoice.warehouse_id}`)
      ...
      console.log(`📝 Dispatching from warehouse ${invoice.warehouse_id}: ${cartonsToDispatch} cartons + ${singlesToDispatch} singles = ${item.quantity} units`)

      // CRITICAL: Use the warehouse_id from the invoice
      const result = await inventoryStore.dispatchItem({
        item_id: item.item_id,
        from_warehouse_id: invoice.warehouse_id, // Use the invoice's warehouse
      ...
      if (!result.success) {
        ...
        alert(`❌ فشل في خصم الصنف ${item.name} من المخزن: ${result.message}`)
        return false
      }
    }
    
    console.log('✅ All items dispatched successfully from warehouse:', invoice.warehouse_id)
    return true
  } catch (error: any) {
    console.error('❌ Dispatch deduction error:', error)
    alert(`❌ حدث خطأ أثناء خصم المخزون: ${error.message || 'خطأ غير معروف'}`)
    return false
  }
}
...
const updateStatus = async (invoice: any) => {
  ...
  if (shouldDeductStock) {
    confirmMessage += `\n\n⚠️ سيتم خصم ${invoice.items?.length || 0} صنف من المخزن (${invoice.warehouse_id || 'غير محدد'}). هل أنت متأكد؟`
  }
  ...
  try {
    ...
    if (result.success) {
      // If we moved from draft to issued/paid, deduct stock from the correct warehouse
      if (shouldDeductStock) {
        // Make sure we have the warehouse_id
        if (!invoice.warehouse_id) {
          alert('⚠️ تم تحديث حالة الفاتورة ولكن لا يمكن خصم المخزون: لم يتم تحديد المخزن في الفاتورة')
        ...
        if (!dispatchSuccess) {
          alert('⚠️ تم تحديث حالة الفاتورة ولكن فشل في خصم المخزون. يرجى التحقق من المخزون يدوياً.')
        } else {
          alert('✅ تم تحديث حالة الفاتورة وخصم المخزون بنجاح')
        }
      } else {
