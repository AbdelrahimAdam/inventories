...
// ============================================================
// CACHE PERSISTENCE - Keep data when navigating away
// ============================================================
const INVOICE_LIST_CACHE_KEY = 'invoice_list_cache'

// Store the cached data in refs since store properties are read-only
...
function saveListToCache() {
  try {
    const cacheData = {
      ...
      totalInvoices: invoiceStore.totalInvoices,
      totalAmount: invoiceStore.totalAmount,
      pendingAmount: invoiceStore.pendingAmount,
      filters: invoiceStore.invoiceFilters,
      pagination: invoiceStore.invoicePagination,
      savedAt: new Date().toISOString()
    }
    localStorage.setItem(INVOICE_LIST_CACHE_KEY, JSON.stringify(cacheData))
  } catch (e) {
    console.warn('Failed to save invoice list cache:', e)
  }
}

function restoreListFromCache(): boolean {
  try {
    const raw = localStorage.getItem(INVOICE_LIST_CACHE_KEY)
    if (!raw) return false
    
    const cache = JSON.parse(raw)
    if (!cache.invoices || cache.invoices.length === 0) return false
    
    // Restore data by assigning to the store's reactive properties
    // Use a workaround since some properties are read-only
    invoiceStore.invoices.splice(0, invoiceStore.invoices.length, ...cache.invoices)
    cachedData.value.invoices = cache.invoices
    cachedData.value.totalInvoices = cache.totalInvoices
    cachedData.value.totalAmount = cache.totalAmount
    cachedData.value.pendingAmount = cache.pendingAmount
    
    // Restore filters and pagination
    invoiceStore.invoiceFilters.search = cache.filters.search || ''
    invoiceStore.invoiceFilters.status = cache.filters.status || ''
    invoiceStore.invoiceFilters.type = cache.filters.type || ''
    invoiceStore.invoiceFilters.dateRange = cache.filters.dateRange || ''
    invoiceStore.invoicePagination.currentPage = cache.pagination.currentPage || 1
    invoiceStore.invoicePagination.pageSize = cache.pagination.pageSize || 15
    ...
    console.warn('Failed to restore invoice list cache:', e)
    return false
  }
}

// ============================================================
// LIFECYCLE HOOKS - Preserve state on navigation
// ============================================================
onActivated(async () => {
  // If we have cached data, use it
  if (dataLoaded && invoiceStore.invoices.length > 0) {
    isLoadingInvoices.value = false
    isLoadingStats.value = false
    return
  }
  ...
  const cacheRestored = restoreListFromCache()
  if (cacheRestored) {
    return
  }
...
const updateStatus = async (invoice: any): Promise<void> => {
  ...
  try {
    ...
    if (result.success) {
      ...
      // Refresh the invoice list and cache
      await invoiceStore.fetchInvoices()
      saveListToCache()
    } else {
