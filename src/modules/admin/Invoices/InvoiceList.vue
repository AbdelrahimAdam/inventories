      ...
      <div class="table-container" style="max-height: 65vh; min-height: 300px; overflow-y: auto; overflow-x: auto;">
        <table class="w-full min-w-[700px]">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <!-- Skeleton rows - only show when loading and no data -->
...
// ⭐ FIXED: Use authStore.canEdit directly (same as ItemList)
...
// ============================================================
// Lifecycle hooks - Using invoiceStore's built-in caching
// ============================================================
onMounted(async () => {
  await invoiceStore.fetchCompanyInfo()
  await warehouseStore.fetchWarehouses()
  
  // ⭐ Use invoiceStore's built-in cache
  // The store handles cache loading automatically
...
onActivated(async () => {
  // ⭐ On reactivation, check if data is loaded
  // The store's dataLoaded flag tells us if we have cached data
  if (!invoiceStore.dataLoaded || invoiceStore.invoices.length === 0) {
    await invoiceStore.fetchInvoices()
  }
})
...
onDeactivated(() => {
  // ⭐ No need to save cache - store handles it automatically
  // Clean up timers only
