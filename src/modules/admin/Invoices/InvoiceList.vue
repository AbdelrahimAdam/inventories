<template>
  <div class="w-full px-2 sm:px-4 py-3 sm:py-6 pb-20 sm:pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3 mb-4">
      <div v-if="invoiceStore.isLoading" class="bg-gradient-to-br from-slate-500 to-slate-600 rounded-xl shadow-lg p-3 sm:p-4 text-white animate-pulse">
...
const confirmDeleteInvoice = async (data: { 
  ...
  try {
    const invoice = invoiceToDelete.value
    
    if (data.returnItems) {
      // Return stock to warehouses
      const returnResult = await returnStockForInvoice(invoice)
      if (!returnResult) {
        alert('فشل إرجاع المخزون. يرجى المحاولة مرة أخرى.')
        return
      }
    }
...
const returnStockForInvoice = async (invoice: any): Promise<boolean> => {
  ...
  for (const [warehouseId, items] of itemsByWarehouse) {
    if (warehouseId === 'unknown') {
      alert(`⚠️ الأصناف التالية ليس لها مخزن محدد: ${items.map((i: any) => i.name).join(', ')}`)
      return false
    }
    ...
    for (const item of items) {
      ...
      if (fetchError) {
        alert(`الصنف ${item.name} غير موجود`)
        return false
      }
      ...
      if (updateError) {
        alert(`فشل إرجاع مخزون الصنف ${item.name}`)
        return false
      }
      ...
      await supabase.from('transactions').insert({
        ...
        destination: 'invoice_delete_return',
        destination_id: invoice.id,
        total_delta: item.quantity,
        new_remaining: newQuantity,
        user_id: authStore.user?.id,
        notes: `إرجاع من الفاتورة المحذوفة #${invoice.invoice_number}`,
