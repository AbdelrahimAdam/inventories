              ...
              <tr v-for="invoice in paginatedInvoices" :key="invoice.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                ...
                <td class="px-2 py-2.5 text-center">
                  <div class="flex items-center justify-center gap-0.5 sm:gap-1">
                    ...
                    <button @click="exportSingleInvoiceToExcel(invoice)" class="p-1.5 sm:p-2 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/30 rounded-lg transition-colors min-h-[40px] min-w-[40px] sm:min-h-[44px] sm:min-w-[44px] flex items-center justify-center" title="تصدير Excel">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="print-invoice max-w-4xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 mb-4 sm:mb-6">
              ...
              <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-2.5 sm:p-4 bg-gray-50 dark:bg-gray-900/30">
                ...
                <!-- Company Logo -->
                <div v-if="invoiceStore.companyInfo?.logoUrl" class="flex justify-center mb-3">
                  <img 
                    :src="invoiceStore.companyInfo.logoUrl" 
                    alt="شعار الشركة" 
                    class="h-12 sm:h-16 object-contain"
                    @error="handleLogoError"
                  />
                </div>
                <p class="text-gray-800 dark:text-gray-200 font-semibold text-xs sm:text-sm">{{ invoiceStore.companyInfo.name || companyInfo.name }}</p>
                <p class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs mt-0.5">السجل الضريبي: {{ invoiceStore.companyInfo.taxNumber || companyInfo.taxNumber }}</p>
                <p class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs">{{ invoiceStore.companyInfo.address || companyInfo.address }}</p>
                <p class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs">هاتف: {{ invoiceStore.companyInfo.phone || companyInfo.phone }}</p>
                <p class="text-gray-600 dark:text-gray-400 text-[10px] sm:text-xs">البريد الإلكتروني: {{ invoiceStore.companyInfo.email || companyInfo.email }}</p>
              </div>
              <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-2.5 sm:p-4 bg-gray-50 dark:bg-gray-900/30">
                <div class="flex items-center gap-2 mb-2 pb-1.5 border-b border-gray-200 dark:border-gray-700">
            ...
            <div class="text-center text-[10px] text-gray-400 dark:text-gray-500 mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
              <p>هذه الفاتورة صادرة من {{ invoiceStore.companyInfo.name || companyInfo.name }} - شكراً لتعاملكم معنا</p>
              <p class="mt-0.5">للتواصل: {{ invoiceStore.companyInfo.phone || companyInfo.phone }} | البريد الإلكتروني: {{ invoiceStore.companyInfo.email || companyInfo.email }}</p>
            </div>
          </div>
        </div>
      </div>
...
// Fallback company info (will be replaced by store data)
...
const generateInvoiceHTML = (invoice: any): string => {
  // Get company info from store with fallback
  ...
  const logoHTML = companyLogo 
    ? `<div style="text-align: center; margin-bottom: 10px;"><img src="${companyLogo}" alt="شعار الشركة" style="max-height: 60px; width: auto;" /></div>` 
    : ''

  return `
    <div style="direction: rtl; font-family: 'Cairo', Arial, sans-serif; padding: 40px; max-width: 1100px; margin: 0 auto; background: white;">
      <div style="text-align: center; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #2F75B5;">
        ${logoHTML}
        <div style="display: inline-block; padding: 10px; background: linear-gradient(135deg, #d97706, #16a34a); border-radius: 50%; margin-bottom: 10px;">
          <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2">
            <path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
        </div>
        <h1 style="font-size: 28px; font-weight: bold; color: #1f2937; margin-bottom: 8px;">فاتورة ضريبية</h1>
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 30px;">
        <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 15px; background-color: #f9fafb;">
          <h3 style="font-size: 16px; font-weight: bold; color: #1f2937; margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid #e5e7eb; display: flex; align-items: center; gap: 8px;">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#d97706" stroke-width="2">
              <path d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
            </svg>
            بيانات الشركة
          </h3>
          ${companyLogo ? `<div style="text-align: center; margin-bottom: 8px;"><img src="${companyLogo}" alt="شعار الشركة" style="max-height: 50px; width: auto;" /></div>` : ''}
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;"><strong>${companyName}</strong></p>
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">السجل الضريبي: ${companyTaxNumber}</p>
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">${companyAddress}</p>
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">هاتف: ${companyPhone}</p>
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">البريد الإلكتروني: ${companyEmail}</p>
        </div>
        ...
        <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 15px; background-color: #f9fafb;">
          ...
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;"><strong>${invoice.customer?.name || '—'}</strong></p>
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">هاتف: ${invoice.customer?.phone || '—'}</p>
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">البريد الإلكتروني: ${invoice.customer?.email || '—'}</p>
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">العنوان: ${invoice.customer?.address || '—'}</p>
          <p style="margin: 4px 0; color: #4b5563; font-size: 13px;">الرقم الضريبي: ${invoice.customer?.tax_number || '—'}</p>
        </div>
      </div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
          <thead>
            <tr style="background-color: #1f2937; color: white;">
              <th style="padding: 12px; text-align: center; font-weight: bold; font-size: 13px; border: 1px solid #374151;">#</th>
              <th style="padding: 12px; text-align: right; font-weight: bold; font-size: 13px; border: 1px solid #374151;">الصنف</th>
              <th style="padding: 12px; text-align: center; font-weight: bold; font-size: 13px; border: 1px solid #374151;">الكمية</th>
              <th style="padding: 12px; text-align: center; font-weight: bold; font-size: 13px; border: 1px solid #374151;">سعر الوحدة</th>
              <th style="padding: 12px; text-align: center; font-weight: bold; font-size: 13px; border: 1px solid #374151;">الإجمالي</th>
            </tr>
          </thead>
          <tbody>
            ${(invoice.items || []).map((item: any, idx: number) => `
              <tr style="border-bottom: 1px solid #e5e7eb;">
                <td style="padding: 10px 12px; text-align: center; font-size: 13px; border: 1px solid #e5e7eb;">${idx + 1}</td>
                <td style="padding: 10px 12px; text-align: right; font-size: 13px; border: 1px solid #e5e7eb;">
                  <div style="font-weight: 500; color: #1f2937;">${item.name}</div>
                  <div style="font-size: 11px; color: #6b7280;">الكود: ${item.code || '—'}</div>
                  ${item.size ? `<div style="font-size: 11px; color: #6b7280;">المقاس: ${item.size}</div>` : ''}
                </td>
                <td style="padding: 10px 12px; text-align: center; font-size: 13px; border: 1px solid #e5e7eb;">${item.quantity}</td>
                <td style="padding: 10px 12px; text-align: center; font-size: 13px; border: 1px solid #e5e7eb;">${formatCurrency(item.unit_price)}</td>
                <td style="padding: 10px 12px; text-align: center; font-size: 13px; border: 1px solid #e5e7eb; font-weight: bold;">${formatCurrency(item.total)}</td>
              </tr>
            `).join('')}
          </tbody>
        </table>
      ...
      <div style="display: flex; justify-content: flex-end; margin-bottom: 30px;">
        <div style="width: 100%; max-width: 400px;">
          <table style="width: 100%; border-collapse: collapse;">
            <tr>
              <td style="padding: 8px 12px; text-align: right; font-weight: bold; color: #4b5563; border-bottom: 1px solid #e5e7eb;">المجموع الفرعي</td>
              <td style="padding: 8px 12px; text-align: left; font-weight: 600; color: #1f2937; border-bottom: 1px solid #e5e7eb;">${formatCurrency(invoice.subtotal)}</td>
            </tr>
            ${invoice.discount_amount > 0 ? `
            <tr>
              <td style="padding: 8px 12px; text-align: right; font-weight: bold; color: #4b5563; border-bottom: 1px solid #e5e7eb;">الخصم (${invoice.discount_value} ${invoice.discount_type === 'percentage' ? '%' : 'ج.م'})</td>
              <td style="padding: 8px 12px; text-align: left; font-weight: 600; color: #dc2626; border-bottom: 1px solid #e5e7eb;">-${formatCurrency(invoice.discount_amount)}</td>
            </tr>` : ''}
            ${invoice.shipping_cost > 0 ? `
            <tr>
              <td style="padding: 8px 12px; text-align: right; font-weight: bold; color: #4b5563; border-bottom: 1px solid #e5e7eb;">الشحن</td>
              <td style="padding: 8px 12px; text-align: left; font-weight: 600; color: #1f2937; border-bottom: 1px solid #e5e7eb;">${formatCurrency(invoice.shipping_cost)}</td>
            </tr>` : ''}
            <tr>
              <td style="padding: 8px 12px; text-align: right; font-weight: bold; color: #4b5563; border-bottom: 1px solid #e5e7eb;">الضريبة (${invoice.vat_rate || 0}%)</td>
              <td style="padding: 8px 12px; text-align: left; font-weight: 600; color: #1f2937; border-bottom: 1px solid #e5e7eb;">${formatCurrency(invoice.vat_amount)}</td>
            </tr>
            <tr style="background-color: #f0fdf4; border-top: 2px solid #16a34a;">
              <td style="padding: 8px 12px; text-align: right; font-weight: bold; font-size: 16px; color: #1f2937;">الإجمالي النهائي</td>
              <td style="padding: 8px 12px; text-align: left; font-weight: 800; font-size: 18px; color: #16a34a;">${formatCurrency(invoice.total_amount)}</td>
            </tr>
          </table>
        </div>
      </div>
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; margin-top: 40px; padding-top: 20px; border-top: 1px solid #e5e7eb;">
        <div style="text-align: center; padding-top: 30px; border-top: 1px solid #9ca3af; margin-top: 10px;">
      ...
      <div style="text-align: center; margin-top: 30px; padding-top: 20px; border-top: 1px solid #e5e7eb; color: #9ca3af; font-size: 11px;">
        ...
        <p style="margin-top: 5px;">للتواصل: ${companyPhone} | البريد الإلكتروني: ${companyEmail}</p>
...
const downloadInvoicePDF = async (invoice: any) => {
  ...
  try {
    ...
    pdf.save(`فاتورة_${invoice.invoice_number}.pdf`)
  } catch (error) {
