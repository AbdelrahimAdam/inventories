<template>
  <div class="w-full px-2 sm:px-4 py-3 sm:py-6 pb-20 sm:pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-1.5 sm:gap-2 mb-3">
      <div v-if="invoiceStore.isLoading" class="bg-gradient-to-br from-slate-500 to-slate-600 rounded-lg shadow p-1.5 sm:p-2 text-white animate-pulse">
        <div class="h-3 w-16 bg-white/30 rounded mb-1"></div>
        <div class="h-5 w-10 bg-white/30 rounded"></div>
      </div>
      <div v-else class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow p-1.5 sm:p-2 text-white overflow-hidden">
      ...
      <div v-else class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow p-1.5 sm:p-2 text-white overflow-hidden">
      ...
      <div v-else class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-lg shadow p-1.5 sm:p-2 text-white overflow-hidden">
      ...
      <div v-else class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow p-1.5 sm:p-2 text-white overflow-hidden">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-3 sm:p-4 mb-4">
      <!-- Search Bar - Prominent like item list -->
      ...
      <div v-if="invoiceStore.invoiceFilters.search.length > 0" class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
        ...
        <span v-if="filteredInvoices.length === 0 && invoiceStore.invoiceFilters.search.length >= 2 && !isSearching" class="text-green-600 dark:text-green-400">لا توجد نتائج مطابقة</span>
      </div>

      <!-- Filter Grid -->
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-2">
        <select v-model="invoiceStore.invoiceFilters.status" @change="applyFilters" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px]">
        ...
        <select v-model="invoiceStore.invoiceFilters.type" @change="applyFilters" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px]">
        ...
        <input
          ...
          @change="applyFilters"
          type="month"
          class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px]"
        />
      </div>

      <!-- Filter Actions -->
      <div class="flex justify-between items-center mt-2">
        <button @click="resetFilters" class="px-4 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-sm font-semibold text-gray-700 dark:text-gray-300 min-h-[44px]">
          إعادة تعيين
        </button>
            ...
            <template v-else>
              <tr v-for="invoice in paginatedInvoices" :key="invoice.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                ...
                <td class="px-2 py-2.5 text-center whitespace-nowrap">
                  ...
                  <span v-else :class="getStatusBadge(invoice.status)" class="px-2 py-0.5 text-[10px] sm:text-xs font-bold rounded-full">
