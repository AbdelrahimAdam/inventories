...
// Perform dispatch deduction for invoice - IDENTICAL to dispatch modal
const performDispatchDeductionForInvoice = async (invoice: any): Promise<boolean> => {
  ...
  try {
    for (const item of invoice.items || []) {
      ...
      // Get the per carton count from the inventory
      ...
      console.log(`📝 Dispatching: ${cartonsToDispatch} cartons + ${singlesToDispatch} singles = ${item.quantity} units`)

      const result = await inventoryStore.dispatchItem({
        item_id: item.item_id,
        from_warehouse_id: invoice.warehouse_id,
    ...
    console.log('✅ All items dispatched successfully!')
    return true
  } catch (error: any) {
...
const updateStatus = async (invoice: any) => {
  ...
  if (shouldDeductStock) {
    confirmMessage += `\n\n⚠️ سيتم خصم ${invoice.items?.length || 0} صنف من المخزون. هل أنت متأكد؟`
  }
  ...
  try {
    ...
    if (result.success) {
      // If we moved from draft to issued/paid, deduct stock
      if (shouldDeductStock) {
