      ...
      <div v-else class="space-y-3 max-h-[450px] overflow-y-auto">
        ...
        <div v-if="outOfStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-red-500">{{ isAllItemsLoaded ? 'الآن' : 'عينة أول 50 صنف' }}</span>
          </div>
          ...
          <div class="mt-2 text-center">
            ...
            <button v-if="!isAllItemsLoaded" @click="loadAllItemsForAlerts" class="text-xs font-bold text-blue-500 hover:underline min-h-[36px] px-2 mr-2">
              تحميل الكل
            </button>
          </div>
        </div>
        ...
        <div v-if="criticalStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-orange-500">{{ isAllItemsLoaded ? 'الآن' : 'عينة أول 50 صنف' }}</span>
          </div>
        ...
        <div v-if="lowStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-yellow-500">{{ isAllItemsLoaded ? 'الآن' : 'عينة أول 50 صنف' }}</span>
          </div>
...
// ✅ Use paginated items for initial display
...
// ✅ Use totalItems from store (paginated)
const totalItemsCount = computed(() => inventoryStore.totalItems)
...
// ✅ Use paginated items for warehouse stats
const warehouseStats = computed(() => {
  ...
  // ✅ Use items (paginated) instead of allItemsForStats
...
const loadDashboardData = async () => {
  ...
  // Load warehouses first (fast)
  await warehouseStore.fetchWarehouses()
  
  // ✅ Load ONLY paginated items (50) for instant display
...
const refreshData = async () => {
  ...
  try {
    ...
    // Reload all items in background
    setTimeout(() => {
      loadAllItemsForAlerts()
    }, 500)
  } catch (error) {
