...
// ✅ Track if this is the first ever load
...
const showSkeleton = computed(() => {
  return isFirstLoad && isLoading.value
})
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    // ✅ Load critical data first (cards + transactions)
    ...
    // ✅ Check if items already loaded (from cache)
...
onActivated(async () => {
  // ✅ On navigation back, don't show skeleton - just refresh stats in background
  if (dataInitialized.value && inventoryStore.items.length > 0) {
    const warehouseId = selectedWarehouseId.value || undefined
    // Silent refresh without skeleton
