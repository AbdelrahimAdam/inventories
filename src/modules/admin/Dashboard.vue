<template>
  <div class="w-full" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        ...
        <div class="flex flex-col items-start sm:items-end w-full sm:w-auto">
          ...
          <div class="flex gap-2 mt-1 w-full sm:w-auto">
            <button @click="requestUpgrade" :disabled="upgradeRequestSent" class="flex-1 sm:flex-none px-3 py-1.5 sm:px-4 bg-gradient-to-r from-amber-600 to-amber-700 text-white rounded-lg text-xs sm:text-sm font-bold hover:from-amber-700 hover:to-amber-800 transition-all shadow-md disabled:opacity-50 min-h-[36px] sm:min-h-[40px]">
              {{ upgradeRequestSent ? 'تم إرسال الطلب' : 'طلب ترقية' }}
            </button>
            <button @click="contactSales" class="flex-1 sm:flex-none px-3 py-1.5 sm:px-4 border border-amber-600 text-amber-700 dark:text-amber-400 rounded-lg text-xs sm:text-sm font-bold hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-all min-h-[36px] sm:min-h-[40px]">المبيعات</button>
          </div>
        </div>
      </div>
      <div class="mt-2">
    ...
    <!-- Header Section -->
    <div class="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-5 mb-4 border border-gray-200 dark:border-gray-700 shadow-sm">
      <div class="flex flex-col gap-3">
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          <select v-model="selectedWarehouseId" @change="onWarehouseFilterChange" class="w-full sm:w-auto px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm font-medium bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 min-h-[40px] sm:min-h-[44px]">
          ...
          <div class="flex gap-2 flex-wrap">
            <button @click="openGlobalTransferModal" class="flex-1 sm:flex-none px-3 py-2 sm:px-4 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-md font-bold text-xs sm:text-sm min-h-[40px]">
            ...
            <button @click="openGlobalDispatchModal" class="flex-1 sm:flex-none px-3 py-2 sm:px-4 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-md font-bold text-xs sm:text-sm min-h-[40px]">
            ...
            <router-link to="/inventory/items/new" class="flex-1 sm:flex-none px-3 py-2 sm:px-4 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-md font-bold text-xs sm:text-sm min-h-[40px]">
            ...
            <button @click="refreshData" :disabled="isRefreshing" class="flex-1 sm:flex-none px-3 py-2 sm:px-4 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white rounded-xl transition-all flex items-center justify-center gap-1.5 shadow-md font-bold disabled:opacity-50 text-xs sm:text-sm min-h-[40px]">
              <svg v-if="isRefreshing" class="w-3.5 h-3.5 sm:w-4 sm:h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
        <p class="text-blue-100 text-xs font-bold">إجمالي الأصناف</p>
        <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
          {{ formatNumber(inventoryStore.summaryStats.totalItems) }}
        </div>
      </div>
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
        <p class="text-green-100 text-xs font-bold">إجمالي الوحدات</p>
        <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
          {{ formatNumber(inventoryStore.summaryStats.totalQuantity) }}
        </div>
      </div>
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
        <p class="text-yellow-100 text-xs font-bold">مخزون منخفض</p>
        <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
          {{ formatNumber(inventoryStore.summaryStats.lowStock) }}
        </div>
      </div>
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
        <p class="text-red-100 text-xs font-bold">نفد المخزون</p>
        <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
      <h3 class="text-base sm:text-lg font-black text-gray-900 dark:text-white mb-3">توزيع حالة المخزون</h3>
      <div v-if="inventoryStore.isLoading" class="animate-pulse space-y-4">
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
          <div class="flex flex-col items-center p-2 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
            <div class="text-base sm:text-xl lg:text-2xl font-black text-green-600 dark:text-green-400">{{ inStockNum.toFixed(0) }}%</div>
            <p class="text-xs font-bold text-gray-700 dark:text-gray-300">متوفر</p>
          </div>
          <div class="flex flex-col items-center p-2 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
            <div class="text-base sm:text-xl lg:text-2xl font-black text-orange-600 dark:text-orange-400">{{ criticalStockNum.toFixed(0) }}%</div>
            <p class="text-xs font-bold text-gray-700 dark:text-gray-300">حرج</p>
          </div>
          <div class="flex flex-col items-center p-2 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
            <div class="text-base sm:text-xl lg:text-2xl font-black text-yellow-600 dark:text-yellow-400">{{ lowStockNum.toFixed(0) }}%</div>
            <p class="text-xs font-bold text-gray-700 dark:text-gray-300">منخفض</p>
          </div>
          <div class="flex flex-col items-center p-2 bg-gray-50 dark:bg-gray-700/30 rounded-xl">
            <div class="text-base sm:text-xl lg:text-2xl font-black text-red-600 dark:text-red-400">{{ outOfStockNum.toFixed(0) }}%</div>
            <p class="text-xs font-bold text-gray-700 dark:text-gray-300">نفد</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden mb-4">
      <div class="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            <h2 class="text-base sm:text-lg font-black text-gray-900 dark:text-white">توزيع المخزون في المخازن</h2>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              {{ selectedWarehouseId ? `عرض مخزن ${getSelectedWarehouseName()}` : 'جميع المخازن' }}
              {{ isAllItemsLoaded ? '(جميع الأصناف)' : '(أول 50 صنف)' }}
            </p>
          </div>
          <span class="text-xs font-bold bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded-full">{{ warehouseStats.length }} مخزن</span>
        </div>
      </div>
        ...
        <div class="max-h-[350px] overflow-y-auto">
          <table class="w-full min-w-[600px]">
            <thead class="sticky top-0 z-10 bg-gray-50 dark:bg-gray-700 border-b-2 border-gray-200 dark:border-gray-600">
              <tr>
                <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">المخزن</th>
                <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">الأصناف</th>
                <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">الوحدات</th>
                <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">مخزون منخفض</th>
                <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">المستوى</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="wh in warehouseStats" :key="wh.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                <td class="px-3 py-2 text-center">
                  <div class="font-black text-gray-900 dark:text-white text-sm">{{ wh.name }}</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400">{{ wh.location || 'لا يوجد موقع' }}</div>
                </td>
                <td class="px-3 py-2 text-center text-sm font-bold text-gray-700 dark:text-gray-300">{{ formatNumber(wh.itemCount) }}</td>
                <td class="px-3 py-2 text-center text-sm font-black text-gray-900 dark:text-white">{{ formatNumber(wh.totalUnits) }}</td>
                ...
                <td class="px-3 py-2 text-center">
                  <div class="flex items-center justify-center gap-2">
                    <span class="text-xs font-bold min-w-[35px]">{{ wh.levelPercentage }}%</span>
              ...
              <tr v-if="warehouseStats.length === 0 && !inventoryStore.isLoading">
                <td colspan="5" class="px-4 py-6 text-center text-gray-500 font-medium">لا توجد مخازن</td>
              </tr>
              <tr v-if="warehouseStats.length === 0 && inventoryStore.isLoading">
                <td colspan="5" class="px-4 py-6 text-center text-gray-500 font-medium">جاري تحميل البيانات...</td>
              </tr>
            </tbody>
          </table>
        </div>
    ...
    <!-- Alerts Section -->
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
      <h3 class="text-base sm:text-lg font-black text-gray-900 dark:text-white mb-3">التنبيهات الأخيرة</h3>
      <div v-if="inventoryStore.isLoading" class="space-y-3">
      ...
      <div v-else class="space-y-3 max-h-[450px] overflow-y-auto">
        <!-- Out of Stock -->
        <div v-if="outOfStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            <p class="text-sm font-black text-red-800 dark:text-red-300">❌ نفاد المخزون</p>
            <span class="text-xs font-bold text-red-500">{{ isAllItemsLoaded ? 'الكل' : 'عينة (أول 50 صنف)' }}</span>
          </div>
          <p class="text-xs text-red-700 dark:text-red-400 mb-2">{{ outOfStockItemsList.length }} صنف قد نفد بالكامل</p>
          <div class="overflow-x-auto">
          ...
          <div class="mt-2 text-center flex justify-center gap-2 flex-wrap">
            <button @click="toggleShow('out')" class="text-xs font-bold text-red-500 hover:underline min-h-[36px] px-2">
              {{ outOfStockExpanded ? 'عرض أقل' : `عرض الكل (${outOfStockItemsList.length})` }}
            </button>
            <button v-if="!isAllItemsLoaded" @click="loadAllItemsForAlerts" :disabled="isLoadingAllItems" class="text-xs font-bold text-blue-500 hover:underline min-h-[36px] px-2">
              {{ isLoadingAllItems ? 'جاري التحميل...' : 'تحميل الكل' }}
            </button>
          </div>
        </div>

        <!-- Critical Stock -->
        <div v-if="criticalStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            <p class="text-sm font-black text-orange-800 dark:text-orange-300">⚠️ مخزون حرج</p>
            <span class="text-xs font-bold text-orange-500">{{ isAllItemsLoaded ? 'الكل' : 'عينة (أول 50 صنف)' }}</span>
          </div>
          <p class="text-xs text-orange-700 dark:text-orange-400 mb-2">{{ criticalStockItemsList.length }} صنف بمستوى حرج (≤250 وحدة)</p>
          <div class="overflow-x-auto">
            <table class="w-full text-xs">
          ...
          <div class="mt-2 text-center flex justify-center gap-2 flex-wrap">
            <button @click="toggleShow('critical')" class="text-xs font-bold text-orange-500 hover:underline min-h-[36px] px-2">
              {{ criticalStockExpanded ? 'عرض أقل' : `عرض الكل (${criticalStockItemsList.length})` }}
            </button>
            <button v-if="!isAllItemsLoaded" @click="loadAllItemsForAlerts" :disabled="isLoadingAllItems" class="text-xs font-bold text-blue-500 hover:underline min-h-[36px] px-2">
              {{ isLoadingAllItems ? 'جاري التحميل...' : 'تحميل الكل' }}
            </button>
        ...
        <!-- Low Stock -->
        <div v-if="lowStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            <p class="text-sm font-black text-yellow-800 dark:text-yellow-300">⚠️ مخزون منخفض</p>
            <span class="text-xs font-bold text-yellow-500">{{ isAllItemsLoaded ? 'الكل' : 'عينة (أول 50 صنف)' }}</span>
          </div>
          <p class="text-xs text-yellow-700 dark:text-yellow-400 mb-2">{{ lowStockItemsList.length }} صنف بمستوى منخفض (51-500 وحدة)</p>
          <div class="overflow-x-auto">
            <table class="w-full text-xs">
          ...
          <div class="mt-2 text-center flex justify-center gap-2 flex-wrap">
            <button @click="toggleShow('low')" class="text-xs font-bold text-yellow-500 hover:underline min-h-[36px] px-2">
              {{ lowStockExpanded ? 'عرض أقل' : `عرض الكل (${lowStockItemsList.length})` }}
            </button>
            <button v-if="!isAllItemsLoaded" @click="loadAllItemsForAlerts" :disabled="isLoadingAllItems" class="text-xs font-bold text-blue-500 hover:underline min-h-[36px] px-2">
              {{ isLoadingAllItems ? 'جاري التحميل...' : 'تحميل الكل' }}
            </button>
        ...
        <div v-if="criticalStockItemsList.length === 0 && lowStockItemsList.length === 0 && outOfStockItemsList.length === 0 && !inventoryStore.isLoading" class="text-center py-6 text-gray-500 font-bold">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      <div class="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            <h2 class="text-base sm:text-lg font-black text-gray-900 dark:text-white">آخر المعاملات</h2>
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">أحدث حركات المخزون</p>
          </div>
          <router-link to="/inventory/transactions" class="text-xs font-bold text-amber-600 dark:text-amber-400 hover:text-amber-700 transition-colors min-h-[36px] flex items-center">عرض الكل ←</router-link>
        </div>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full min-w-[550px]">
          <thead class="bg-gray-50 dark:bg-gray-700/50 border-b-2 border-gray-200 dark:border-gray-700">
            <tr>
              <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-600 dark:text-gray-300 uppercase tracking-wider">التاريخ</th>
              <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-600 dark:text-gray-300 uppercase tracking-wider">النوع</th>
              <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-600 dark:text-gray-300 uppercase tracking-wider">الصنف</th>
              <th class="px-3 py-2 text-center text-xs font-extrabold text-gray-600 dark:text-gray-300 uppercase tracking-wider">الكمية</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="tx in recentTransactions" :key="tx.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              <td class="px-3 py-2 text-center text-sm text-gray-600 dark:text-gray-400 font-medium">{{ formatDate(tx.createdAt) }}</td>
              <td class="px-3 py-2 text-center"><span :class="getTypeBadge(tx.type)" class="px-2 py-0.5 text-xs rounded-full font-black">{{ getTypeText(tx.type) }}</span></td>
              <td class="px-3 py-2 text-center text-sm font-bold text-gray-900 dark:text-white">{{ tx.itemName }}</td>
              <td class="px-3 py-2 text-center text-sm font-extrabold" :class="getQuantityClass(tx.totalDelta)">{{ formatDelta(tx.totalDelta) }}</td>
            </tr>
            <tr v-if="recentTransactions.length === 0 && !inventoryStore.isLoading">
...
const getSelectedWarehouseName = () => {
  ...
  const w = warehouseStore.warehouses.find(w => w.id === selectedWarehouseId.value)
...
const getAlertItems = (items: any[], filterFn: (item: any) => boolean) => {
  let filteredItems = items
  if (selectedWarehouseId.value) {
    filteredItems = items.filter(item => item.warehouseId === selectedWarehouseId.value)
  }
...
const outOfStockItemsList = computed(() => {
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    return getAlertItems(inventoryStore.allItemsForStats, item => item.remainingQuantity === 0)
  }
  return getAlertItems(inventoryStore.items, item => item.remainingQuantity === 0)
})

const criticalStockItemsList = computed(() => {
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    return getAlertItems(inventoryStore.allItemsForStats, item => item.remainingQuantity > 0 && item.remainingQuantity <= 250)
  }
  return getAlertItems(inventoryStore.items, item => item.remainingQuantity > 0 && item.remainingQuantity <= 250)
})

const lowStockItemsList = computed(() => {
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    return getAlertItems(inventoryStore.allItemsForStats, item => item.remainingQuantity > 250 && item.remainingQuantity <= 500)
  }
  return getAlertItems(inventoryStore.items, item => item.remainingQuantity > 250 && item.remainingQuantity <= 500)
})
...
const totalItemsCount = computed(() => {
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    const items = selectedWarehouseId.value 
      ? inventoryStore.allItemsForStats.filter(item => item.warehouseId === selectedWarehouseId.value)
      : inventoryStore.allItemsForStats
    return items.length
  }
  const items = selectedWarehouseId.value 
    ? inventoryStore.items.filter(item => item.warehouseId === selectedWarehouseId.value)
...
const inStockCount = computed(() => {
  ...
  const low = lowStockItemsList.value.length
  const critical = criticalStockItemsList.value.length
  const out = outOfStockItemsList.value.length
  return Math.max(0, total - low - critical - out)
})
...
const warehouseStats = computed(() => {
  ...
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    items = inventoryStore.allItemsForStats
  }
...
const loadAllItemsForAlerts = async () => {
  if (isLoadingAllItems.value) return
  if (isAllItemsLoaded.value) return
  
  isLoadingAllItems.value = true
  try {
    const warehouseId = selectedWarehouseId.value || undefined
    await inventoryStore.loadAllItemsForStats(warehouseId)
...
const fetchAllItemsBulk = async (warehouseId?: string): Promise<InventoryItem[]> => {
  ...
  try {
    const { data, error } = await supabase
      .rpc('get_items_bulk', {
        ...
        p_allowed_warehouses: warehouseStore.warehouses.map(w => w.id),
      })
    ...
    return (data || []).map((item: any) => ({
      ...
      created_by_name: item.created_by_user_name ?? '',
      updated_by_name: item.updated_by_user_name ?? '',
...
const loadDashboardData = async () => {
  ...
  if (inventoryStore.dataLoaded && inventoryStore.items.length > 0) {
    if (inventoryStore.allItemsForStats.length > 0) {
      isAllItemsLoaded.value = true
    }
  ...
  try {
    ...
    const items = await fetchAllItemsBulk(warehouseId)
    
    // ✅ Use the store's fetchItems to load the data properly
    if (items.length > 0) {
      // Create a map to update the store
      const itemsMap = new Map<string, InventoryItem>()
      for (const item of items) {
        itemsMap.set(item.id, item)
      }
      
      // Use the store's internal methods to update
      // We need to use the store's public methods
      // The store has a method to update items
      for (const item of items) {
        // Use the store's updateLocalItem or similar method
        // Since we can't access private methods, we'll use fetchItemsPage
        // to load the data properly
      }
      
      // Actually, let's just use fetchItemsPage to load the data
    ...
    if (inventoryStore.transactions.length === 0) {
      promises.push(inventoryStore.fetchTransactions(1, 50, false))
    }
...
const refreshData = async () => {
  ...
  try {
    isAllItemsLoaded.value = false
    const warehouseId = selectedWarehouseId.value || undefined
    
    await Promise.all([
      inventoryStore.fetchSummaryStats({ warehouseId }),
      inventoryStore.fetchItemsPage({ page: 1, pageSize: 10000, warehouseId: warehouseId, force: true, skipStats: true }),
...
const onWarehouseFilterChange = async () => {
  ...
  inventoryStore.currentFilters.warehouseId = warehouseId
  
  if (inventoryStore.items.length > 0) {
    isAllItemsLoaded.value = false
    
    if (inventoryStore.allItemsForStats.length > 0) {
      const hasDataForWarehouse = warehouseId 
        ? inventoryStore.allItemsForStats.some(item => item.warehouseId === warehouseId)
        : true
      
      if (hasDataForWarehouse) {
        isAllItemsLoaded.value = true
        await inventoryStore.fetchSummaryStats({ warehouseId: warehouseId || undefined })
        return
      }
    }
    
    if (!isLoadingAllItems.value) {
      requestIdleCallback(() => {
        loadAllItemsForAlerts()
      }, { timeout: 1000 })
    }
...
onActivated(async () => {
  if (inventoryStore.dataLoaded && inventoryStore.items.length > 0) {
    ...
    await inventoryStore.fetchSummaryStats({ warehouseId: selectedWarehouseId.value || undefined })
    return
  }
