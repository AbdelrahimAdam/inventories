      ...
      <div class="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ isAllItemsLoaded ? 'حسب جميع الأصناف' : 'عرض أول 50 صنف (يتم تحميل الباقي في الخلفية)' }}</p>
          </div>
...
const loadAllItemsForAlerts = async () => {
  ...
  try {
    await inventoryStore.loadAllItemsForStats(selectedWarehouseId.value || undefined)
...
const refreshData = async () => {
  ...
  try {
    ...
    await Promise.all([
      ...
      inventoryStore.fetchItemsPage({ page: 1, pageSize: 50, force: true }),
...
function onWarehouseFilterChange() {
  ...
  inventoryStore.loadAllItemsForStats(selectedWarehouseId.value || undefined)
}
