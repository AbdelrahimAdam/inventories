<template>
  <div class="w-full pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
      ...
      <div v-else-if="!showSkeleton && !itemsLoaded" class="text-center py-4 text-gray-500">
        <div class="animate-spin rounded-full h-8 w-8 border-2 border-amber-500 border-t-transparent mx-auto mb-2"></div>
        <p class="text-sm">جاري تحميل بيانات المخزون...</p>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden mb-4">
        ...
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              ...
              <span v-else-if="!itemsLoaded">(جاري التحميل...)</span>
            </p>
          </div>
      ...
      <div class="overflow-x-auto">
        <div class="max-h-[350px] overflow-y-auto">
          ...
          <div v-else-if="!itemsLoaded" class="p-4 text-center text-gray-500">
            <div class="animate-spin rounded-full h-8 w-8 border-2 border-amber-500 border-t-transparent mx-auto mb-2"></div>
          ...
          <table v-else class="w-full min-w-[600px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-if="warehouseStats.length === 0 && itemsLoaded">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
      ...
      <div v-else-if="!itemsLoaded" class="text-center py-4 text-gray-500">
        <div class="animate-spin rounded-full h-8 w-8 border-2 border-amber-500 border-t-transparent mx-auto mb-2"></div>
      ...
      <div v-else class="space-y-3 max-h-[450px] overflow-y-auto">
        ...
        <div v-if="criticalStockItems.length === 0 && lowStockItems.length === 0 && outOfStockItems.length === 0 && itemsLoaded" class="text-center py-6 text-gray-500 font-bold">
...
const showSkeleton = computed(() => {
  return isFirstLoad.value && isLoading.value
})
...
// Filter items by warehouse - client-side filtering
...
// Alert items - using store's computed properties
...
// Chart data - using all items from store
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    // ✅ LOAD CRITICAL DATA FIRST - Cards + Transactions (Fast)
    ...
    // ✅ Show cards and transactions immediately
    isLoading.value = false
    isFirstLoad.value = false
    
    // ✅ LOAD HEAVY DATA IN BACKGROUND (Items)
    // This doesn't block the UI - user sees cards and transactions instantly
    setTimeout(async () => {
      try {
        ...
        console.error('Failed to load items in background:', error)
        loadingItems.value = false
      }
    }, 100)
...
const onWarehouseFilterChange = async () => {
  ...
  // Just update stats and transactions - filtering is client-side
  await Promise.allSettled([
    inventoryStore.fetchSummaryStats({ warehouseId }),
    inventoryStore.fetchTransactions(1, 10)
  ])
...
onActivated(async () => {
  // Only show skeleton on first load, not on navigation back
