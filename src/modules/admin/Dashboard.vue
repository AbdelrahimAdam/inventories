...
// Alert items - using store's computed properties with client-side filtering
...
// Chart data - using all items from store with client-side filtering
...
// Warehouse stats from cached items with client-side filtering
...
// Transactions with client-side filtering
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    // ✅ Load all items into cache ONCE - no warehouse filter
...
const refreshData = async () => {
  ...
  try {
    ...
    // ✅ Refresh all items - no warehouse filter
    await inventoryStore.fetchItemsBulk()
...
const onWarehouseFilterChange = async () => {
  ...
  // ✅ DON'T refetch items - just update stats
  // All filtering is done client-side via computed properties
  await Promise.allSettled([
    inventoryStore.fetchSummaryStats({ warehouseId }),
    inventoryStore.fetchTransactions(1, 10)
  ])
