...
async function loadDashboardData() {
  ...
  // Use store's dataLoaded instead of local variable
  if (inventoryStore.dataLoaded && inventoryStore.allItemsForStats.length > 0 && !isRefreshing.value) {
    return
  }
...
function onWarehouseFilterChange() {
  ...
  // Reset store's dataLoaded so it fetches for the new warehouse
...
onActivated(async () => {
  // Use store's dataLoaded instead of local variable
  if (inventoryStore.dataLoaded && inventoryStore.allItemsForStats.length > 0) {
    return
  }
