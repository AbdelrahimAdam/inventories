<template>
  <div class="w-full pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div v-if="isLoadingData" class="mb-4">
      <div class="flex items-center gap-3">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md overflow-hidden mb-3 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md p-3 sm:p-4 mb-3 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
      ...
      <div v-else-if="inventoryStore.items.length > 0" class="space-y-3 max-h-[450px] overflow-y-auto">
        <div v-if="inventoryStore.outOfStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 hover:shadow-md transition-shadow">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            <p class="text-xs sm:text-sm font-black text-red-800 dark:text-red-300">❌ نفاد المخزون</p>
            <span class="text-[10px] sm:text-xs font-bold bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 px-2 py-0.5 rounded-full border border-red-200 dark:border-red-800">{{ inventoryStore.outOfStockItems.length }} صنف</span>
          </div>
          <p class="text-[10px] sm:text-xs text-red-700 dark:text-red-400 mb-2">{{ inventoryStore.outOfStockItems.length }} صنف قد نفد بالكامل</p>
          <div class="overflow-x-auto">
          ...
          <div class="mt-2 text-center">
            <button @click="toggleShow('out')" class="text-[10px] sm:text-xs font-bold text-red-500 hover:underline min-h-[44px] px-2">
              {{ outOfStockExpanded ? 'عرض أقل' : `عرض الكل (${inventoryStore.outOfStockItems.length})` }}
            </button>
          </div>
        </div>

        <div v-if="inventoryStore.criticalStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10 border border-orange-200 dark:border-orange-800 hover:shadow-md transition-shadow">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            <p class="text-xs sm:text-sm font-black text-orange-800 dark:text-orange-300">⚠️ مخزون حرج</p>
            <span class="text-[10px] sm:text-xs font-bold bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded-full border border-orange-200 dark:border-orange-800">{{ inventoryStore.criticalStockItems.length }} صنف</span>
          </div>
          <p class="text-[10px] sm:text-xs text-orange-700 dark:text-orange-400 mb-2">{{ inventoryStore.criticalStockItems.length }} صنف بمستوى حرج (≤250 وحدة)</p>
          <div class="overflow-x-auto">
            <table class="w-full text-[10px] sm:text-xs">
          ...
          <div class="mt-2 text-center">
            <button @click="toggleShow('critical')" class="text-[10px] sm:text-xs font-bold text-orange-500 hover:underline min-h-[44px] px-2">
              {{ criticalStockExpanded ? 'عرض أقل' : `عرض الكل (${inventoryStore.criticalStockItems.length})` }}
            </button>
          </div>
        </div>

        <div v-if="inventoryStore.lowStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 hover:shadow-md transition-shadow">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            <p class="text-xs sm:text-sm font-black text-yellow-800 dark:text-yellow-300">⚠️ مخزون منخفض</p>
            <span class="text-[10px] sm:text-xs font-bold bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 px-2 py-0.5 rounded-full border border-yellow-200 dark:border-yellow-800">{{ inventoryStore.lowStockItems.length }} صنف</span>
          </div>
          <p class="text-[10px] sm:text-xs text-yellow-700 dark:text-yellow-400 mb-2">{{ inventoryStore.lowStockItems.length }} صنف بمستوى منخفض (51-500 وحدة)</p>
          <div class="overflow-x-auto">
            <table class="w-full text-[10px] sm:text-xs">
          ...
          <div class="mt-2 text-center">
            <button @click="toggleShow('low')" class="text-[10px] sm:text-xs font-bold text-yellow-500 hover:underline min-h-[44px] px-2">
              {{ lowStockExpanded ? 'عرض أقل' : `عرض الكل (${inventoryStore.lowStockItems.length})` }}
            </button>
          </div>
        </div>

        <div v-if="inventoryStore.criticalStockItems.length === 0 && inventoryStore.lowStockItems.length === 0 && inventoryStore.outOfStockItems.length === 0 && !isLoadingData" class="text-center py-6 text-gray-500 font-bold">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md overflow-hidden hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[550px]">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="isLoadingData && inventoryStore.transactions.length === 0" class="animate-pulse">
            ...
            <tr v-if="!isLoadingData && inventoryStore.transactions.length === 0">
...
function calculateWarehouseStats(items: InventoryItem[]) {
...
const displayedOutOfStockItems = computed(() => outOfStockExpanded.value ? inventoryStore.outOfStockItems : inventoryStore.outOfStockItems.slice(0, 5))
const displayedCriticalStockItems = computed(() => criticalStockExpanded.value ? inventoryStore.criticalStockItems : inventoryStore.criticalStockItems.slice(0, 5))
const displayedLowStockItems = computed(() => lowStockExpanded.value ? inventoryStore.lowStockItems : inventoryStore.lowStockItems.slice(0, 5))
...
const warehouseStats = computed(() => {
  ...
  const stats = calculateWarehouseStats(items)
...
// ✅ Watch for filter changes in the store and update dashboard
watch(
  () => inventoryStore.currentFilters.warehouseId,
  (newVal) => {
    if (selectedWarehouseId.value !== newVal) {
...
onActivated(async () => {
  if (authStore.isAuthenticated) {
    ...
    if (tenantId) {
      ...
      if (dataLoaded.value && inventoryStore.items.length > 0) {
        await inventoryStore.fetchTransactions(1, 10)
        await inventoryStore.fetchSummaryStats({
