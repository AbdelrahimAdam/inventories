        ...
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              ...
              <span v-else-if="!inventoryStore.dataLoaded && !showSkeleton">(جاري التحميل...)</span>
            </p>
          </div>
...
// ✅ Use store's dataLoaded flag
const showSkeleton = computed(() => {
  return !dataInitialized.value && !inventoryStore.dataLoaded
})

// ✅ Computed properties using store data directly
const summaryStats = computed(() => inventoryStore.summaryStats)

// ✅ Watch for dataLoaded changes to update dataInitialized
watch(() => inventoryStore.dataLoaded, (loaded) => {
  if (loaded && !dataInitialized.value) {
...
// ✅ Use store's currentFilters for warehouse filtering
...
// ✅ Only load what's needed - the store handles the heavy lifting
const loadDashboardData = async () => {
  if (!authStore.currentTenantId) return
  
  // ✅ Check if store already has data - if so, use it
  ...
  try {
    // ✅ Load warehouses first (they're fast)
    await warehouseStore.fetchWarehouses()
    
    // ✅ Try loading from cache first (instant)
    const tenantId = authStore.currentTenantId
    if (tenantId) {
      const hasCache = inventoryStore.loadFromCache(tenantId)
      if (hasCache) {
        dataInitialized.value = true
        return
      }
    ...
    // ✅ Only fetch items if cache is empty or expired
    await inventoryStore.fetchItems()
    
    // ✅ Fetch transactions (separate from items for better UX)
    await inventoryStore.fetchTransactions(1, 10)
    
    dataInitialized.value = true
  } catch (error) {
    console.error('Failed to load dashboard data:', error)
    // ✅ Even on error, mark as initialized to show empty state
    dataInitialized.value = true
  }
}
...
const refreshData = async () => {
  ...
  try {
    // ✅ Only refresh the specific data that might have changed
    const warehouseId = selectedWarehouseId.value || undefined
    
    // ✅ Refresh summary stats (lightweight - only quantity data)
    await inventoryStore.fetchSummaryStats({ warehouseId })
    
    // ✅ Refresh recent transactions (lightweight - only last 10)
    await inventoryStore.fetchTransactions(1, 10)
    
    // ✅ Check if any items were updated (using version check)
    const tenantId = authStore.currentTenantId
    if (tenantId) {
      const hasChanged = await inventoryStore.hasDataChanged(tenantId)
      if (hasChanged) {
        // ✅ Only fetch items if data has changed
        await inventoryStore.fetchItems()
      }
    }
    
    // ✅ Refresh warehouse data (rarely changes)
    await warehouseStore.fetchWarehouses()
...
// ✅ Watch for filter changes and update only the affected data
const onWarehouseFilterChange = async () => {
  const warehouseId = selectedWarehouseId.value || undefined
  
  // ✅ Only update stats and transactions (not all items)
...
// ✅ Optimized onActivated - use store data if available
onActivated(async () => {
  // ✅ If data is already initialized and store has data, just refresh stats
