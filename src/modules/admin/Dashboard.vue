<template>
  <div class="w-full pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <!-- Loading Progress - Only show when actively loading -->
      ...
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          ...
          <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
            <div class="bg-gradient-to-r from-amber-500 to-amber-600 h-1.5 rounded-full transition-all duration-500" :style="{ width: loadingProgress + '%' }"></div>
          </div>
...
// ===== OPTIMIZED: Load data in parallel with progress =====
...
async function loadDashboardData(force = false) {
  ...
  if (dataAvailable && !force && !inventoryStore.hasDataChanged(tenantId)) {
  ...
  try {
    ...
    // Fetch items, stats, and transactions in parallel
    const [itemsResult, statsResult, transactionsResult] = await Promise.all([
...
onActivated(async () => {
  if (authStore.isAuthenticated) {
    ...
    if (tenantId) {
      ...
      // Check if data exists and hasn't changed
      const dataAvailable = inventoryStore.isDataAvailable(tenantId)
      if (dataAvailable && !inventoryStore.hasDataChanged(tenantId)) {
        await inventoryStore.fetchSummaryStats({
          warehouseId: inventoryStore.currentFilters.warehouseId || undefined
        })
        dataInitialized.value = true
