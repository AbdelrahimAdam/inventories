  ...
  <div class="w-full px-3 sm:px-6 py-4 sm:py-8" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl p-5 sm:p-6 mb-6 border border-gray-200 dark:border-gray-700 shadow-md">
      <div class="flex flex-col gap-4">
        ...
        <!-- Warehouse Dropdown + Buttons - stacked on mobile, inline on desktop -->
        <div class="flex flex-col sm:flex-row gap-3">
          <select v-model="inventoryStore.currentFilters.warehouseId" class="w-full sm:w-auto px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl text-sm font-medium bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500">
          ...
          <div class="flex gap-2 flex-wrap">
            <button @click="openGlobalTransferModal" class="flex-1 sm:flex-none px-4 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl transition-all flex items-center justify-center gap-2 shadow-md font-bold text-sm">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
              <span>نقل</span>
            </button>
            <button @click="openGlobalDispatchModal" class="flex-1 sm:flex-none px-4 py-2.5 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-xl transition-all flex items-center justify-center gap-2 shadow-md font-bold text-sm">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
              <span>صرف</span>
            </button>
            <router-link to="/inventory/items/new" class="flex-1 sm:flex-none px-4 py-2.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-xl transition-all flex items-center justify-center gap-2 shadow-md font-bold text-sm">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
              <span>إضافة صنف</span>
            </router-link>
            <button @click="refreshData" :disabled="isRefreshing" class="flex-1 sm:flex-none px-4 py-2.5 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white rounded-xl transition-all flex items-center justify-center gap-2 shadow-md font-bold disabled:opacity-50 text-sm">
    ...
    <!-- Stats Cards (Gradient) - responsive numbers -->
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6">
      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-4 sm:p-5 text-white">
        ...
        <p class="text-2xl sm:text-3xl lg:text-4xl font-black break-words">{{ formatNumber(inventoryStore.summaryStats.totalItems) }}</p>
      </div>
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-4 sm:p-5 text-white">
        ...
        <p class="text-2xl sm:text-3xl lg:text-4xl font-black break-words">{{ formatNumber(inventoryStore.summaryStats.totalQuantity) }}</p>
      </div>
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-4 sm:p-5 text-white">
        <p class="text-yellow-100 text-xs sm:text-sm font-bold">مخزون منخفض</p>
        <p class="text-2xl sm:text-3xl lg:text-4xl font-black break-words">{{ formatNumber(inventoryStore.summaryStats.lowStock) }}</p>
      </div>
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-4 sm:p-5 text-white">
        ...
        <p class="text-2xl sm:text-3xl lg:text-4xl font-black break-words">{{ formatNumber(inventoryStore.summaryStats.outOfStock) }}</p>
      </div>
    </div>

    <!-- Inventory Status Chart - Moved right under cards -->
      ...
      <div v-else>
        <div class="mb-6">
          <div class="flex h-8 rounded-full overflow-hidden shadow-sm">
            <div class="bg-green-500 h-full flex items-center justify-center text-white text-[10px] sm:text-xs font-extrabold transition-all duration-500" :style="{ width: inStockNum + '%' }" v-if="inStockNum > 5">{{ inStockNum.toFixed(0) }}%</div>
            <div class="bg-orange-500 h-full flex items-center justify-center text-white text-[10px] sm:text-xs font-extrabold transition-all duration-500" :style="{ width: criticalStockNum + '%' }" v-if="criticalStockNum > 5">{{ criticalStockNum.toFixed(0) }}%</div>
            <div class="bg-yellow-500 h-full flex items-center justify-center text-white text-[10px] sm:text-xs font-extrabold transition-all duration-500" :style="{ width: lowStockNum + '%' }" v-if="lowStockNum > 5">{{ lowStockNum.toFixed(0) }}%</div>
            <div class="bg-red-500 h-full flex items-center justify-center text-white text-[10px] sm:text-xs font-extrabold transition-all duration-500" :style="{ width: outOfStockNum + '%' }" v-if="outOfStockNum > 5">{{ outOfStockNum.toFixed(0) }}%</div>
          </div>
          <div class="flex mt-3 text-xs sm:text-sm font-semibold">
            <div class="flex-1 text-center"><span class="inline-block w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-green-500 ml-1"></span>متوفر</div>
            <div class="flex-1 text-center"><span class="inline-block w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-orange-500 ml-1"></span>حرج (≤250)</div>
            <div class="flex-1 text-center"><span class="inline-block w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-yellow-500 ml-1"></span>منخفض (≤500)</div>
            <div class="flex-1 text-center"><span class="inline-block w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-red-500 ml-1"></span>نفد</div>
          </div>
        </div>
              ...
              <tr v-for="wh in warehouseStats" :key="wh.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                ...
                <td class="px-3 sm:px-4 py-3 text-center">
                  <div class="flex items-center justify-end gap-2" :class="languageStore.isRTL ? 'flex-row-reverse' : ''">
                    <span class="text-xs sm:text-sm font-bold min-w-[45px] text-right">{{ wh.levelPercentage }}%</span>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md p-5 mb-8">
      ...
      <div v-else class="space-y-4 max-h-[500px] overflow-y-auto">
        ...
        <div v-if="outOfStockItemsList.length > 0" class="p-4 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10">
          ...
          <p class="text-xs sm:text-sm text-red-700 dark:text-red-400 mb-2">{{ outOfStockItemsList.length }} صنف قد نفد بالكامل</p>
          <div class="overflow-x-auto">
        ...
        <div v-if="criticalStockItemsList.length > 0" class="p-4 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10">
          ...
          <p class="text-xs sm:text-sm text-orange-700 dark:text-orange-400 mb-2">{{ criticalStockItemsList.length }} صنف بمستوى حرج (≤250 وحدة)</p>
          <div class="overflow-x-auto">
            <table class="w-full text-xs">
        ...
        <div v-if="lowStockItemsList.length > 0" class="p-4 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10">
          ...
          <p class="text-xs sm:text-sm text-yellow-700 dark:text-yellow-400 mb-2">{{ lowStockItemsList.length }} صنف بمستوى منخفض (51-500 وحدة)</p>
          <div class="overflow-x-auto">
            <table class="w-full text-xs">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md overflow-hidden">
      <div class="p-4 sm:p-5 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center flex-wrap gap-2">
          ...
          <router-link to="/inventory/transactions" class="text-xs sm:text-sm font-bold text-amber-600 dark:text-amber-400 hover:text-amber-700 transition-colors">عرض الكل ←</router-link>
        </div>
      </div>
      <div class="overflow-x-auto">
...
// Computed - ONLY PRIMARY WAREHOUSES (no dispatch)
...
// Warehouse stats using ALL items
