<template>
  <div class="w-full" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    <!-- Loading Skeleton -->
    <div v-if="isLoadingInitial" class="space-y-4">
      <div class="bg-white dark:bg-gray-800 rounded-xl p-4 sm:p-5 border border-gray-200 dark:border-gray-700 shadow-sm">
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
        <div v-for="i in 4" :key="i" class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-3 sm:p-4 border border-gray-200 dark:border-gray-700">
    ...
    <template v-else>
      ...
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
          ...
          <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
            {{ formatNumber(summaryStats.totalItems) }}
          </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
          ...
          <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
            {{ formatNumber(summaryStats.lowStock) }}
          </div>
        </div>
        <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
          ...
          <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
            {{ formatNumber(summaryStats.outOfStock) }}
          </div>
        </div>
      </div>
          ...
          <div class="max-h-[350px] overflow-y-auto">
            <table class="w-full min-w-[600px]">
              ...
              <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tr v-if="warehouseStats.length === 0 && !isLoadingInitial">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
        ...
        <div class="space-y-3 max-h-[450px] overflow-y-auto">
          ...
          <div v-if="criticalStockItemsList.length === 0 && lowStockItemsList.length === 0 && outOfStockItemsList.length === 0 && !isLoadingInitial" class="text-center py-6 text-gray-500 font-bold">
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[550px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-if="recentTransactions.length === 0 && !isLoadingInitial">
...
// ============================================================
// COMPUTED - Summary Stats (from store)
// ============================================================
const summaryStats = computed(() => inventoryStore.summaryStats)

// ============================================================
// COMPUTED - User & Warehouse
// ============================================================
...
// ============================================================
// COMPUTED - Alert Items (Optimized with caching)
// ============================================================
...
// ============================================================
// COMPUTED - Warehouse Stats (Optimized)
// ============================================================
...
// ============================================================
// DATA LOADING - Optimized with progressive loading
// ============================================================
const loadDashboardData = async (isInitial = true) => {
  if (isInitial) isLoadingInitial.value = true
  
  try {
    const warehouseId = selectedWarehouseId.value || undefined
    
    // Load warehouses first (lightweight)
    if (warehouseStore.warehouses.length === 0) {
      await warehouseStore.fetchWarehouses()
    }
    
    // Load summary stats (lightweight)
    await inventoryStore.fetchSummaryStats({ warehouseId })
    
    // Load initial items (50 items)
    ...
    // Load recent transactions (10 items)
    if (inventoryStore.transactions.length === 0) {
      await inventoryStore.fetchTransactions(1, 10, false)
    }
    
    // Load all items in background for alerts (non-blocking)
    if (!isAllItemsLoaded.value && !isLoadingAllItems.value) {
    ...
    if (isInitial) isLoadingInitial.value = false
  }
}

const loadAllItemsForAlerts = async () => {
  if (isLoadingAllItems.value || isAllItemsLoaded.value) return
...
const refreshData = async () => {
  ...
  try {
    ...
    await Promise.all([
      ...
      inventoryStore.fetchTransactions(1, 10, true),
...
const onWarehouseFilterChange = async () => {
  ...
  // Reset all-items state when warehouse changes
  isAllItemsLoaded.value = false
...
onActivated(async () => {
  // Only reload if data is stale (could add timestamp check)
