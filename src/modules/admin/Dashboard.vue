<template>
  <div class="w-full px-2 sm:px-4 py-4 sm:py-8" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
      ...
      <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div class="flex flex-col items-end">
          ...
          <div class="flex gap-2 mt-2">
            <button @click="requestUpgrade" :disabled="upgradeRequestSent" class="px-4 py-1.5 bg-gradient-to-r from-amber-600 to-amber-700 text-white rounded-lg text-sm font-bold hover:from-amber-700 hover:to-amber-800 transition-all shadow-md disabled:opacity-50">
              {{ upgradeRequestSent ? 'تم إرسال الطلب' : 'طلب ترقية' }}
            </button>
            <button @click="contactSales" class="px-4 py-1.5 border border-amber-600 text-amber-700 dark:text-amber-400 rounded-lg text-sm font-bold hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-all">المبيعات</button>
          </div>
        </div>
      </div>
      <div class="mt-3">
    ...
    <!-- Header Section -->
    <div class="bg-white dark:bg-gray-800 rounded-xl p-6 mb-6 border border-gray-200 dark:border-gray-700 shadow-md">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 class="text-3xl sm:text-4xl font-black tracking-tight text-gray-900 dark:text-white">مرحباً {{ userName }}</h1>
          <p class="text-gray-600 dark:text-gray-400 mt-1 font-medium">مرحباً بعودتك! إليك ملخص المخزون اليوم.</p>
        </div>
        <div class="flex gap-2 flex-wrap">
          <select v-model="inventoryStore.currentFilters.warehouseId" class="px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-xl text-sm font-medium bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500">
            <option value="">جميع المخازن</option>
            <option v-for="warehouse in accessibleWarehouses" :key="warehouse.id" :value="warehouse.id">
              {{ warehouse.name_ar || warehouse.name }}
            </option>
          </select>
          <button @click="openGlobalTransferModal" class="px-5 py-2.5 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white rounded-xl transition-all flex items-center gap-2 shadow-md font-bold">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7h12m0 0l-4-4m4 4l-4 4m0 6H4m0 0l4 4m-4-4l4-4" /></svg>
            <span class="hidden sm:inline">نقل</span>
          </button>
          <button @click="openGlobalDispatchModal" class="px-5 py-2.5 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white rounded-xl transition-all flex items-center gap-2 shadow-md font-bold">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
            <span class="hidden sm:inline">صرف</span>
          </button>
          <router-link to="/inventory/items/new" class="px-5 py-2.5 bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white rounded-xl transition-all flex items-center gap-2 shadow-md font-bold">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" /></svg>
            <span class="hidden sm:inline">إضافة صنف</span>
          </router-link>
          <button @click="refreshData" :disabled="isRefreshing" class="px-5 py-2.5 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white rounded-xl transition-all flex items-center gap-2 shadow-md font-bold disabled:opacity-50">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-8">
      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-5 text-white">
        <p class="text-blue-100 text-sm font-bold">إجمالي الأصناف</p>
        <p class="text-3xl sm:text-4xl font-black">{{ formatNumber(inventoryStore.summaryStats.totalItems) }}</p>
      </div>
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-5 text-white">
        <p class="text-green-100 text-sm font-bold">إجمالي الوحدات</p>
        <p class="text-3xl sm:text-4xl font-black">{{ formatNumber(inventoryStore.summaryStats.totalQuantity) }}</p>
      </div>
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-5 text-white">
        <p class="text-yellow-100 text-sm font-bold">مخزون منخفض</p>
        <p class="text-3xl sm:text-4xl font-black">{{ formatNumber(inventoryStore.summaryStats.lowStock) }}</p>
      </div>
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-5 text-white">
        <p class="text-red-100 text-sm font-bold">نفد المخزون</p>
        <p class="text-3xl sm:text-4xl font-black">{{ formatNumber(inventoryStore.summaryStats.outOfStock) }}</p>
      </div>
    </div>

    <!-- Warehouse Distribution (fixed logic – uses all items) -->
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md overflow-hidden mb-8">
      <div class="p-5 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center">
          <div>
            <h2 class="text-xl font-black text-gray-900 dark:text-white">توزيع المخزون في المخازن</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">حسب جميع الأصناف (بدون تقسيم صفحات)</p>
          </div>
          <span class="text-sm font-bold bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded-full">{{ warehouseStats.length }} مخزن</span>
        </div>
      </div>
          ...
          <table class="w-full min-w-[700px]">
            <thead class="sticky top-0 z-10 bg-gray-50 dark:bg-gray-700">
              <tr class="border-b border-gray-200 dark:border-gray-600">
                <th class="px-4 py-3 text-center text-sm font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">المخزن</th>
                <th class="px-4 py-3 text-center text-sm font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">الأصناف</th>
                <th class="px-4 py-3 text-center text-sm font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">الوحدات</th>
                <th class="px-4 py-3 text-center text-sm font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">مخزون منخفض</th>
                <th class="px-4 py-3 text-center text-sm font-extrabold text-gray-700 dark:text-gray-200 uppercase tracking-wider">المستوى</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="wh in warehouseStats" :key="wh.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                <td class="px-4 py-3 text-center">
                  <div class="font-black text-gray-900 dark:text-white">{{ wh.name }}</div>
                  <div class="text-xs text-gray-500 dark:text-gray-400">{{ wh.location || 'لا يوجد موقع' }}</div>
                </td>
                <td class="px-4 py-3 text-center text-base font-bold text-gray-700 dark:text-gray-300">{{ formatNumber(wh.itemCount) }}</td>
                <td class="px-4 py-3 text-center text-base font-black text-gray-900 dark:text-white">{{ formatNumber(wh.totalUnits) }}</td>
                <td class="px-4 py-3 text-center"><span :class="wh.lowStockCount > 0 ? 'text-yellow-600 dark:text-yellow-400 font-bold' : 'text-gray-500'">{{ formatNumber(wh.lowStockCount) }}</span></td>
                <td class="px-4 py-3 text-center">
                  <div class="flex items-center justify-end gap-2" :class="languageStore.isRTL ? 'flex-row-reverse' : ''">
                    <span class="text-sm font-bold min-w-[50px] text-right">{{ wh.levelPercentage }}%</span>
                    <div class="w-28 bg-gray-200 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md p-5">
        <h3 class="text-xl font-black text-gray-900 dark:text-white mb-4">توزيع حالة المخزون</h3>
        ...
        <div v-else>
          <div class="mb-6">
            <div class="flex h-8 rounded-full overflow-hidden shadow-sm">
              <div class="bg-green-500 h-full flex items-center justify-center text-white text-xs font-extrabold transition-all duration-500" :style="{ width: inStockNum + '%' }" v-if="inStockNum > 5">{{ inStockNum.toFixed(1) }}%</div>
              <div class="bg-orange-500 h-full flex items-center justify-center text-white text-xs font-extrabold transition-all duration-500" :style="{ width: criticalStockNum + '%' }" v-if="criticalStockNum > 5">{{ criticalStockNum.toFixed(1) }}%</div>
              <div class="bg-yellow-500 h-full flex items-center justify-center text-white text-xs font-extrabold transition-all duration-500" :style="{ width: lowStockNum + '%' }" v-if="lowStockNum > 5">{{ lowStockNum.toFixed(1) }}%</div>
              <div class="bg-red-500 h-full flex items-center justify-center text-white text-xs font-extrabold transition-all duration-500" :style="{ width: outOfStockNum + '%' }" v-if="outOfStockNum > 5">{{ outOfStockNum.toFixed(1) }}%</div>
            </div>
            <div class="flex mt-3 text-sm font-semibold">
              <div class="flex-1 text-center"><span class="inline-block w-3 h-3 rounded-full bg-green-500 ml-1"></span>متوفر</div>
              <div class="flex-1 text-center"><span class="inline-block w-3 h-3 rounded-full bg-orange-500 ml-1"></span>حرج (≤250)</div>
              <div class="flex-1 text-center"><span class="inline-block w-3 h-3 rounded-full bg-yellow-500 ml-1"></span>منخفض (≤500)</div>
              <div class="flex-1 text-center"><span class="inline-block w-3 h-3 rounded-full bg-red-500 ml-1"></span>نفد</div>
            </div>
          </div>
          <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div class="flex flex-col items-center">
              <div class="text-3xl font-black text-green-600 dark:text-green-400">{{ inStockNum.toFixed(0) }}%</div>
              <p class="text-sm font-bold text-gray-700 dark:text-gray-300">متوفر</p>
            </div>
            <div class="flex flex-col items-center">
              <div class="text-3xl font-black text-orange-600 dark:text-orange-400">{{ criticalStockNum.toFixed(0) }}%</div>
              <p class="text-sm font-bold text-gray-700 dark:text-gray-300">حرج</p>
            </div>
            <div class="flex flex-col items-center">
              <div class="text-3xl font-black text-yellow-600 dark:text-yellow-400">{{ lowStockNum.toFixed(0) }}%</div>
              <p class="text-sm font-bold text-gray-700 dark:text-gray-300">منخفض</p>
            </div>
            <div class="flex flex-col items-center">
              <div class="text-3xl font-black text-red-600 dark:text-red-400">{{ outOfStockNum.toFixed(0) }}%</div>
              <p class="text-sm font-bold text-gray-700 dark:text-gray-300">نفد</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md p-5">
        <h3 class="text-xl font-black text-gray-900 dark:text-white mb-4">التنبيهات الأخيرة</h3>
        ...
        <div v-else class="space-y-4 max-h-[500px] overflow-y-auto">
          ...
          <div v-if="outOfStockItemsList.length > 0" class="p-4 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10">
            <div class="flex justify-between items-start mb-2">
              <p class="text-base font-black text-red-800 dark:text-red-300">❌ نفاد المخزون</p>
              <span class="text-xs font-bold text-red-500">الآن</span>
            </div>
            <p class="text-sm text-red-700 dark:text-red-400 mb-2">{{ outOfStockItemsList.length }} صنف قد نفد بالكامل</p>
            <div class="overflow-x-auto">
              <table class="w-full text-xs">
                <thead class="border-b border-red-200 dark:border-red-800"><tr><th class="py-1 text-right font-bold">الصنف</th><th class="py-1 text-center font-bold">الكود</th><th class="py-1 text-center font-bold">المخزن</th><th class="py-1 text-center font-bold">الكمية</th></tr></thead>
                <tbody>
                  <tr v-for="item in displayedOutOfStockItems" :key="item.id">
                    <td class="py-1 text-right font-medium">{{ item.name }}</td>
                    <td class="py-1 text-center">{{ item.code }}</td>
                    <td class="py-1 text-center">{{ getWarehouseName(item.warehouseId) }}</td>
                    <td class="py-1 text-center font-black text-red-600">{{ formatNumber(item.remainingQuantity) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div class="mt-2 text-center"><button @click="toggleShow('out')" class="text-xs font-bold text-red-500 hover:underline">{{ outOfStockExpanded ? 'عرض أقل' : `عرض الكل (${outOfStockItemsList.length})` }}</button></div>
          </div>
          ...
          <div v-if="criticalStockItemsList.length > 0" class="p-4 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10">
            <div class="flex justify-between items-start mb-2">
              <p class="text-base font-black text-orange-800 dark:text-orange-300">⚠️ مخزون حرج</p>
              <span class="text-xs font-bold text-orange-500">الآن</span>
            </div>
            <p class="text-sm text-orange-700 dark:text-orange-400 mb-2">{{ criticalStockItemsList.length }} صنف بمستوى حرج (≤250 وحدة)</p>
            <div class="overflow-x-auto">
              <table class="w-full text-xs">
                <thead class="border-b border-orange-200 dark:border-orange-800"><tr><th class="py-1 text-right font-bold">الصنف</th><th class="py-1 text-center font-bold">الكود</th><th class="py-1 text-center font-bold">المخزن</th><th class="py-1 text-center font-bold">الكمية</th></tr></thead>
                <tbody>
                  <tr v-for="item in displayedCriticalStockItems" :key="item.id">
                    <td class="py-1 text-right font-medium">{{ item.name }}</td>
                    <td class="py-1 text-center">{{ item.code }}</td>
                    <td class="py-1 text-center">{{ getWarehouseName(item.warehouseId) }}</td>
                    <td class="py-1 text-center font-black text-orange-600">{{ formatNumber(item.remainingQuantity) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div class="mt-2 text-center"><button @click="toggleShow('critical')" class="text-xs font-bold text-orange-500 hover:underline">{{ criticalStockExpanded ? 'عرض أقل' : `عرض الكل (${criticalStockItemsList.length})` }}</button></div>
          </div>

          <!-- Low Stock (1-500 but >250 is low? Actually low stock from summary stats is ≤500 and >0; but we already show critical separately. To avoid duplication, we show low stock as 251-500) -->
          <div v-if="lowStockItemsList.length > 0" class="p-4 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10">
            <div class="flex justify-between items-start mb-2">
              <p class="text-base font-black text-yellow-800 dark:text-yellow-300">⚠️ مخزون منخفض</p>
              <span class="text-xs font-bold text-yellow-500">الآن</span>
            </div>
            <p class="text-sm text-yellow-700 dark:text-yellow-400 mb-2">{{ lowStockItemsList.length }} صنف بمستوى منخفض (51-500 وحدة)</p>
            <div class="overflow-x-auto">
              <table class="w-full text-xs">
                <thead class="border-b border-yellow-200 dark:border-yellow-800"><tr><th class="py-1 text-right font-bold">الصنف</th><th class="py-1 text-center font-bold">الكود</th><th class="py-1 text-center font-bold">المخزن</th><th class="py-1 text-center font-bold">الكمية</th></tr></thead>
                <tbody>
                  <tr v-for="item in displayedLowStockItems" :key="item.id">
                    <td class="py-1 text-right font-medium">{{ item.name }}</td>
                    <td class="py-1 text-center">{{ item.code }}</td>
                    <td class="py-1 text-center">{{ getWarehouseName(item.warehouseId) }}</td>
                    <td class="py-1 text-center font-black text-yellow-600">{{ formatNumber(item.remainingQuantity) }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <div class="mt-2 text-center"><button @click="toggleShow('low')" class="text-xs font-bold text-yellow-500 hover:underline">{{ lowStockExpanded ? 'عرض أقل' : `عرض الكل (${lowStockItemsList.length})` }}</button></div>
          </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-md overflow-hidden">
      <div class="p-5 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center">
          <div>
            <h2 class="text-xl font-black text-gray-900 dark:text-white">آخر المعاملات</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">أحدث حركات المخزون</p>
          </div>
          <router-link to="/inventory/transactions" class="text-sm font-bold text-amber-600 dark:text-amber-400 hover:text-amber-700 transition-colors">عرض الكل ←</router-link>
        </div>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full min-w-[600px]">
          <thead class="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
            <tr>
              <th class="px-5 py-3 text-center text-sm font-extrabold text-gray-600 dark:text-gray-300 uppercase tracking-wider">التاريخ</th>
              <th class="px-5 py-3 text-center text-sm font-extrabold text-gray-600 dark:text-gray-300 uppercase tracking-wider">النوع</th>
              <th class="px-5 py-3 text-center text-sm font-extrabold text-gray-600 dark:text-gray-300 uppercase tracking-wider">الصنف</th>
              <th class="px-5 py-3 text-center text-sm font-extrabold text-gray-600 dark:text-gray-300 uppercase tracking-wider">الكمية</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="tx in recentTransactions" :key="tx.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              <td class="px-5 py-3 text-center text-base text-gray-600 dark:text-gray-400 font-medium">{{ formatDate(tx.createdAt) }}</td>
              <td class="px-5 py-3 text-center"><span :class="getTypeBadge(tx.type)" class="px-3 py-1 text-sm rounded-full font-black">{{ getTypeText(tx.type) }}</span></td>
              <td class="px-5 py-3 text-center text-base font-bold text-gray-900 dark:text-white">{{ tx.itemName }}</td>
              <td class="px-5 py-3 text-center text-base font-extrabold" :class="getQuantityClass(tx.totalDelta)">{{ formatDelta(tx.totalDelta) }}</td>
            </tr>
...
// Alert items categorised correctly (critical ≤250, low 251-500, out=0)
...
// Warehouse stats using ALL items (by querying without pagination)
...
async function loadFullWarehouseStats() {
  try {
    // Fetch all items without pagination (respect filters)
    ...
    // Find max totalUnits for scaling (optional)
    let maxUnits = 1
