...
const displayStats = computed(() => {
  ...
  if (inventoryStore.items.length > 0) {
    return calculateStats(inventoryStore.items)
  }
...
async function loadDashboardData(force = false) {
  ...
  if (inventoryStore.dataLoaded && !force) {
  ...
  try {
    ...
    // Load summary stats and transactions in parallel
    await Promise.all([
