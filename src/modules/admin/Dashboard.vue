...
// ✅ OPTIMIZED: Parallel loading for faster dashboard
const loadDashboardData = async () => {
  if (!authStore.currentTenantId) return
  
  const warehouseId = selectedWarehouseId.value || undefined
  
  // If data already loaded, just refresh stats
  if (inventoryStore.dataLoaded && inventoryStore.items.length > 0 && !isRefreshing.value) {
  ...
  // Start fetching transactions if empty
  ...
  // ✅ Fetch stats (depends on items data)
  await inventoryStore.fetchSummaryStats({ warehouseId })
  
  // ✅ Load alerts in background - doesn't block UI
...
// ✅ OPTIMIZED: Parallel loading for refresh
