<template>
  <div class="w-full pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
      ...
      <div v-if="isLoadingStats" class="animate-pulse space-y-4">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden mb-4">
      ...
      <div class="overflow-x-auto">
        <div class="max-h-[350px] overflow-y-auto">
          <div v-if="isLoadingStats" class="p-4 text-center text-gray-500">جاري تحميل البيانات...</div>
          <table v-else class="w-full min-w-[600px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-if="warehouseStats.length === 0 && !isLoadingStats">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
      ...
      <div v-else class="space-y-3 max-h-[450px] overflow-y-auto">
        ...
        <div v-if="criticalStockItemsList.length === 0 && lowStockItemsList.length === 0 && outOfStockItemsList.length === 0 && !isLoadingStats" class="text-center py-6 text-gray-500 font-bold">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[550px]">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-if="isLoadingTransactions" class="animate-pulse">
            ...
            <tr v-if="!isLoadingTransactions && recentTransactions.length === 0">
...
const outOfStockItemsList = computed(() => {
  return getFilteredItems().filter(item => item.remainingQuantity === 0)
})

const criticalStockItemsList = computed(() => {
  return getFilteredItems().filter(item => item.remainingQuantity > 0 && item.remainingQuantity <= 250)
})

const lowStockItemsList = computed(() => {
  return getFilteredItems().filter(item => item.remainingQuantity > 250 && item.remainingQuantity <= 500)
})
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    if (inventoryStore.items.length === 0) {
      await inventoryStore.fetchItemsBulk()
    }
...
const refreshData = async () => {
  ...
  try {
    ...
    await inventoryStore.fetchItemsBulk(warehouseId)
...
const onWarehouseFilterChange = async () => {
  ...
  try {
    ...
    console.error('Warehouse filter change failed:', error)
  } finally {
    isLoadingStats.value = false
  }
}
...
onActivated(async () => {
  if (dataInitialized.value && inventoryStore.items.length > 0) {
    const warehouseId = selectedWarehouseId.value || undefined
