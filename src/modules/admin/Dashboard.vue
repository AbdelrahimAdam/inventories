...
// ✅ calculateWarehouseStats - used for fallback
...
// Use dashboardStats for display
...
// Use dashboardAlerts for alert items
...
// Use dashboardStats for warehouse distribution
...
// Load dashboard data using optimized functions
async function loadDashboardData(force = false) {
  const tenantId = authStore.currentTenantId
  if (!tenantId) return

  if (dataInitialized.value && !force) {
  ...
  try {
    ...
    // Load stats from database (fast, no items)
    ...
    // Load alerts (limited to 100 items)
    ...
    // Only load items if the dashboard actually needs them
    // The dashboard's chart uses inventoryStore.items
    // If items are already in cache, use them
    if (inventoryStore.items.length === 0) {
      // Load a small preview (50 items) for the chart
...
// Refresh stats without reloading everything
