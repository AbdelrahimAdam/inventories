      ...
      <div v-else class="space-y-3 max-h-[450px] overflow-y-auto">
        <div v-if="outOfStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10">
          ...
          <div class="mt-2 text-center">
            <button @click="toggleShow('out')" class="text-xs font-bold text-red-500 hover:underline min-h-[44px] px-2">
        ...
        <div v-if="criticalStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10">
          ...
          <div class="mt-2 text-center">
            <button @click="toggleShow('critical')" class="text-xs font-bold text-orange-500 hover:underline min-h-[44px] px-2">
        ...
        <div v-if="lowStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10">
          ...
          <div class="mt-2 text-center">
            <button @click="toggleShow('low')" class="text-xs font-bold text-yellow-500 hover:underline min-h-[44px] px-2">
...
// ✅ Load with cache first, then refresh in background
async function loadDashboardData() {
  ...
  try {
    ...
    if (cacheResult.loaded && cacheResult.isComplete) {
      ...
      // ✅ No cache or incomplete - load full data
...
// ✅ Background refresh - uses fetchItems() not fetchItemsPage()
async function refreshDataInBackground() {
  if (inventoryStore.isLoading) return
  
  try {
    // ✅ Use fetchItems() which checks cache completeness
    await inventoryStore.fetchItems()
...
// ✅ Refresh - uses fetchItems() with force
async function refreshData() {
  if (inventoryStore.isLoading) return
  
  try {
    // ✅ Use forceRefreshAll to clear cache and reload
...
// ✅ Filter change - only update transactions, stats from computed
async function onWarehouseFilterChange() {
  const warehouseId = selectedWarehouseId.value || undefined
  // ✅ Only fetch transactions for the selected warehouse
  // Stats will update automatically via computed properties
  await inventoryStore.fetchTransactions(1, 10)
}

// ✅ Success handlers - only refresh summary stats, not full reload
...
onActivated(async () => {
  if (dataInitialized.value && inventoryStore.items.length > 0) {
    const warehouseId = selectedWarehouseId.value || undefined
    // ✅ Only update stats, no full reload
    await inventoryStore.fetchSummaryStats({ warehouseId })
    scrollToTop()
    return
  }
