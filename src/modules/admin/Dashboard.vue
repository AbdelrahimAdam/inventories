<template>
  <div class="w-full pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <!-- Welcome Section -->
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md p-3 sm:p-4 mb-3 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
      <div class="flex flex-col gap-3">
        ...
        <div class="flex flex-col sm:flex-row gap-2">
          <select v-model="selectedWarehouseId" @change="onWarehouseFilterChange" class="w-full sm:w-auto px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-xl text-sm font-medium bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-amber-500 min-h-[44px] sm:min-h-[48px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
    ...
    <!-- Empty State -->
    <div v-if="!isLoadingData && inventoryStore.items.length === 0 && !dataInitialized" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md p-8 text-center">
      <svg class="w-16 h-16 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
      </svg>
      <h3 class="text-lg font-bold text-gray-700 dark:text-gray-300 mb-2">لا توجد بيانات</h3>
      <p class="text-gray-500 dark:text-gray-400 mb-4">لم يتم تحميل أي أصناف بعد. قم بزيارة صفحة الأصناف لتحميل البيانات.</p>
      <router-link to="/inventory/items" class="inline-block px-6 py-2 bg-gradient-to-r from-amber-500 to-amber-600 text-white rounded-lg font-bold hover:from-amber-600 hover:to-amber-700 transition-all shadow-md">
        الذهاب إلى الأصناف
      </router-link>
    </div>

    <!-- Stats Cards -->
    <div v-if="inventoryStore.items.length > 0 || dashboardStats.value.totalItems > 0" class="grid grid-cols-2 md:grid-cols-4 gap-1.5 sm:gap-2 mb-3">
    ...
    <div v-if="inventoryStore.items.length > 0" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md p-3 sm:p-4 mb-3 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
    ...
    <div v-if="inventoryStore.items.length > 0" class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md overflow-hidden mb-3 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
      <div class="p-3 sm:p-4 border-b border-gray-300 dark:border-gray-600 bg-gray-50/50 dark:bg-gray-700/30">
        <div class="flex justify-between items-center flex-wrap gap-2">
          ...
          <span class="text-[10px] sm:text-xs font-bold bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 px-2 py-0.5 rounded-full border border-amber-200 dark:border-amber-800">{{ warehouseStats.length }} مخزن</span>
        </div>
      </div>
      ...
      <div v-else-if="inventoryStore.items.length > 0" class="space-y-3 max-h-[450px] overflow-y-auto">
        <div v-if="outOfStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10 border border-red-200 dark:border-red-800 hover:shadow-md transition-shadow">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-[10px] sm:text-xs font-bold bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300 px-2 py-0.5 rounded-full border border-red-200 dark:border-red-800">{{ outOfStockItems.length }} صنف</span>
          </div>
        ...
        <div v-if="criticalStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10 border border-orange-200 dark:border-orange-800 hover:shadow-md transition-shadow">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-[10px] sm:text-xs font-bold bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300 px-2 py-0.5 rounded-full border border-orange-200 dark:border-orange-800">{{ criticalStockItems.length }} صنف</span>
          </div>
        ...
        <div v-if="lowStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800 hover:shadow-md transition-shadow">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-[10px] sm:text-xs font-bold bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300 px-2 py-0.5 rounded-full border border-yellow-200 dark:border-yellow-800">{{ lowStockItems.length }} صنف</span>
          </div>
...
const dashboardAlerts = ref({
  ...
  criticalStock: [] as InventoryItem[],
  lowStock: [] as InventoryItem[]
})

const displayStats = computed(() => {
  if (dashboardStats.value.totalItems > 0) {
...
const outOfStockItems = computed(() => {
  const items = dashboardAlerts.value.outOfStock
...
const criticalStockItems = computed(() => {
  const items = dashboardAlerts.value.criticalStock
  if (selectedWarehouseId.value) {
    return items.filter(item => item.warehouseId === selectedWarehouseId.value)
  }
  return items
...
const lowStockItems = computed(() => {
  const items = dashboardAlerts.value.lowStock
  if (selectedWarehouseId.value) {
    return items.filter(item => item.warehouseId === selectedWarehouseId.value)
  }
  return items
...
const warehouseStats = computed(() => {
  if (dashboardStats.value.warehouseStats.length > 0) {
    let stats = dashboardStats.value.warehouseStats
    
    if (selectedWarehouseId.value) {
      stats = stats.filter(w => w.id === selectedWarehouseId.value)
    }
    
    const maxUnits = Math.max(1, ...stats.map(w => w.totalUnits))
    return stats.map(w => ({
      ...w,
      levelPercentage: Math.min(Math.round((w.totalUnits / maxUnits) * 100), 100),
      levelColor: w.totalUnits / maxUnits < 0.2 ? 'bg-red-500' :
                  w.totalUnits / maxUnits < 0.5 ? 'bg-yellow-500' :
                  w.totalUnits / maxUnits < 0.8 ? 'bg-blue-500' : 'bg-green-600'
    }))
  }
...
function calculateStatsFromItems(items: InventoryItem[]) {
...
function getAlertsFromItems(items: InventoryItem[]) {
  return {
    outOfStock: items.filter(item => item.remainingQuantity === 0),
    criticalStock: items.filter(item => item.remainingQuantity > 0 && item.remainingQuantity <= 250),
    lowStock: items.filter(item => item.remainingQuantity > 250 && item.remainingQuantity <= 500)
  }
}

function calculateWarehouseStatsFromItems(items: InventoryItem[]) {
  ...
  for (const item of items) {
    ...
    const data = warehouseMap.get(wid)!
    data.itemCount++
    data.totalUnits += item.remainingQuantity || 0
    if (item.remainingQuantity > 0 && item.remainingQuantity <= 500) {
      data.lowStockCount++
    }
  }
...
watch(
  () => inventoryStore.loadingProgress,
  (progress) => {
    if (inventoryStore.isLoading) {
      loadingProgress.value = progress
      isLoadingData.value = true
    } else if (loadingProgress.value >= 100) {
...
watch(
  ...
  (count) => {
    if (count > 0 && inventoryStore.totalItems > 0) {
      const progress = Math.min((count / inventoryStore.totalItems) * 100, 100)
      loadingProgress.value = progress
    }
  }
)
...
async function loadDashboardData() {
  ...
  if (dataInitialized.value && dashboardStats.value.totalItems > 0) {
  ...
  try {
    ...
    if (inventoryStore.items.length > 0) {
      // Use cached items (FAST - no API calls)
      loadingProgress.value = 30
      const cachedStats = calculateStatsFromItems(inventoryStore.items)
      dashboardStats.value = {
        totalItems: cachedStats.totalItems,
        totalQuantity: cachedStats.totalQuantity,
        lowStock: cachedStats.lowStock,
        criticalStock: cachedStats.criticalStock,
        outOfStock: cachedStats.outOfStock,
        warehouseCount: 0,
        warehouseStats: []
      }
      
      const warehouseStatsData = calculateWarehouseStatsFromItems(inventoryStore.items)
      dashboardStats.value.warehouseStats = warehouseStatsData
      dashboardStats.value.warehouseCount = warehouseStatsData.length
      
      loadingProgress.value = 60
      
      const alerts = getAlertsFromItems(inventoryStore.items)
    ...
    // No cached items - show empty state
    // The user needs to visit the Item List page first
    loadingProgress.value = 100
    dataInitialized.value = true
...
async function refreshData() {
  ...
  try {
    ...
    const tenantId = authStore.currentTenantId
    if (tenantId) {
      inventoryStore.clearAllCache(tenantId)
    }
    
    // After clearing cache, we need to reload from the item list page
    // Show empty state with message
    inventoryStore.itemsMap.value.clear()
    dashboardStats.value = {
      totalItems: 0,
      totalQuantity: 0,
      lowStock: 0,
    ...
    dashboardAlerts.value = { outOfStock: [], criticalStock: [], lowStock: [] }
