...
// ============================================
// NEW: Dashboard-specific state (optimized)
// ============================================
...
// ============================================
// UPDATED: Use dashboardStats for display
// ============================================
const displayStats = computed(() => {
  // Use dashboard stats if available (faster)
  if (dashboardStats.value.totalItems > 0) {
    return dashboardStats.value
  }
  // Fallback to inventory store stats
...
// Warehouse name map for fast lookups
...
// ============================================
// UPDATED: Use dashboardAlerts for alert items
// ============================================
...
// ============================================
// UPDATED: Use dashboardStats for warehouse distribution
// ============================================
...
// Watch inventory store loading progress
watch(
...
// Watch inventory items count to show progress
watch(
...
// ============================================
// UPDATED: Optimized dashboard data loading
// ============================================
async function loadDashboardData(force = false) {
  ...
  if (inventoryStore.dataLoaded && !force && dataInitialized.value) {
    // Just refresh stats
  ...
  try {
    ...
    // STEP 1: Load dashboard stats (FAST - no items)
    ...
    // STEP 2: Load alert items only (limited to 100)
    ...
    // STEP 3: Load recent transactions (last 10)
    await inventoryStore.fetchTransactions(1, 10)
    loadingProgress.value = 80
    
    // STEP 4: Load items for the table (paginated - 50 items)
    await inventoryStore.fetchItemsPage(1, {
      pageSize: 50,
...
async function onWarehouseFilterChange() {
  // Reload stats and alerts for new warehouse
  await refreshDashboardStats()
  await inventoryStore.fetchTransactions(1, 10)
}
...
onActivated(async () => {
  if (authStore.isAuthenticated) {
    ...
    if (tenantId) {
      ...
      if (inventoryStore.dataLoaded && dataInitialized.value) {
        await refreshDashboardStats()
        scrollToTop()
        return
      }
