<template>
  <div class="w-full" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
      ...
      <div v-if="inventoryStore.isLoadingAllItems" class="animate-pulse space-y-4">
      ...
      <div class="p-3 sm:p-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">حسب جميع الأصناف (بدون تقسيم صفحات)</p>
          </div>
        ...
        <div class="max-h-[350px] overflow-y-auto">
          <table class="w-full min-w-[600px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-if="warehouseStats.length === 0 && !inventoryStore.isLoadingAllItems">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm p-4 sm:p-5 mb-4">
      ...
      <div v-if="inventoryStore.isLoadingAllItems" class="space-y-3">
      ...
      <div v-else class="space-y-3 max-h-[450px] overflow-y-auto">
        ...
        <div v-if="outOfStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10">
          ...
          <div class="mt-2 text-center"><button @click="toggleShow('out')" class="text-xs font-bold text-red-500 hover:underline min-h-[36px] px-2">{{ outOfStockExpanded ? 'عرض أقل' : `عرض الكل (${outOfStockItemsList.length})` }}</button></div>
        </div>
        ...
        <div v-if="criticalStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10">
          ...
          <div class="mt-2 text-center"><button @click="toggleShow('critical')" class="text-xs font-bold text-orange-500 hover:underline min-h-[36px] px-2">{{ criticalStockExpanded ? 'عرض أقل' : `عرض الكل (${criticalStockItemsList.length})` }}</button></div>
        </div>
        ...
        <div v-if="lowStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10">
          ...
          <div class="mt-2 text-center"><button @click="toggleShow('low')" class="text-xs font-bold text-yellow-500 hover:underline min-h-[36px] px-2">{{ lowStockExpanded ? 'عرض أقل' : `عرض الكل (${lowStockItemsList.length})` }}</button></div>
        </div>

        <div v-if="criticalStockItemsList.length === 0 && lowStockItemsList.length === 0 && outOfStockItemsList.length === 0 && !inventoryStore.isLoadingAllItems" class="text-center py-6 text-gray-500 font-bold">
      ...
      <div class="overflow-x-auto">
        <table class="w-full min-w-[550px]">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr v-if="recentTransactions.length === 0 && !inventoryStore.isLoadingAllItems">
...
// ============================================================
// ALL DATA COMES FROM STORE - No local duplication
// ============================================================

// Alert items from store (full dataset)
const outOfStockItemsList = computed(() => inventoryStore.allOutOfStockItems)
const criticalStockItemsList = computed(() => inventoryStore.allCriticalStockItems)
const lowStockItemsList = computed(() => inventoryStore.allLowStockItems)
...
const totalItemsCount = computed(() => inventoryStore.allItemsCount)
...
// Warehouse distribution from store
const warehouseStats = computed(() => {
  ...
  const items = inventoryStore.allItemsForStats
...
// ============================================================
// DASHBOARD LOADS ITS OWN DATA FROM STORE
// ============================================================
...
async function loadDashboardData() {
  ...
  // Load all items for stats using store method
...
// ============================================================
// LIFECYCLE - Dashboard loads data independently
// ============================================================

onActivated(async () => {
  // If data already loaded, just show it
  if (inventoryStore.allItemsForStats.length > 0 && !inventoryStore.isLoadingAllItems) {
    return
  }
