...
async function loadDashboardData(force = false) {
  ...
  if (dataInitialized.value && !force && dashboardStats.value.totalItems > 0) {
  ...
  try {
    ...
    const hasCachedItems = inventoryStore.items.length > 0
    ...
    const stats = await inventoryStore.fetchDashboardStats(
      selectedWarehouseId.value || undefined
    )
    dashboardStats.value = stats
    loadingProgress.value = 40
    
    const alerts = await inventoryStore.fetchAlertItemsOptimized(
      selectedWarehouseId.value || undefined
    )
    dashboardAlerts.value = alerts
    loadingProgress.value = 60
