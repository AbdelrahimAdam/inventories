        ...
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              ...
              <span v-if="alertItems.length > 0">({{ alertItems.length }} صنف في التنبيهات)</span>
            </p>
          </div>
        ...
        <div class="max-h-[350px] overflow-y-auto">
          ...
          <table v-else class="w-full min-w-[600px]">
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <tr v-if="warehouseStats.length === 0 && !showSkeleton">
      ...
      <div v-else class="space-y-3 max-h-[450px] overflow-y-auto">
        <div v-if="outOfStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-red-500">{{ outOfStockItems.length }} صنف</span>
          </div>
          <p class="text-xs text-red-700 dark:text-red-400 mb-2">{{ outOfStockItems.length }} صنف قد نفد بالكامل</p>
          <div class="overflow-x-auto">
        ...
        <div v-if="criticalStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-orange-500">{{ criticalStockItems.length }} صنف</span>
          </div>
          <p class="text-xs text-orange-700 dark:text-orange-400 mb-2">{{ criticalStockItems.length }} صنف بمستوى حرج (≤250 وحدة)</p>
          <div class="overflow-x-auto">
            <table class="w-full text-xs">
        ...
        <div v-if="lowStockItems.length > 0" class="p-3 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-yellow-500">{{ lowStockItems.length }} صنف</span>
          </div>
          <p class="text-xs text-yellow-700 dark:text-yellow-400 mb-2">{{ lowStockItems.length }} صنف بمستوى منخفض (51-500 وحدة)</p>
          <div class="overflow-x-auto">
            <table class="w-full text-xs">
        ...
        <div v-if="criticalStockItems.length === 0 && lowStockItems.length === 0 && outOfStockItems.length === 0 && !showSkeleton" class="text-center py-6 text-gray-500 font-bold">
...
const showSkeleton = computed(() => {
  return isFirstLoad.value && isLoadingStats.value
})
...
const filterItemsByWarehouse = (items: InventoryItem[]) => {
  if (!selectedWarehouseId.value) return items
  return items.filter(item => item.warehouseId === selectedWarehouseId.value)
}

const outOfStockItems = computed(() => {
  return filterItemsByWarehouse(alertItems.value).filter(item => item.remainingQuantity === 0)
})

const criticalStockItems = computed(() => {
  return filterItemsByWarehouse(alertItems.value).filter(item => item.remainingQuantity > 0 && item.remainingQuantity <= 250)
})

const lowStockItems = computed(() => {
  return filterItemsByWarehouse(alertItems.value).filter(item => item.remainingQuantity > 250 && item.remainingQuantity <= 500)
})
...
const totalItemsCount = computed(() => {
  return filterItemsByWarehouse(alertItems.value).length
})

const inStockCount = computed(() => {
  const total = totalItemsCount.value
  const low = lowStockItems.value.length
  const critical = criticalStockItems.value.length
  const out = outOfStockItems.value.length
...
const criticalStockNum = computed(() => totalItemsCount.value ? (criticalStockItems.value.length / totalItemsCount.value) * 100 : 0)
const lowStockNum = computed(() => totalItemsCount.value ? (lowStockItems.value.length / totalItemsCount.value) * 100 : 0)
const outOfStockNum = computed(() => totalItemsCount.value ? (outOfStockItems.value.length / totalItemsCount.value) * 100 : 0)
...
const fetchAlertItems = async (warehouseId?: string) => {
  try {
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    const [statsResult, transactionsResult, alertsResult] = await Promise.allSettled([
      inventoryStore.fetchSummaryStats({ warehouseId }),
      inventoryStore.fetchTransactions(1, 10),
      fetchAlertItems(warehouseId)
    ])
...
onActivated(async () => {
  if (dataInitialized.value && alertItems.value.length > 0) {
    const warehouseId = selectedWarehouseId.value || undefined
    await Promise.allSettled([
      inventoryStore.fetchSummaryStats({ warehouseId }),
      fetchAlertItems(warehouseId)
