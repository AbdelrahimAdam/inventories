        ...
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              ...
              <span v-else-if="!itemsLoaded && !showSkeleton">(جاري التحميل...)</span>
            </p>
          </div>
...
// ✅ Only show skeleton on initial load when we don't have data yet
const showSkeleton = computed(() => {
  return !dataInitialized.value && isLoading.value
})
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    // ✅ Mark data as initialized so skeleton disappears
    ...
    isLoading.value = false
    dataInitialized.value = true // Even on error, stop showing skeleton
  }
}
...
onActivated(async () => {
  // ✅ If data is already initialized, just refresh stats silently
