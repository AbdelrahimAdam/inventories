...
const outOfStockItemsList = computed(() => {
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    return inventoryStore.allItemsForStats.filter(item => item.remainingQuantity === 0)
  }
  return inventoryStore.outOfStockItems
})

const criticalStockItemsList = computed(() => {
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    return inventoryStore.allItemsForStats.filter(item => item.remainingQuantity > 0 && item.remainingQuantity <= 250)
  }
  return inventoryStore.criticalStockItems
})

const lowStockItemsList = computed(() => {
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    return inventoryStore.allItemsForStats.filter(item => item.remainingQuantity > 250 && item.remainingQuantity <= 500)
  }
...
const totalItemsCount = computed(() => {
  if (isAllItemsLoaded.value && inventoryStore.allItemsForStats.length > 0) {
    return inventoryStore.allItemsForStats.length
  }
  return inventoryStore.totalItems
})
...
const onWarehouseFilterChange = () => {
  ...
  // Reset and reload for the new warehouse
  inventoryStore.dataLoaded = false
  inventoryStore.allItemsForStats = []
  
  // Reload dashboard data with new warehouse filter
  loadDashboardData()
}
