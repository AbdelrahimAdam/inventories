<template>
  <div class="w-full pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <div v-if="inventoryStore.isLoading && inventoryStore.loadingProgress < 100" class="mb-4 bg-blue-50 dark:bg-blue-900/30 border border-blue-200 dark:border-blue-700 rounded-lg p-3">
      <div class="flex items-center gap-3">
        <div class="animate-spin rounded-full h-5 w-5 border-2 border-blue-500 border-t-transparent"></div>
        <div class="flex-1">
          <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
            <div class="bg-blue-600 h-2 rounded-full transition-all duration-300" :style="{ width: inventoryStore.loadingProgress + '%' }"></div>
          </div>
          <div class="text-xs text-blue-600 dark:text-blue-400 mt-1 font-semibold">
            جاري تحميل البيانات... {{ Math.round(inventoryStore.loadingProgress) }}%
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
        ...
        <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
          {{ formatNumber(displayStats.totalItems) }}
        </div>
      </div>
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
        ...
        <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
          {{ formatNumber(displayStats.totalQuantity) }}
        </div>
      </div>
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
        ...
        <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
          {{ formatNumber(displayStats.lowStock) }}
        </div>
      </div>
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-xl shadow-lg p-3 sm:p-4 text-white overflow-hidden">
        ...
        <div class="text-lg sm:text-xl lg:text-2xl font-black break-words leading-tight" :class="languageStore.isRTL ? 'text-right' : 'text-left'">
          {{ formatNumber(displayStats.outOfStock) }}
        </div>
      </div>
    </div>
