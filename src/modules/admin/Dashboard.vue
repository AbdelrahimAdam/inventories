...
const loadDashboardData = async () => {
  ...
  try {
    ...
    if (items.length > 0) {
      ...
      const itemsByUniqueKey = new Map<string, string>()
      for (const item of items) {
        itemsMap.set(item.id, item)
        const key = `${item.name}|${item.code}|${item.color}|${item.size}|${item.warehouseId}`
        itemsByUniqueKey.set(key, item.id)
      }
      inventoryStore.itemsMap = itemsMap
      inventoryStore.itemsByUniqueKey = itemsByUniqueKey
      inventoryStore.totalCount = items.length
      inventoryStore.dataLoaded = true
      inventoryStore.cacheLoaded = true
    }
...
const refreshData = async () => {
  ...
  try {
    ...
    await Promise.all([
      ...
      inventoryStore.fetchItemsPage({ page: 1, pageSize: 50, warehouseId: warehouseId, force: true }),
