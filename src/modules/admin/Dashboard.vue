<template>
  <div class="w-full pb-20" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <!-- Stats Cards - Using cached items -->
    ...
    <!-- Inventory Distribution - Using cached items -->
    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-300 dark:border-gray-600 shadow-md p-3 sm:p-4 mb-3 hover:border-amber-400 dark:hover:border-amber-600 hover:shadow-lg transition-all duration-300">
    ...
    <!-- Warehouse Distribution - Using cached items -->
...
// ============================================
// Stats calculated from cached items
// ============================================
...
// ============================================
// Display stats from cached items
// ============================================
...
const criticalStockItems = computed(() => {
  const items = filterItemsByWarehouse(inventoryStore.items)
  return items.filter(item => item.remainingQuantity > 0 && item.remainingQuantity <= 250)
})

const lowStockItems = computed(() => {
  const items = filterItemsByWarehouse(inventoryStore.items)
  return items.filter(item => item.remainingQuantity > 250 && item.remainingQuantity <= 500)
})
...
// ============================================
// Load dashboard data using cached items
// ============================================
async function loadDashboardData(force = false) {
  const tenantId = authStore.currentTenantId
  if (!tenantId) return

  // If data is already initialized and not forced, just refresh stats
  if (dataInitialized.value && !force && inventoryStore.items.length > 0) {
  ...
  try {
    ...
    // If items already exist in cache (from item list page), use them
    if (inventoryStore.items.length > 0) {
      loadingProgress.value = 60
      // Stats and alerts are computed from cached items
    ...
    // Load all items with progress
    await inventoryStore.fetchAllItemsForExport({
      isFullLoad: true,
      onProgress: (loaded, total) => {
        loadingProgress.value = Math.min(80, Math.round((loaded / total) * 80))
      }
    })
    loadingProgress.value = 80
...
async function refreshData() {
  ...
  try {
    ...
    // Clear the store items so they reload
...
// ============================================
// Filter and modal handlers
// ============================================
async function onWarehouseFilterChange() {
  // Just update the filter - stats will recompute
  await inventoryStore.fetchTransactions(1, 10)
}

async function onTransferSuccess() {
  // Stats will recompute from cached items
  await inventoryStore.fetchTransactions(1, 10)
}
...
onActivated(async () => {
  if (authStore.isAuthenticated) {
    ...
    if (tenantId) {
      ...
      if (inventoryStore.items.length > 0 && dataInitialized.value) {
