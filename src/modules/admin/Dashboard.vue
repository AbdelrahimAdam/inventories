        ...
        <div class="flex justify-between items-center flex-wrap gap-2">
          <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
              ...
              <span v-if="totalItemsForDistribution > 0">({{ totalItemsForDistribution }} صنف)</span>
            </p>
          </div>
...
// Use store's cached items for distribution - this gives accurate data
const allItemsForDistribution = computed(() => {
...
const totalItemsForDistribution = computed(() => allItemsForDistribution.value.length)

// Filter alert items by warehouse
const filterAlertItemsByWarehouse = (items: InventoryItem[]) => {
  if (!selectedWarehouseId.value) return items
  return items.filter(item => item.warehouseId === selectedWarehouseId.value)
}

// For alerts - use the alertItems from the store
const outOfStockItems = computed(() => {
  const items = filterAlertItemsByWarehouse(inventoryStore.outOfStockItems)
  return items
})

const criticalStockItems = computed(() => {
  const items = filterAlertItemsByWarehouse(inventoryStore.criticalStockItems)
  return items
})

const lowStockItems = computed(() => {
  const items = filterAlertItemsByWarehouse(inventoryStore.lowStockItems)
  return items
})
...
// For chart - use all items from store
...
// Warehouse stats from cached items
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    // Load items into cache if empty
...
const onWarehouseFilterChange = async () => {
  ...
  // Just update stats and transactions - items already cached
  await Promise.allSettled([
    inventoryStore.fetchSummaryStats({ warehouseId }),
    inventoryStore.fetchTransactions(1, 10)
  ])
...
onActivated(async () => {
  // If data is already loaded and items exist, just refresh stats silently
