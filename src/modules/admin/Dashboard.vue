      ...
      <div v-else class="space-y-3 max-h-[450px] overflow-y-auto">
        ...
        <div v-if="outOfStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-red-500 bg-red-50/50 dark:bg-red-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-red-500">{{ outOfStockItemsList.length }} صنف</span>
          </div>
        ...
        <div v-if="criticalStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-orange-500 bg-orange-50/50 dark:bg-orange-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-orange-500">{{ criticalStockItemsList.length }} صنف</span>
          </div>
        ...
        <div v-if="lowStockItemsList.length > 0" class="p-3 rounded-xl border-r-4 border-yellow-500 bg-yellow-50/50 dark:bg-yellow-900/10">
          <div class="flex justify-between items-start mb-2 flex-wrap gap-2">
            ...
            <span class="text-xs font-bold text-yellow-500">{{ lowStockItemsList.length }} صنف</span>
          </div>
...
// ✅ Fix #1: Use store's currentFilters instead of duplicate state
...
// ✅ Fix #2 & #6: Use store's computed properties for alerts
const outOfStockItemsList = computed(() => {
  const items = selectedWarehouseId.value 
    ? inventoryStore.outOfStockItems.filter(item => item.warehouseId === selectedWarehouseId.value)
    : inventoryStore.outOfStockItems
  return items
})

const criticalStockItemsList = computed(() => {
  const items = selectedWarehouseId.value 
    ? inventoryStore.criticalStockItems.filter(item => item.warehouseId === selectedWarehouseId.value)
    : inventoryStore.criticalStockItems
  return items
})

const lowStockItemsList = computed(() => {
  const items = selectedWarehouseId.value 
    ? inventoryStore.lowStockItems.filter(item => item.warehouseId === selectedWarehouseId.value)
    : inventoryStore.lowStockItems
  return items
})

// ✅ Fix #3 & #5: Use items as single source of truth
...
// ✅ Fix #9: Warehouse stats computed from store data
...
// ✅ Fix #3: Use fetchItemsBulk for efficient bulk loading
const loadDashboardData = async () => {
  ...
  try {
    ...
    // ✅ Use fetchItemsBulk for optimized bulk loading
    const items = await inventoryStore.fetchItemsBulk(warehouseId)
    
    // Update the store's items map with bulk data
    if (items && items.length > 0) {
      ...
      // Use store's internal maps
      inventoryStore.itemsMap = newMap
      inventoryStore.itemsByUniqueKey = newUniqueMap
...
const refreshData = async () => {
  ...
  try {
    ...
    // ✅ Use fetchItemsBulk for refresh
    const items = await inventoryStore.fetchItemsBulk(warehouseId)
    ...
    if (items && items.length > 0) {
      ...
      if (tenantId) {
        inventoryStore.saveToCache(tenantId)
        inventoryStore.updateDataVersion(tenantId)
      }
    }
    ...
    const promises: Promise<any>[] = [
      ...
      inventoryStore.fetchTransactions(1, 10),
...
const onWarehouseFilterChange = async () => {
  ...
  // Reload data with new warehouse filter
  await loadDashboardData()
}

// Trial/Subscription Logic (Business Logic - Keep as is)
...
onActivated(async () => {
  if (dataInitialized.value && inventoryStore.dataLoaded) {
...
onDeactivated(() => {
  // Nothing to clean up - store handles its own state
})
