...
// ✅ Simply use inventoryStore items - they are loaded by the store watch
...
// ============================================
// ✅ SIMPLIFIED: Dashboard just waits for items to load
// The store watch already loads all items
// ============================================
async function loadDashboardData(force = false) {
  const tenantId = authStore.currentTenantId
  if (!tenantId) return

  // If data already initialized and items exist, just refresh transactions
  if (dataInitialized.value && !force && inventoryStore.items.length > 0) {
    await inventoryStore.fetchTransactions(1, 10)
    return
  }

  // If items are already loaded by the store watch, use them
  ...
  // If no items exist yet, wait for them (store watch is loading)
  ...
  try {
    ...
    // Wait for items to load (store watch is already loading them)
    // We'll watch the store's loading state
