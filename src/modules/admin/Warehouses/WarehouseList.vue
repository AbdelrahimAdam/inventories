        ...
        <div class="card-content">
          ...
          <div class="card-footer">
            <span class="status-badge" :class="statusVariant(warehouse)">
              <span class="status-dot" :class="warehouse.is_active ? 'status-dot--active' : 'status-dot--inactive'"></span>
...
// FIXED: Proper interface - extends Warehouse without changing required fields
// All fields from Warehouse are inherited exactly as defined
interface WarehouseExtended extends Warehouse {
  // Only add optional extra fields, don't override existing ones
  created_by_name?: string
  updated_by_name?: string
  itemCount?: number
  // Note: name_ar, name_en, location, description, created_by, updated_by, created_at, updated_at, 
  // tenant_id, is_main, is_active, type, id, name all come from Warehouse
}
...
// FIXED: Properly typed filtered warehouses with all required Warehouse fields
const filteredWarehouses = computed((): WarehouseExtended[] => {
  ...
  // Ensure all required Warehouse fields are present
  return warehouses.map((w: any) => {
    // Create a proper Warehouse-compatible object
    const warehouse: WarehouseExtended = {
      id: w.id || '',
      name: w.name || '',
      name_ar: w.name_ar,
      name_en: w.name_en,
      type: w.type || 'primary',
      is_main: w.is_main !== undefined ? w.is_main : false,
      is_active: w.is_active !== undefined ? w.is_active : true,
      location: w.location,
      description: w.description,
      capacity: w.capacity,
      tenant_id: w.tenant_id || w.tenantId || '',
      ...
      updated_by_name: w.updated_by_name,
...
const cardVariant = (warehouse: WarehouseExtended) => {
  if (!warehouse.is_active) return 'card--inactive'
...
const topBarVariant = (warehouse: WarehouseExtended) => {
  if (!warehouse.is_active) return 'top-bar--inactive'
...
const statusVariant = (warehouse: WarehouseExtended) => {
  return warehouse.is_active ? 'status--active' : 'status--inactive'
}
