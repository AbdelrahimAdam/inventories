  ...
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
      ...
      <div class="flex gap-2 w-full sm:w-auto">
        <button @click="exportToExcel" class="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-xl transition-all inline-flex items-center justify-center gap-1.5 shadow-md hover:shadow-lg font-bold text-xs sm:text-sm min-h-[44px] border border-blue-400/20">
        ...
        <button @click="refreshData" :disabled="isRefreshing" class="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-700 hover:to-gray-800 text-white rounded-xl transition-all inline-flex items-center justify-center gap-1.5 shadow-md hover:shadow-lg font-bold disabled:opacity-50 text-xs sm:text-sm min-h-[44px] border border-gray-500/20">
    ...
    <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-7 gap-1.5 sm:gap-2 mb-4">
      <div class="stat-card-warm">
        <p class="text-white/80 text-[10px] sm:text-xs font-bold truncate">إجمالي الحركات</p>
        <p class="text-white text-sm sm:text-base lg:text-lg font-black truncate" :title="String(storeStats.total)">{{ formatNumber(storeStats.total) }}</p>
      </div>
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-green-100 text-[10px] sm:text-xs font-bold truncate">إضافة</p>
        <p class="text-sm sm:text-base lg:text-lg font-black truncate" :title="String(storeStats.add)">{{ formatNumber(storeStats.add) }}</p>
      </div>
      <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-blue-100 text-[10px] sm:text-xs font-bold truncate">تعديل</p>
        <p class="text-sm sm:text-base lg:text-lg font-black truncate" :title="String(storeStats.update)">{{ formatNumber(storeStats.update) }}</p>
      </div>
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-red-100 text-[10px] sm:text-xs font-bold truncate">حذف</p>
        <p class="text-sm sm:text-base lg:text-lg font-black truncate" :title="String(storeStats.delete)">{{ formatNumber(storeStats.delete) }}</p>
      </div>
      <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-purple-100 text-[10px] sm:text-xs font-bold truncate">تحويل</p>
        <p class="text-sm sm:text-base lg:text-lg font-black truncate" :title="String(storeStats.transfer)">{{ formatNumber(storeStats.transfer) }}</p>
      </div>
      <div class="bg-gradient-to-br from-amber-500 to-amber-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-amber-100 text-[10px] sm:text-xs font-bold truncate">صرف</p>
        <p class="text-sm sm:text-base lg:text-lg font-black truncate" :title="String(storeStats.dispatch)">{{ formatNumber(storeStats.dispatch) }}</p>
      </div>
      <div class="bg-gradient-to-br from-teal-500 to-teal-600 rounded-lg shadow-md p-1.5 sm:p-2 text-white overflow-hidden border border-white/10 hover:shadow-lg transition-all duration-300">
        <p class="text-teal-100 text-[10px] sm:text-xs font-bold truncate">مرتجع</p>
        <p class="text-sm sm:text-base lg:text-lg font-black truncate" :title="String(storeStats.return)">{{ formatNumber(storeStats.return) }}</p>
      </div>
    </div>
    ...
    <div class="card-warm p-3 sm:p-4 mb-4">
      <div class="relative mb-3">
        ...
        <button v-if="searchQuery.length > 0 && !isSearching" @click="clearSearch" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 min-h-[44px] px-2">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div v-if="searchQuery.length > 0" class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400 mb-2">
        <span>{{ filteredTransactions.length }} نتيجة <span v-if="isSearching">(جارٍ البحث...)</span></span>
        <span v-if="filteredTransactions.length === 0 && searchQuery.length >= 2 && !isSearching" class="text-amber-600 dark:text-amber-400">لا توجد نتائج مطابقة</span>
      </div>

      <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-2">
        <select v-model="typeFilter" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
        ...
        <select v-model="warehouseFilter" class="px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors">
        ...
        <div class="flex gap-2">
          <input
            ...
            type="date"
            class="flex-1 px-3 py-1.5 border border-gray-300 dark:border-gray-600 rounded-xl focus:ring-2 focus:ring-amber-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm font-medium min-h-[44px] hover:border-amber-300 dark:hover:border-amber-700 transition-colors"
          />
          <button @click="setTodayFilter" class="px-3 py-1.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-xl text-sm font-bold hover:bg-amber-200 dark:hover:bg-amber-800/30 transition-colors whitespace-nowrap min-h-[44px] border border-amber-200 dark:border-amber-700">اليوم</button>
        </div>
      </div>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              <template v-else>
                <tr v-for="tx in paginatedTransactions" :key="tx.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                  ...
                  <td class="px-3 py-2.5 text-center"><span :class="getTypeBadge(tx.type)" class="px-2 py-1 text-xs font-black rounded-full inline-block border">{{ getTypeText(tx.type) }}</span></td>
                  ...
                  <td class="px-3 py-2.5 text-center"><span class="px-2 py-0.5 bg-blue-50 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded-md text-xs font-semibold border border-blue-200 dark:border-blue-800">{{ tx.itemSize || '—' }}</span></td>
    ...
    <div v-if="!isSearchActive && paginatedTransactions.length < filteredTransactions.length" class="flex justify-center mt-4">
      <button @click="loadMoreItems" class="px-5 py-2 bg-gradient-to-r from-amber-600 to-amber-700 hover:from-amber-700 hover:to-amber-800 text-white rounded-xl shadow-md hover:shadow-lg font-bold transition-all text-sm min-h-[44px] border border-amber-500/20">
...
watch(
  () => inventoryStore.dataVersion,
  () => {
    if (inventoryStore.transactionsLoaded) {
      refreshData()
    }
  }
)
...
const storeStats = computed(() => {
  ...
  return {
    ...
    return: filteredForStats.filter(tx => tx.type === 'RETURN').length,
  }
})
