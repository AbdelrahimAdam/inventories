<template>
  <div :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    ...
    <!-- Stats Cards -->
    <div class="grid grid-cols-2 md:grid-cols-7 gap-2 sm:gap-3 mb-4">
      ...
      <div class="bg-gradient-to-br from-teal-500 to-teal-600 rounded-xl shadow-lg p-3 text-white overflow-hidden">
        <p class="text-teal-100 text-xs font-bold">مرتجع</p>
        <p class="text-lg sm:text-xl lg:text-2xl font-black break-words max-w-full truncate" :title="String(storeStats.return)">{{ formatNumber(storeStats.return) }}</p>
      </div>
    </div>
...
// ⭐ NEW: Watch for transaction changes to auto-refresh
watch(
...
// ⭐ NEW: Watch for dataVersion changes to refresh
watch(
