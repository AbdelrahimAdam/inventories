<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900 p-3 sm:p-6" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
      ...
      <div class="flex gap-2 w-full sm:w-auto">
        ...
        >
          ...
          <span class="hidden xs:inline">{{ isLoading ? 'جاري التحديث...' : 'تحديث' }}</span>
          <span class="xs:hidden">{{ isLoading ? '...' : '🔄' }}</span>
        </button>
        <router-link
    ...
    <!-- Loading Progress -->
    <div v-if="isLoading && inventoryStore.loadingProgress < 100" class="mb-4">
      <div class="flex items-center gap-3">
        ...
        <div class="flex-1">
          ...
          <div class="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 overflow-hidden">
            <div class="bg-gradient-to-r from-amber-500 to-amber-600 h-1.5 rounded-full transition-all duration-500" :style="{ width: Math.min(inventoryStore.loadingProgress, 100) + '%' }"></div>
          </div>
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl sm:text-3xl font-bold">
              {{ isLoading ? '...' : formatNumber(displayStats.totalItems) }}
            </p>
          </div>
      ...
      <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-lg shadow-lg p-3 sm:p-6 text-white hover:shadow-xl transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl sm:text-3xl font-bold">
              {{ isLoading ? '...' : formatNumber(displayStats.lowStock) }}
            </p>
          </div>
          <div class="bg-white/20 rounded-full p-2 sm:p-3">
            <svg class="w-4 h-4 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-gradient-to-br from-red-500 to-red-600 rounded-lg shadow-lg p-3 sm:p-6 text-white hover:shadow-xl transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl sm:text-3xl font-bold">
              {{ isLoading ? '...' : formatNumber(displayStats.criticalStock) }}
            </p>
          </div>
          <div class="bg-white/20 rounded-full p-2 sm:p-3">
            <svg class="w-4 h-4 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-lg p-3 sm:p-6 text-white hover:shadow-xl transition-shadow">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-xl sm:text-3xl font-bold">
              {{ isLoading ? '...' : formatNumber(displayStats.totalQuantity) }}
            </p>
          </div>
          <div class="bg-white/20 rounded-full p-2 sm:p-3">
            <svg class="w-4 h-4 sm:w-6 sm:h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Low Stock Alerts -->
    <div v-if="!isLoading && (lowStockItems.length > 0 || criticalStockItems.length > 0)" class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 p-3 sm:p-6 mb-8">
    ...
    <!-- Recent Transactions -->
    <div v-if="!isLoading && recentTransactions.length > 0" class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 p-3 sm:p-6">
    ...
    <div v-if="!isLoading && inventoryStore.items.length === 0" class="text-center py-12">
      <svg class="w-16 h-16 mx-auto text-gray-300 dark:text-gray-600 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
      </svg>
      <p class="text-gray-500 dark:text-gray-400 font-bold text-base">لا توجد بيانات لعرضها</p>
      <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">قم بإضافة أصناف جديدة للبدء</p>
      <router-link
        to="/inventory/items/new"
        class="inline-block mt-4 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors text-sm font-bold"
      >
        إضافة صنف جديد
      </router-link>
    </div>
...
// Load dashboard data with cache support
