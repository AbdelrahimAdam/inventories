        ...
        <table class="w-full">
          <thead class="bg-amber-700 sticky top-0 z-10">
            <tr>
              ...
              <th class="px-6 py-4 text-center text-base font-bold text-white uppercase tracking-wider">التجربة تنتهي</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-center">
                <span :class="getSubscriptionBadge(user.isTrial)" class="px-2 py-1 text-xs rounded-full">
                  {{ user.isTrial ? 'تجريبي' : 'نشط' }}
                </span>
              </td>
            ...
            <tr v-if="filteredUsers.length === 0">
              <td colspan="9" class="px-6 py-12 text-center text-gray-500 dark:text-gray-400">لا توجد مستخدمين</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const filteredUsers = computed((): User[] => {
  ...
  if (filters.value.subscriptionStatus === 'trial') filtered = filtered.filter(u => u.isTrial)
  if (filters.value.subscriptionStatus === 'active') filtered = filtered.filter(u => !u.isTrial)
  if (filters.value.subscriptionStatus === 'expired') filtered = filtered.filter(u => u.isTrial && u.trialEndsAt && new Date(u.trialEndsAt) < new Date())
  return filtered
})
...
const getSubscriptionBadge = (isTrial: boolean): string => {
  return isTrial 
...
const saveUser = async () => {
  ...
  try {
    ...
    if (isEditing.value) {
      ...
      // Use environment variable for edge function URL
