    ...
    <div class="container mx-auto px-3 sm:px-4 py-4 sm:py-8 pb-20 md:pb-8">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <div class="flex flex-wrap gap-2 w-full sm:w-auto">
          <button @click="openAddModal" class="flex-1 sm:flex-none bg-green-600 hover:bg-green-700 dark:bg-green-700 dark:hover:bg-green-800 text-white px-4 py-2.5 rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 text-sm">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
            </svg>
            إضافة مستخدم
          </button>
          <button @click="openInviteModal" class="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-700 dark:bg-blue-700 dark:hover:bg-blue-800 text-white px-4 py-2.5 rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 text-sm">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
            دعوة مستخدمين
          </button>
          <button @click="refreshData" :disabled="isRefreshing" class="flex-1 sm:flex-none bg-amber-600 hover:bg-amber-700 dark:bg-amber-700 dark:hover:bg-amber-800 text-white px-4 py-2.5 rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 text-sm disabled:opacity-50">
          ...
          <table class="w-full">
            <thead class="bg-gradient-to-r from-amber-700 to-amber-800 sticky top-0 z-10">
              <tr>
                ...
                <th class="px-4 py-3 text-center text-xs font-bold text-white uppercase tracking-wider">آخر نشاط</th>
            ...
            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
              <tr v-for="user in paginatedUsers" :key="user.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-center text-gray-600 dark:text-gray-400 text-sm">{{ getTenantName(user.tenantId) }}</td>
                <td class="px-4 py-3 whitespace-nowrap text-center">
                ...
                <td class="px-4 py-3 whitespace-nowrap text-center text-gray-600 dark:text-gray-400 text-sm">{{ user.lastLogin ? formatDate(user.lastLogin) : '—' }}</td>
                <td class="px-4 py-3 whitespace-nowrap text-center">
                  <div class="flex gap-1 justify-center">
                    <button @click="editUser(user)" class="p-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors" title="تعديل">
        ...
        <div v-for="user in paginatedUsers" :key="user.id" class="bg-white dark:bg-gray-800 rounded-lg shadow p-4 border border-gray-200 dark:border-gray-700">
          ...
          <div class="grid grid-cols-2 gap-2 text-sm mb-3">
            <div>
              ...
              <p class="font-medium text-gray-900 dark:text-white">{{ getTenantName(user.tenantId) }}</p>
            </div>
            <div>
            ...
            <div>
              <span class="text-gray-500 dark:text-gray-400 text-xs">آخر نشاط</span>
              <p class="font-medium text-gray-900 dark:text-white">{{ user.lastLogin ? formatDate(user.lastLogin) : '—' }}</p>
            </div>
            <div>
              <span class="text-gray-500 dark:text-gray-400 text-xs">تاريخ الإنشاء</span>
              <p class="font-medium text-gray-900 dark:text-white">{{ formatDate(user.createdAt) }}</p>
            </div>
          </div>

          <div class="flex gap-2">
            <button @click="editUser(user)" class="flex-1 px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs transition-colors flex items-center justify-center gap-1">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
              تعديل
            </button>
            <button @click="toggleUserStatus(user)" class="flex-1 px-3 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg text-xs transition-colors flex items-center justify-center gap-1">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
              </svg>
              {{ user.isActive ? 'تعطيل' : 'تفعيل' }}
            </button>
            <button @click="confirmDelete(user)" class="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs transition-colors flex items-center justify-center">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
              </svg>
            </button>
      ...
      <div v-if="selectedUsers.length > 0" class="fixed bottom-20 sm:bottom-8 left-1/2 -translate-x-1/2 z-40 bg-white dark:bg-gray-800 rounded-lg shadow-2xl border border-gray-200 dark:border-gray-700 p-3 flex items-center gap-3">
        ...
        <button @click="bulkDeactivate" class="px-3 py-1.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm transition-colors">تعطيل</button>
        <button @click="bulkDelete" class="px-3 py-1.5 bg-gray-600 hover:bg-gray-700 text-white rounded-lg text-sm transition-colors">حذف</button>
              ...
              <div class="border border-gray-300 dark:border-gray-600 rounded-lg p-4 max-h-48 overflow-y-auto bg-white dark:bg-gray-700">
                ...
                <div class="border-t border-gray-200 dark:border-gray-600 pt-2 mt-2">
                  ...
                  <div v-if="tenantWarehouses.length === 0" class="text-gray-500 dark:text-gray-400 text-sm">
                    لا توجد مستودعات لهذا المستأجر
                  </div>
                </div>
              </div>
            </div>
      ...
      <div v-if="showInviteModal" class="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50 p-4" @click.self="showInviteModal = false">
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full mx-4 shadow-2xl">
          ...
          <div class="flex flex-col sm:flex-row justify-end gap-3 mt-6">
            <button @click="showInviteModal = false" class="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300">إلغاء</button>
            <button @click="sendInvitations" :disabled="isLoading" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors shadow-md flex items-center justify-center gap-2">
              <svg v-if="isLoading" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
              </svg>
