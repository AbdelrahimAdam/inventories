<template>
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    <div class="container mx-auto px-3 sm:px-4 py-4 md:py-8 pb-20 md:pb-8">
      ...
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        <div class="flex flex-wrap gap-2 w-full sm:w-auto">
          ...
          <button 
            ...
            class="flex-1 sm:flex-none bg-gray-600 hover:bg-gray-700 dark:bg-gray-600 dark:hover:bg-gray-700 text-white px-4 py-2.5 rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 text-sm disabled:opacity-50"
          >
      ...
      <!-- Last Updated -->
      <div class="text-xs text-gray-400 dark:text-gray-500 mb-4 text-left">
        آخر تحديث: {{ lastUpdated ? formatTime(lastUpdated) : 'جاري التحميل...' }}
        <span v-if="isCached" class="mr-2 text-green-500">(من التخزين المؤقت)</span>
      </div>
...
const loadFromCache = (): Tenant[] | null => {
  try {
    ...
    // Restore dates
    const data = cacheData.data.map((item: any) => ({
      ...item,
...
const fetchTenants = async (forceRefresh = false) => {
  ...
  try {
    ...
    // Parallel queries for user and item counts
    const [userCountsResult, itemCountsResult] = await Promise.all([
      supabase
        .from('users')
    ...
    if (userCountsResult.error) throw userCountsResult.error
    ...
    itemCountsResult.data?.forEach((item: any) => {
      itemCountMap[item.tenant_id] = (itemCountMap[item.tenant_id] || 0) + 1
    })

    const mappedData = tenantsData.map((tenant: any) => ({
      ...tenant,
...
const saveTenant = async () => {
  ...
  try {
    ...
    closeModal()
    await fetchTenants(true) // Force refresh after save
  } catch (error: any) {
...
const extendSubscription = async (months: number) => {
  ...
  try {
    ...
    if (data?.success) {
      ...
      await Promise.all([fetchTenants(true), fetchPendingRequests()])
    } else {
...
const exportData = () => {
  ...
  const headers = ['الاسم', 'المعرف', 'النطاق', 'المستخدمين', 'الأصناف', 'الحالة', 'ينتهي في', 'تاريخ الإنشاء']
  const csv = [
    headers.join(','),
...
const approveRequest = async (request: UpgradeRequest) => {
  ...
  try {
    ...
    if (data?.success) {
      ...
      await Promise.all([fetchPendingRequests(), fetchTenants(true)])
    } else {
