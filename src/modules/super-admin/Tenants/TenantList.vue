...
// ✅ FIX: Convert request.id to UUID by using the string directly
const approveRequest = async (request: UpgradeRequest) => {
  ...
  try {
    // ✅ The RPC expects UUID, but we can pass the string directly
    // Supabase will handle the conversion if the string is a valid UUID
    ...
    if (data?.success) {
      ...
      alert(data?.message || 'حدث خطأ أثناء قبول الطلب')
    }
  } catch (error: any) {
...
// ✅ FIX: Convert request.id to UUID
...
// ✅ FIX: Convert tenant.id (string) to UUID for the RPC
const extendSubscription = async (months: number) => {
  if (!selectedTenant.value) return

  isLoading.value = true
  try {
    // The RPC expects UUID, we pass the string directly
    ...
    if (data?.success) {
      ...
      alert(`✅ ${data.message}\nالتاريخ الجديد: ${new Date(data.new_paid_until).toLocaleDateString('ar-EG')}`)
      await fetchTenants()
      await fetchPendingRequests()
    } else {
      alert(data?.message || 'حدث خطأ أثناء تمديد الاشتراك')
    }
  } catch (error: any) {
...
// ✅ FIX: Convert tenant.id (string) to UUID
