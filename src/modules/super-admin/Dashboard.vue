      ...
      <div class="bg-white rounded-lg shadow p-6">
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class="text-2xl font-bold">${{ formatNumber(stats.totalValue) }}</p>
          </div>
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200">
            <tr v-for="tenant in recentTenants" :key="tenant.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4"><span class="px-2 py-1 bg-gray-100 rounded text-sm">{{ tenant.slug }}</span></td>
              <td class="px-6 py-4">{{ tenant.userCount || 0 }}</td>
              <td class="px-6 py-4">{{ formatDate(tenant.createdAt) }}</td>
            </tr>
            <tr v-if="recentTenants.length === 0">
              <td colspan="4" class="px-6 py-12 text-center text-gray-500">No tenants found</td>
            </tr>
          </tbody>
        </table>
      </div>
