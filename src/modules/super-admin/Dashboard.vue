<template>
  <div class="p-3 sm:p-6" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
      ...
      <div class="sm:hidden divide-y divide-gray-200 dark:divide-gray-700">
        <div v-for="tenant in expiringTenants" :key="tenant.id" class="p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
          ...
          <div class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400">
            ...
            <router-link to="/super-admin/tenants" class="text-blue-600 dark:text-blue-400 font-medium">تمديد</router-link>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
      ...
      <!-- Mobile Cards -->
      <div class="sm:hidden divide-y divide-gray-200 dark:divide-gray-700">
        <div v-for="tenant in recentTenants" :key="tenant.id" class="p-3 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors">
          <div class="flex justify-between items-start mb-2">
            <div>
            ...
            <span :class="getSubscriptionBadge(tenant.subscription_status)" class="px-2 py-0.5 text-xs rounded-full">
              {{ formatSubscriptionStatus(tenant.subscription_status) }}
            </span>
          </div>
...
const ensureDataLoaded = async (force = false) => {
  ...
  isLoadingInitial.value = !dataLoaded
  try {
    await fetchStats()
  } catch (error) {
