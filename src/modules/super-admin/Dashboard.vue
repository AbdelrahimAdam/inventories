<template>
  <div class="p-6" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">لوحة تحكم المشرف العام</h1>
      <div class="flex gap-2">
        <button @click="refreshData" :disabled="isRefreshing" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors shadow-md disabled:opacity-50 flex items-center gap-2">
        ...
        <router-link to="/superadmin/tenants" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-colors shadow-md flex items-center gap-2">
          إدارة المستأجرين
        </router-link>
      </div>
    </div>
    ...
    <div v-if="stats.pendingRequests > 0" class="mb-6 bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border border-purple-200 dark:border-purple-800 rounded-lg p-4 flex justify-between items-center">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="font-bold text-purple-800 dark:text-purple-300">{{ stats.pendingRequests }} طلب ترقية في انتظار الموافقة</p>
          <p class="text-sm text-purple-600 dark:text-purple-400">قم بمراجعة طلبات الترقية من صفحة إدارة المستأجرين</p>
        </div>
      </div>
      <router-link to="/superadmin/tenants" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-colors shadow-md text-sm font-bold">
        عرض الطلبات
      </router-link>
    </div>

    <!-- Expiring Soon Alert -->
    <div v-if="stats.expiringSoon > 0" class="mb-6 bg-gradient-to-r from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4 flex justify-between items-center">
      <div class="flex items-center gap-3">
        ...
        <div>
          <p class="font-bold text-amber-800 dark:text-amber-300">{{ stats.expiringSoon }} اشتراكات تنتهي خلال 7 أيام</p>
          <p class="text-sm text-amber-600 dark:text-amber-400">قم بتمديد الاشتراكات قبل انتهائها</p>
        </div>
      </div>
      <router-link to="/superadmin/tenants" class="px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-colors shadow-md text-sm font-bold">
        عرض الاشتراكات
      </router-link>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 dark:text-gray-400 text-sm">اشتراكات نشطة</p>
            <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ formatNumber(stats.activeSubscriptions) }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 dark:text-gray-400 text-sm">فترة تجريبية</p>
            <p class="text-2xl font-bold text-amber-600 dark:text-amber-400">{{ formatNumber(stats.trialTenants) }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 dark:text-gray-400 text-sm">منتهية</p>
            <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ formatNumber(stats.expiredSubscriptions) }}</p>
          </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 border border-gray-200 dark:border-gray-700">
        <div class="flex items-center justify-between">
          <div>
            <p class="text-gray-500 dark:text-gray-400 text-sm">طلبات معلقة</p>
            <p class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ formatNumber(stats.pendingRequests) }}</p>
          </div>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6 mb-8">
      <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">حالة اشتراكات المستأجرين</h3>
      <div class="flex items-center gap-4 flex-wrap">
        <div class="flex items-center gap-2">
          <div class="w-4 h-4 rounded-full bg-green-500"></div>
          <span class="text-sm text-gray-700 dark:text-gray-300">نشط ({{ stats.activeSubscriptions }})</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="w-4 h-4 rounded-full bg-amber-500"></div>
          <span class="text-sm text-gray-700 dark:text-gray-300">تجريبي ({{ stats.trialTenants }})</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="w-4 h-4 rounded-full bg-red-500"></div>
          <span class="text-sm text-gray-700 dark:text-gray-300">منتهي ({{ stats.expiredSubscriptions }})</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="w-4 h-4 rounded-full bg-gray-500"></div>
          <span class="text-sm text-gray-700 dark:text-gray-300">ملغي ({{ stats.cancelledSubscriptions }})</span>
        </div>
      </div>
      <div class="mt-4 h-4 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden flex">
    ...
    <div v-if="expiringTenants.length > 0" class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 mb-8">
      <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">⚠️ اشتراكات على وشك الانتهاء</h3>
        <span class="text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 px-2 py-1 rounded-full">
        ...
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">تاريخ الانتهاء</th>
              <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">الأيام المتبقية</th>
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="tenant in expiringTenants" :key="tenant.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-gray-600 dark:text-gray-400">{{ formatDate(tenant.paid_until) }}</td>
              <td class="px-6 py-4 whitespace-nowrap">
                <span :class="tenant.daysLeft <= 3 ? 'text-red-600 font-bold' : 'text-amber-600'">
                  {{ tenant.daysLeft }} يوم
                </span>
              </td>
              <td class="px-6 py-4 whitespace-nowrap">
                <router-link :to="`/superadmin/tenants`" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-sm font-medium">
                  تمديد
                </router-link>
              </td>
            </tr>
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700">
      <div class="p-6 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
        ...
        <router-link to="/superadmin/tenants" class="text-sm text-amber-600 dark:text-amber-400 hover:underline">عرض الكل</router-link>
      </div>
      <div class="overflow-x-auto">
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-gray-700">
            <tr>
              ...
              <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">حالة الاشتراك</th>
...
const fetchStats = async () => {
  try {
    // Get total tenants with subscription status
    ...
    for (const tenant of (tenants || [])) {
      ...
      // Check for expiring soon (active subscriptions ending within 7 days)
    ...
    expiringTenants.value = expiring.sort((a, b) => (a.daysLeft || 0) - (b.daysLeft || 0))

    // Get recent tenants with user counts
