  ...
  <div class="p-3 sm:p-6" :dir="languageStore.isRTL ? 'rtl' : 'ltr'">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4 sm:mb-6">
      ...
      <div class="flex flex-wrap gap-2 w-full sm:w-auto">
        <button @click="refreshData" :disabled="isRefreshing" class="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all shadow-md hover:shadow-lg disabled:opacity-50 flex items-center justify-center gap-2 text-sm min-h-[40px] border border-blue-400/20">
        ...
        <router-link to="/super-admin/tenants" class="flex-1 sm:flex-none px-3 sm:px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-all shadow-md hover:shadow-lg flex items-center justify-center gap-2 text-sm min-h-[40px] border border-amber-500/20">
    ...
    <div v-if="stats.pendingRequests > 0" class="mb-4 sm:mb-6 bg-gradient-to-r from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-800/20 border border-purple-300 dark:border-purple-700 rounded-lg p-3 sm:p-4 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
      <div class="flex items-start sm:items-center gap-3 w-full sm:w-auto">
        <div class="w-8 h-8 sm:w-10 sm:h-10 bg-purple-500 rounded-full flex items-center justify-center flex-shrink-0 shadow-md">
      ...
      <router-link to="/super-admin/tenants" class="w-full sm:w-auto px-3 sm:px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white rounded-lg transition-all shadow-md hover:shadow-lg text-xs sm:text-sm font-bold text-center min-h-[36px] sm:min-h-[40px] flex items-center justify-center border border-purple-500/20">
        عرض الطلبات
      </router-link>
    </div>

    <!-- Expiring Soon Alert -->
    <div v-if="stats.expiringSoon > 0" class="mb-4 sm:mb-6 bg-gradient-to-r from-amber-50 to-amber-100 dark:from-amber-900/20 dark:to-amber-800/20 border border-amber-300 dark:border-amber-700 rounded-lg p-3 sm:p-4 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
      <div class="flex items-start sm:items-center gap-3 w-full sm:w-auto">
        <div class="w-8 h-8 sm:w-10 sm:h-10 bg-amber-500 rounded-full flex items-center justify-center flex-shrink-0 shadow-md">
          <svg class="w-4 h-4 sm:w-5 sm:h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <router-link to="/super-admin/tenants" class="w-full sm:w-auto px-3 sm:px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white rounded-lg transition-all shadow-md hover:shadow-lg text-xs sm:text-sm font-bold text-center min-h-[36px] sm:min-h-[40px] flex items-center justify-center border border-amber-500/20">
        عرض الاشتراكات
      </router-link>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3 mb-4 sm:mb-6">
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-3 sm:p-4 border border-gray-300 dark:border-gray-600 hover:shadow-lg transition-all duration-300 hover:border-amber-400 dark:hover:border-amber-600">
        <div class="flex items-center justify-between">
          ...
          <div class="bg-blue-100 dark:bg-blue-900/30 rounded-full p-2 sm:p-2.5 flex-shrink-0 border border-blue-200 dark:border-blue-800">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-3 sm:p-4 border border-gray-300 dark:border-gray-600 hover:shadow-lg transition-all duration-300 hover:border-amber-400 dark:hover:border-amber-600">
        <div class="flex items-center justify-between">
          ...
          <div class="bg-green-100 dark:bg-green-900/30 rounded-full p-2 sm:p-2.5 flex-shrink-0 border border-green-200 dark:border-green-800">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-3 sm:p-4 border border-gray-300 dark:border-gray-600 hover:shadow-lg transition-all duration-300 hover:border-amber-400 dark:hover:border-amber-600">
        <div class="flex items-center justify-between">
          ...
          <div class="bg-amber-100 dark:bg-amber-900/30 rounded-full p-2 sm:p-2.5 flex-shrink-0 border border-amber-200 dark:border-amber-800">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-3 sm:p-4 border border-gray-300 dark:border-gray-600 hover:shadow-lg transition-all duration-300 hover:border-amber-400 dark:hover:border-amber-600">
        <div class="flex items-center justify-between">
          ...
          <div class="bg-red-100 dark:bg-red-900/30 rounded-full p-2 sm:p-2.5 flex-shrink-0 border border-red-200 dark:border-red-800">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-3 sm:p-4 border border-gray-300 dark:border-gray-600 hover:shadow-lg transition-all duration-300 hover:border-amber-400 dark:hover:border-amber-600 col-span-2 sm:col-span-1">
        <div class="flex items-center justify-between">
          <div class="min-w-0">
          ...
          <div class="bg-purple-100 dark:bg-purple-900/30 rounded-full p-2 sm:p-2.5 flex-shrink-0 border border-purple-200 dark:border-purple-800">
    ...
    <div class="card-warm p-3 sm:p-5 mb-4 sm:mb-6">
      ...
      <div class="mt-3 h-3 sm:h-4 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
      ...
      <div v-if="stats.totalTenants > 0" class="mt-2 text-xs text-gray-500 dark:text-gray-400 text-center">
        {{ stats.activeSubscriptionsPercentage.toFixed(0) }}% نشط • {{ stats.trialTenantsPercentage.toFixed(0) }}% تجريبي • {{ stats.expiredSubscriptionsPercentage.toFixed(0) }}% منتهي • {{ stats.cancelledSubscriptionsPercentage.toFixed(0) }}% ملغي
      </div>
    </div>

    <!-- Expiring Subscriptions - Desktop Table -->
    <div v-if="expiringTenants.length > 0" class="card-warm mb-4 sm:mb-6">
      <div class="p-3 sm:p-4 border-b border-gray-300 dark:border-gray-600 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 bg-gray-50/50 dark:bg-gray-700/30">
        <h3 class="text-sm sm:text-base font-semibold text-gray-900 dark:text-white">⚠️ اشتراكات على وشك الانتهاء</h3>
        <span class="badge badge-warning text-[10px] sm:text-xs px-2 py-0.5">
        ...
        <table class="w-full min-w-[400px]">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="tenant in expiringTenants" :key="tenant.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              ...
              <td class="px-3 sm:px-4 py-2 sm:py-3 whitespace-nowrap">
                <router-link to="/super-admin/tenants" class="text-amber-600 hover:text-amber-800 dark:text-amber-400 dark:hover:text-amber-300 text-xs sm:text-sm font-medium hover:underline">
                  تمديد
                </router-link>
              </td>
            </tr>
      ...
      <div class="sm:hidden divide-y divide-gray-200 dark:divide-gray-700">
        <div v-for="tenant in expiringTenants" :key="tenant.id" class="p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
          <div class="flex justify-between items-start mb-2">
            <span class="font-medium text-gray-900 dark:text-white text-sm">{{ tenant.name }}</span>
            <span :class="(tenant.daysLeft || 0) <= 3 ? 'text-red-600 font-bold' : 'text-amber-600'" class="text-sm">
              {{ tenant.daysLeft || 0 }} يوم
            </span>
          </div>
          <div class="flex justify-between items-center text-xs text-gray-500 dark:text-gray-400">
            <span>ينتهي: {{ formatDate(tenant.paid_until) }}</span>
            <router-link to="/super-admin/tenants" class="text-amber-600 dark:text-amber-400 font-medium hover:underline">تمديد</router-link>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="card-warm">
      <div class="p-3 sm:p-4 border-b border-gray-300 dark:border-gray-600 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 bg-gray-50/50 dark:bg-gray-700/30">
        ...
        <router-link to="/super-admin/tenants" class="text-xs sm:text-sm text-amber-600 dark:text-amber-400 hover:text-amber-700 dark:hover:text-amber-300 font-medium hover:underline">عرض الكل</router-link>
      </div>
        ...
        <table class="w-full min-w-[450px]">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
            <tr v-for="tenant in recentTenants" :key="tenant.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
              <td class="px-3 sm:px-4 py-2 sm:py-3 whitespace-nowrap font-medium text-gray-900 dark:text-white text-xs sm:text-sm">{{ tenant.name }}</td>
              <td class="px-3 sm:px-4 py-2 sm:py-3 whitespace-nowrap">
                <span class="px-1.5 sm:px-2 py-0.5 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded text-[10px] sm:text-xs border border-gray-200 dark:border-gray-600">{{ tenant.slug }}</span>
              </td>
              ...
              <td class="px-3 sm:px-4 py-2 sm:py-3 whitespace-nowrap">
                <span :class="getSubscriptionBadge(tenant.subscription_status)" class="px-1.5 sm:px-2 py-0.5 text-[10px] sm:text-xs rounded-full border">
      ...
      <!-- Mobile Cards -->
      <div class="sm:hidden divide-y divide-gray-200 dark:divide-gray-700">
        <div v-for="tenant in recentTenants" :key="tenant.id" class="p-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
          <div class="flex justify-between items-start mb-2">
            <div>
              <p class="font-medium text-gray-900 dark:text-white text-sm">{{ tenant.name }}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400">المعرف: {{ tenant.slug }}</p>
            </div>
            <span :class="getSubscriptionBadge(tenant.subscription_status)" class="px-2 py-0.5 text-xs rounded-full border">
              {{ formatSubscriptionStatus(tenant.subscription_status) }}
            </span>
          </div>
          <div class="flex justify-between text-xs text-gray-500 dark:text-gray-400">
            <span>المستخدمين: {{ formatNumber(tenant.userCount || 0) }}</span>
            <span>{{ formatDate(tenant.createdAt) }}</span>
          </div>
        </div>
        <div v-if="recentTenants.length === 0" class="p-6 text-center text-gray-500 dark:text-gray-400 text-sm">
          لا توجد مستأجرين
        </div>
      </div>
    </div>
...
const getSubscriptionBadge = (status?: string): string => {
  if (status === 'active') return 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 border-green-200 dark:border-green-800'
  if (status === 'trial') return 'bg-amber-100 dark:bg-amber-900/30 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-800'
  if (status === 'expired') return 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 border-red-200 dark:border-red-800'
  if (status === 'cancelled') return 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300 border-gray-200 dark:border-gray-700'
