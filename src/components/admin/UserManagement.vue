    ...
    <div v-else class="user-management__content">
      ...
      <div class="page-header">
        <div>
          ...
          <p class="page-header__subtitle">قم بإدارة المستخدمين في نظامك. يمكنك إضافة مدراء مستودعات ومستخدمين للعرض فقط.</p>
        </div>
      ...
      <div v-if="activeForm === 'manager'" class="form-section form-section--manager">
        <div class="form-section__header">
          ...
          <div>
            ...
            <p class="form-section__subtitle">قم بإضافة مدير مستودع جديد وتعيين المستودعات المسموح بها</p>
          </div>
        </div>
        ...
        <form @submit.prevent="createWarehouseManager" class="form-section__body">
          <div class="form-grid">
            <div class="form-group">
              ...
              <input type="text" v-model="warehouseManagerForm.name" required class="form-input" placeholder="أدخل اسم مدير المستودع" />
            </div>
            <div class="form-group">
              <label class="form-label">البريد الإلكتروني <span class="text-red-500">*</span></label>
              <input type="email" v-model="warehouseManagerForm.email" required class="form-input" placeholder="example@company.com" />
            </div>
            <div class="form-group">
              <label class="form-label">كلمة المرور المؤقتة <span class="text-red-500">*</span></label>
              <input type="password" v-model="warehouseManagerForm.password" required class="form-input" placeholder="كلمة مرور مؤقتة للمستخدم" />
              <p class="form-hint">سيتمكن المستخدم من تغيير كلمة المرور بعد تسجيل الدخول</p>
            </div>
            <div class="form-group">
              <label class="form-label">المستودعات الرئيسية المسموح بها <span class="text-red-500">*</span></label>
              <select v-model="warehouseManagerForm.allowedWarehouses" multiple class="form-select" size="3">
                <option v-for="warehouse in primaryWarehouses" :key="warehouse.id" :value="warehouse.id">
                  {{ warehouse.name_ar || warehouse.name }}
                </option>
              </select>
              <p class="form-hint">اضغط مع الاستمرار على Ctrl (أو Cmd) لتحديد عدة مستودعات</p>
            </div>
            <div class="form-group">
              <label class="form-label">مواقع الصرف المسموح بها</label>
              <select v-model="warehouseManagerForm.allowedDispatchWarehouses" multiple class="form-select" size="3">
                <option v-for="warehouse in dispatchWarehouses" :key="warehouse.id" :value="warehouse.id">
                  {{ warehouse.name_ar || warehouse.name }}
                </option>
              </select>
              <p class="form-hint">اختر مواقع الصرف التي يمكن لمدير المستودع الصرف منها</p>
            </div>
          </div>
          <div class="form-actions">
            <button type="submit" :disabled="isCreating" class="btn btn--primary btn--md">
      ...
      <div v-if="activeForm === 'viewer'" class="form-section form-section--viewer">
        <div class="form-section__header">
          ...
          <div>
            ...
            <p class="form-section__subtitle">مستخدم يمكنه الاطلاع على البيانات فقط دون صلاحية التعديل</p>
          </div>
        </div>
        ...
        <form @submit.prevent="createViewer" class="form-section__body">
          <div class="form-grid">
            <div class="form-group">
              <label class="form-label">الاسم <span class="text-red-500">*</span></label>
              <input type="text" v-model="viewerForm.name" required class="form-input" placeholder="أدخل اسم المستخدم" />
            </div>
            <div class="form-group">
              <label class="form-label">البريد الإلكتروني <span class="text-red-500">*</span></label>
              <input type="email" v-model="viewerForm.email" required class="form-input" placeholder="viewer@company.com" />
            </div>
            <div class="form-group">
              <label class="form-label">كلمة المرور المؤقتة <span class="text-red-500">*</span></label>
              <input type="password" v-model="viewerForm.password" required class="form-input" placeholder="كلمة مرور مؤقتة للمستخدم" />
              <p class="form-hint">سيتمكن المستخدم من تغيير كلمة المرور بعد تسجيل الدخول</p>
            </div>
            <div class="form-group">
              <label class="form-label">المستودعات المسموح بعرضها <span class="text-red-500">*</span></label>
              <select v-model="viewerForm.allowedWarehouses" multiple class="form-select" size="3">
                <option v-for="warehouse in primaryWarehouses" :key="warehouse.id" :value="warehouse.id">
                  {{ warehouse.name_ar || warehouse.name }}
                </option>
              </select>
              <p class="form-hint">اضغط مع الاستمرار على Ctrl (أو Cmd) لتحديد عدة مستودعات. المستخدم سيرى فقط البيانات من هذه المستودعات.</p>
            </div>
          </div>
          <div class="form-actions">
            <button type="submit" :disabled="isCreatingViewer" class="btn btn--secondary btn--md">
      ...
      <div class="users-section">
        <div class="users-section__header">
          <div>
            ...
            <p class="users-section__subtitle">جميع المستخدمين في نظامك</p>
          </div>
          ...
          <table class="users-table">
            <thead>
              <tr>
                ...
                <th class="hide-tablet">مواقع الصرف</th>
                <th>الحالة</th>
                <th>إجراءات</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="user in users" :key="user.id" class="users-table__row">
                ...
                <td>
                  <button @click="toggleUserStatus(user)" :class="['status-toggle', user.is_active ? 'status-toggle--active' : 'status-toggle--inactive']">
                    {{ user.is_active ? 'نشط' : 'غير نشط' }}
                  </button>
                </td>
                <td>
                  <div class="action-buttons">
                    <button v-if="user.role === 'warehouse_manager' || user.role === 'viewer'" @click="editUserWarehouses(user)" class="action-btn action-btn--edit" title="تعديل المستودعات">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                      </svg>
                    </button>
                    <button v-if="user.role !== 'superadmin' && user.id !== authStore.user?.id" @click="deleteUser(user)" class="action-btn action-btn--delete" title="حذف المستخدم">
                      <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-for="user in users" :key="user.id" class="user-card">
            ...
            <div class="user-card__body">
              <div class="user-card__detail">
                ...
                <span class="user-card__value">{{ user.allowed_warehouses?.length || 0 }}</span>
              </div>
              <div class="user-card__detail">
                <span class="user-card__label">مواقع الصرف</span>
                <span class="user-card__value">{{ user.allowed_dispatch_warehouses?.length || 0 }}</span>
              </div>
              <div class="user-card__detail">
            ...
            <div class="user-card__actions">
              <button v-if="user.role === 'warehouse_manager' || user.role === 'viewer'" @click="editUserWarehouses(user)" class="btn btn--outline btn--sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                </svg>
                صلاحيات
              </button>
              <button v-if="user.role !== 'superadmin' && user.id !== authStore.user?.id" @click="deleteUser(user)" class="btn btn--danger btn--sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                </svg>
                حذف
      ...
      <div v-if="showEditWarehousesModal" class="modal-overlay" @click.self="showEditWarehousesModal = false">
        <div class="modal">
          ...
          <div class="modal__body">
            ...
            <div class="space-y-4">
              <div class="form-group">
                ...
                <select v-model="editForm.allowedWarehouses" multiple class="form-select" size="4">
              ...
              <div v-if="editingUser?.role === 'warehouse_manager'" class="form-group">
                <label class="form-label">مواقع الصرف المسموح بها</label>
                <select v-model="editForm.allowedDispatchWarehouses" multiple class="form-select" size="3">
          ...
          <div class="modal__footer">
            <button @click="showEditWarehousesModal = false" class="btn btn--outline btn--md">إلغاء</button>
...
// ===== ALL ORIGINAL FUNCTIONS PRESERVED - NO CHANGES =====
...
.users-section__subtitle {
  font-size: 0.75rem;
  color: #6b7280;
}

.dark .users-section__subtitle {
  color: #9ca3af;
}
