      ...
      <div class="flex min-h-full items-center justify-center p-4">
        ...
        >
          ...
          <div v-if="isLoading" class="p-8 text-center">
            ...
            <p class="mt-3 text-sm font-medium text-gray-600 dark:text-gray-400">جاري تحميل بيانات الفاتورة...</p>
          </div>

          <!-- Body -->
          <div v-else-if="invoiceData" class="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
            <!-- Invoice Summary -->
            <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 mb-6">
              <div class="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <p class="text-sm font-bold text-gray-900 dark:text-white mt-0.5">{{ invoiceData.invoice_number || '—' }}</p>
                </div>
                <div>
                ...
                <div class="col-span-2">
                  ...
                  <p class="text-sm font-bold text-gray-900 dark:text-white mt-0.5">{{ invoiceData.customer?.name || '—' }}</p>
                  <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ invoiceData.customer?.phone || '' }}</p>
                </div>
                <div class="col-span-2">
                ...
                <div class="col-span-2">
                  <p class="text-xs font-medium text-gray-500 dark:text-gray-400">حالة الفاتورة</p>
                  <span :class="getStatusBadge(invoiceData.status)" class="px-2 py-0.5 text-xs font-bold rounded-full inline-block mt-0.5">
            ...
            <!-- Items Summary -->
            <div v-if="invoiceData.items && invoiceData.items.length > 0" class="mb-6">
              <div class="flex items-center justify-between mb-2">
                <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">
                  الأصناف ({{ invoiceData.items.length }})
                </p>
              ...
              <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3 max-h-32 overflow-y-auto">
                ...
                >
                  ...
                  <div class="text-right flex-shrink-0 mr-2">
                    ...
                    <span v-else class="text-xs text-red-500">⚠️ مخزن غير محدد</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <!-- Error State -->
          <div v-else-if="!isLoading && !invoiceData" class="p-8 text-center">
            <svg class="w-16 h-16 mx-auto text-gray-400 dark:text-gray-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
            </svg>
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2">فاتورة غير موجودة</h3>
            <p class="text-gray-600 dark:text-gray-400 mb-4">لم نتمكن من العثور على بيانات الفاتورة</p>
            <button @click="handleClose" class="px-6 py-2 bg-gray-600 hover:bg-gray-700 text-white rounded-lg">إغلاق</button>
          </div>

          <!-- Footer -->
          <div v-if="invoiceData" class="bg-gray-50 dark:bg-gray-700/50 px-6 py-4 flex gap-3 rounded-b-2xl">
            <button
              @click="handleClose"
...
watch(() => props.isOpen, async (isOpen) => {
  if (isOpen && props.invoiceId) {
    ...
    try {
      const invoice = await invoiceStore.getInvoiceById(props.invoiceId)
...
const formattedDate = computed(() => {
  if (!invoiceData.value?.invoice_date) return '—'
  const date = new Date(invoiceData.value.invoice_date)
...
const formattedTotal = computed(() => {
  ...
  const currency = invoiceData.value.currency || 'EGP'
  return new Intl.NumberFormat('en-US', { 
    style: 'currency', 
    currency: currency,
    minimumFractionDigits: 2,
    maximumFractionDigits: 2
  }).format(invoiceData.value.total_amount || 0)
})

const totalItemsQuantity = computed(() => {
  if (!invoiceData.value?.items) return 0
  return invoiceData.value.items.reduce((sum: number, item: any) => sum + (item.quantity || 0), 0)
})

const uniqueWarehousesCount = computed(() => {
  if (!invoiceData.value?.items) return 0
  const warehouses = new Set<string>()
  invoiceData.value.items.forEach((item: any) => {
...
const hasUnknownWarehouse = computed(() => {
  if (!invoiceData.value?.items) return false
  return invoiceData.value.items.some((item: any) => !item.warehouse_id)
})
...
const handleDelete = async (): Promise<void> => {
  if (isProcessing.value || !props.invoiceId) return
