...
const downloadPDF = async () => {
  ...
  try {
    ...
    if (!invoice) {
      ...
      await PDFExportService.generateInvoicePDF(minimalInvoice, companyInfo.value)
    } else {
      await PDFExportService.generateInvoicePDF(invoice, companyInfo.value)
    }
...
const printInvoice = () => {
  ...
  // Generate clean HTML for printing without large SVG icons
  const companyName = companyInfo.value.name || 'P.commerce'
  const companyTaxNumber = companyInfo.value.taxNumber || '—'
  const companyAddress = companyInfo.value.address || '—'
  const companyPhone = companyInfo.value.phone || '—'
  const companyEmail = companyInfo.value.email || '—'
  const companyLogo = companyInfo.value.logoUrl || ''

  const logoHTML = companyLogo 
    ? `<div style="text-align: center; margin-bottom: 8px;"><img src="${companyLogo}" alt="شعار الشركة" style="max-height: 50px; width: auto; border-radius: 50%;" /></div>` 
    : ''

  const itemsHTML = (props.formData.items || []).map((item: any, idx: number) => `
  ...
  const htmlContent = `
    <div style="direction: rtl; font-family: 'Cairo', Arial, sans-serif; padding: 30px; max-width: 1100px; margin: 0 auto; background: white; font-size: 12px;">
      <div style="text-align: center; margin-bottom: 20px; padding-bottom: 15px; border-bottom: 2px solid #2F75B5;">
        ${logoHTML}
        <h1 style="font-size: 22px; font-weight: bold; color: #1f2937; margin-bottom: 4px;">فاتورة ضريبية</h1>
        <div style="color: #4b5563; font-size: 12px;">
          رقم الفاتورة: <span style="font-weight: bold; color: #1f2937;">${invoiceNumber.value}</span> &nbsp;|&nbsp;
          التاريخ: <span style="font-weight: bold; color: #1f2937;">${currentDate.value}</span>
        </div>
      </div>
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
        <div style="border: 1px solid #e5e7eb; border-radius: 6px; padding: 12px; background-color: #f9fafb;">
          ...
          ${companyLogo ? `<div style="text-align: center; margin-bottom: 6px;"><img src="${companyLogo}" alt="شعار الشركة" style="max-height: 40px; width: auto; border-radius: 50%;" /></div>` : ''}
        ...
        <div style="border: 1px solid #e5e7eb; border-radius: 6px; padding: 12px; background-color: #f9fafb;">
          ...
          <p style="margin: 3px 0; color: #4b5563; font-size: 11px;"><strong>${props.formData.customer.name || '—'}</strong></p>
          <p style="margin: 3px 0; color: #4b5563; font-size: 11px;">هاتف: ${props.formData.customer.phone || '—'}</p>
          <p style="margin: 3px 0; color: #4b5563; font-size: 11px;">البريد الإلكتروني: ${props.formData.customer.email || '—'}</p>
          <p style="margin: 3px 0; color: #4b5563; font-size: 11px;">العنوان: ${props.formData.customer.address || '—'}</p>
          <p style="margin: 3px 0; color: #4b5563; font-size: 11px;">الرقم الضريبي: ${props.formData.customer.tax_number || '—'}</p>
        </div>
      </div>
        ...
        <div style="width: 100%; max-width: 350px;">
          <table style="width: 100%; border-collapse: collapse; font-size: 11px;">
            <tr>
              ...
              <td style="padding: 4px 10px; text-align: left; font-weight: 600; color: #1f2937; border-bottom: 1px solid #e5e7eb;">${formatCurrency(props.calculations.subtotal)}</td>
            </tr>
            ${props.formData.discount_value > 0 ? `
            <tr>
              <td style="padding: 4px 10px; text-align: right; font-weight: bold; color: #4b5563; border-bottom: 1px solid #e5e7eb;">الخصم (${props.formData.discount_value} ${props.formData.discount_type === 'percentage' ? '%' : props.selectedCurrency})</td>
              <td style="padding: 4px 10px; text-align: left; font-weight: 600; color: #dc2626; border-bottom: 1px solid #e5e7eb;">-${formatCurrency(props.calculations.discountAmount)}</td>
            </tr>` : ''}
            ${props.formData.shipping_cost > 0 ? `
            <tr>
              <td style="padding: 4px 10px; text-align: right; font-weight: bold; color: #4b5563; border-bottom: 1px solid #e5e7eb;">الشحن</td>
              <td style="padding: 4px 10px; text-align: left; font-weight: 600; color: #1f2937; border-bottom: 1px solid #e5e7eb;">${formatCurrency(props.formData.shipping_cost)}</td>
            </tr>` : ''}
            <tr>
              <td style="padding: 4px 10px; text-align: right; font-weight: bold; color: #4b5563; border-bottom: 1px solid #e5e7eb;">الضريبة (${props.formData.vat_rate || 0}%)</td>
              <td style="padding: 4px 10px; text-align: left; font-weight: 600; color: #1f2937; border-bottom: 1px solid #e5e7eb;">${formatCurrency(props.calculations.vatAmount)}</td>
            </tr>
            <tr style="background-color: #f0fdf4; border-top: 2px solid #16a34a;">
              ...
              <td style="padding: 5px 10px; text-align: left; font-weight: 800; font-size: 15px; color: #16a34a;">${formatCurrency(props.calculations.totalAmount)}</td>
            </tr>
          </table>
        </div>
      </div>
      ...
      <div style="margin-top: 15px; padding: 10px; border: 1px solid #e5e7eb; border-radius: 6px; background: #f9fafb; font-size: 11px;">
        ...
        <p style="color: #4b5563;">${props.formData.notes}</p>
      </div>` : ''}
      ${props.formData.terms ? `
      <div style="margin-top: 8px; padding: 10px; border: 1px solid #e5e7eb; border-radius: 6px; background: #f9fafb; font-size: 11px;">
        ...
        <p style="color: #4b5563;">${props.formData.terms}</p>
      </div>` : ''}
