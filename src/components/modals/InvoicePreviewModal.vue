...
const exportToExcel = async () => {
  ...
  try {
    ...
    const exportData = {
      ...
      'اسم العميل': props.formData.customer.name || '—',
      'هاتف العميل': props.formData.customer.phone || '—',
      'البريد الإلكتروني للعميل': props.formData.customer.email || '—',
      'عنوان العميل': props.formData.customer.address || '—',
      'الرقم الضريبي للعميل': props.formData.customer.tax_number || '—',
      'المجموع الفرعي': props.calculations.subtotal,
      'الخصم': props.calculations.discountAmount,
      'الشحن': props.formData.shipping_cost || 0,
      'الضريبة': props.calculations.vatAmount,
      'الإجمالي': props.calculations.totalAmount,
      'العملة': props.selectedCurrency,
      'الحالة': getStatusText(props.formData.status),
      'اسم الشركة': companyInfo.value.name || 'P.commerce',
    ...
    // Create workbook with invoice details
    const wb = XLSX.utils.book_new()
    
    // Main invoice data
    const wsData = XLSX.utils.json_to_sheet([exportData])
    XLSX.utils.book_append_sheet(wb, wsData, 'الفاتورة')
    
    // Items sheet
    const itemsData = props.formData.items.map((item: any, idx: number) => ({
      'رقم': idx + 1,
      'الصنف': item.name,
      'الكود': item.code || '—',
      'المقاس': item.size || '—',
    ...
    // Save file
    const fileName = `فاتورة_${invoiceNumber.value}_${new Date().toISOString().split('T')[0]}.xlsx`
