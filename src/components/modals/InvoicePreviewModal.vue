        ...
        <div class="flex flex-wrap justify-between items-center gap-2 p-3 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
          ...
          <div class="flex flex-wrap gap-1.5">
            ...
            <button 
              ...
              class="px-3 py-1.5 sm:px-4 sm:py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg text-xs sm:text-sm transition-colors min-h-[44px] flex items-center gap-1.5 disabled:opacity-50"
            >
              <svg v-if="isSharingWhatsApp" class="w-4 h-4 animate-spin" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
              </svg>
              <svg v-else class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <span class="hidden xs:inline">{{ isSharingWhatsApp ? 'جاري الإرسال...' : 'واتساب' }}</span>
              <span class="xs:hidden">{{ isSharingWhatsApp ? '...' : 'واتساب' }}</span>
            </button>
            <button 
              @click="printInvoice" 
        ...
        <div class="overflow-y-auto flex-1 p-3 sm:p-6 pb-24 sm:pb-6" id="invoice-preview-area" ref="previewAreaRef">
          <div class="print-invoice max-w-4xl mx-auto" id="print-content">
            ...
            <div class="text-center mb-4 sm:mb-6">
              ...
              <div v-if="companyInfo.logoUrl" class="flex justify-start mb-3">
                <img 
                  ...
                  alt="شعار الشركة" 
                  class="h-14 w-14 sm:h-20 sm:w-20 rounded-full object-cover border-2 border-gray-200 dark:border-gray-600 shadow-sm"
                  @error="handleLogoError"
                />
              </div>
              <div v-else class="flex justify-start mb-3">
                <div class="h-14 w-14 sm:h-20 sm:w-20 rounded-full bg-gradient-to-r from-amber-600 to-green-600 flex items-center justify-center shadow-sm">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <!-- Company Card -->
              <div class="border border-gray-200 dark:border-gray-700 rounded-lg p-4 bg-gray-50 dark:bg-gray-900/30 print:bg-gray-50">
                <div class="flex items-center gap-2 mb-3 pb-2 border-b border-gray-200 dark:border-gray-700 print:border-gray-300">
                ...
                <table class="w-full text-xs sm:text-sm border-collapse">
                  <thead>
                    <tr class="bg-gray-800 dark:bg-gray-700 text-white print:bg-gray-800">
                      <th class="px-4 py-2.5 text-right font-bold w-2/5">الصنف</th>
                      <th class="px-4 py-2.5 text-center font-bold w-1/6">الكمية</th>
                      <th class="px-4 py-2.5 text-center font-bold w-1/5">سعر الوحدة</th>
                      <th class="px-4 py-2.5 text-center font-bold w-1/5">الإجمالي</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="(item, idx) in props.formData.items" :key="idx" class="border-b border-gray-200 dark:border-gray-700 print:border-gray-300">
                      ...
                      <td class="px-4 py-2.5 text-center text-gray-800 dark:text-gray-200">{{ item.quantity }}</td>
                      <td class="px-4 py-2.5 text-center text-gray-800 dark:text-gray-200">{{ formatCurrency(item.unit_price) }}</td>
                      <td class="px-4 py-2.5 text-center font-semibold text-gray-800 dark:text-gray-200">{{ formatCurrency(item.total) }}</td>
                    </tr>
                  </tbody>
                  <tfoot class="bg-gray-50 dark:bg-gray-800/50 print:bg-gray-50">
                    <tr>
                      <td colspan="3" class="px-4 py-2 text-left font-bold text-gray-700 dark:text-gray-300">المجموع الفرعي</td>
                      <td class="px-4 py-2 text-center font-bold text-gray-800 dark:text-gray-200">{{ formatCurrency(props.calculations.subtotal) }}</td>
                    </tr>
                    <tr v-if="props.formData.discount_value > 0">
                      <td colspan="3" class="px-4 py-2 text-left font-bold text-gray-700 dark:text-gray-300">الخصم ({{ props.formData.discount_value }} {{ props.formData.discount_type === 'percentage' ? '%' : selectedCurrency }})</td>
                      <td class="px-4 py-2 text-center font-bold text-red-600 dark:text-red-400">-{{ formatCurrency(props.calculations.discountAmount) }}</td>
                    </tr>
                    <tr v-if="props.formData.shipping_cost > 0">
                      <td colspan="3" class="px-4 py-2 text-left font-bold text-gray-700 dark:text-gray-300">الشحن</td>
                      <td class="px-4 py-2 text-center font-bold text-gray-800 dark:text-gray-200">{{ formatCurrency(props.formData.shipping_cost) }}</td>
                    </tr>
                    <tr>
                      <td colspan="3" class="px-4 py-2 text-left font-bold text-gray-700 dark:text-gray-300">الضريبة ({{ props.formData.vat_rate }}%)</td>
                      <td class="px-4 py-2 text-center font-bold text-gray-800 dark:text-gray-200">{{ formatCurrency(props.calculations.vatAmount) }}</td>
                    </tr>
                    <tr class="bg-gray-100 dark:bg-gray-700/50 print:bg-gray-100">
                      <td colspan="3" class="px-4 py-2.5 text-left font-bold text-gray-800 dark:text-white text-base">الإجمالي النهائي</td>
                      <td class="px-4 py-2.5 text-center font-bold text-green-600 dark:text-green-400 text-base">{{ formatCurrency(props.calculations.totalAmount) }}</td>
                    </tr>
                  </tfoot>
                </table>
              </div>
            ...
            <div v-if="props.formData.notes || props.formData.terms" class="grid grid-cols-1 gap-2 mb-4 text-xs sm:text-sm">
              <div v-if="props.formData.notes" class="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-900/30 print:bg-gray-50 print:border-gray-300">
              ...
              <div v-if="props.formData.terms" class="border border-gray-200 dark:border-gray-700 rounded-lg p-3 bg-gray-50 dark:bg-gray-900/30 print:bg-gray-50 print:border-gray-300">
            ...
            <!-- Footer -->
            <div class="text-center text-[10px] text-gray-400 dark:text-gray-500 mt-4 pt-3 border-t border-gray-200 dark:border-gray-700 print:border-gray-300">
            ...
            <!-- Signatures -->
            <div class="grid grid-cols-2 gap-4 pt-4 border-t-2 border-gray-300 dark:border-gray-600 mt-4 print:border-gray-400">
              <div class="text-center">
                <div class="border-t-2 border-gray-400 dark:border-gray-500 pt-2 mt-6">
...
const close = () => {
  if (!isGeneratingPDF.value && !isSharingWhatsApp.value) {
    emit('close')
  }
}
...
const downloadPDF = async () => {
  ...
  try {
    ...
    link.href = url
    link.download = `فاتورة_${invoiceNumber.value}.pdf`
    link.click()
...
const shareWhatsApp = async () => {
  ...
  if (props.formData.items.length === 0) {
    showToast('لا توجد أصناف في الفاتورة للمشاركة', 'error')
    return
  }
  ...
  try {
    showToast('جاري تحضير الفاتورة للمشاركة...', 'success')
    
    // Generate PDF
    const blob = await generatePDFBlob()
    if (!blob) {
      showToast('حدث خطأ في تحضير الفاتورة', 'error')
      return
    }
    
    // Create a file from the blob
    const file = new File([blob], `فاتورة_${invoiceNumber.value}.pdf`, { type: 'application/pdf' })
    
    // Check if the Web Share API is available (for mobile)
    if (navigator.share) {
      try {
        await navigator.share({
          title: `فاتورة ${invoiceNumber.value}`,
          text: `مرحباً ${props.formData.customer.name || 'العميل'},\n\nنشكرك على تعاملك معنا.\n\nرقم الفاتورة: ${invoiceNumber.value}\nالتاريخ: ${currentDate.value}\nالإجمالي: ${formatCurrency(props.calculations.totalAmount)}\n\nشكراً لثقتكم بنا.`,
          files: [file]
        })
        showToast('تم مشاركة الفاتورة بنجاح', 'success')
        isSharingWhatsApp.value = false
        return
      } catch (shareError: any) {
        // If user cancels or share fails, fallback to WhatsApp URL
    ...
    // Fallback: Upload to a temporary URL and share via WhatsApp
    // First, try to upload the PDF to a temporary storage (like a free file hosting)
    // Since we can't upload to a server, we'll use the Data URL approach for WhatsApp
    ...
    reader.onload = function(e) {
      ...
      // For WhatsApp, we need to send the PDF as a document
      // Since WhatsApp Web/App doesn't support direct PDF sharing via URL,
      // we'll send a message with the invoice details and offer to share the PDF file
      
      const message = `مرحباً ${props.formData.customer.name || 'العميل'},\n\nنشكرك على تعاملك معنا.\n\n📄 فاتورة: ${invoiceNumber.value}\n📅 التاريخ: ${currentDate.value}\n💰 الإجمالي: ${formatCurrency(props.calculations.totalAmount)}\n\nيمكنك تحميل الفاتورة من خلال الرابط أدناه:\n\n📎 [فاتورة ${invoiceNumber.value}](${dataUrl})\n\nشكراً لثقتكم بنا.`
