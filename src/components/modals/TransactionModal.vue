...
// ✅ FIXED: submit function with correct types
const submit = async () => {
  ...
  try {
    // ✅ CRITICAL: Convert quantity to number explicitly
    const quantity = Number(form.value.quantity)
    
    // ✅ Pass transactionId as the LAST parameter (12th parameter)
    const result = await transactionStore.addTransaction(
      ...
      props.itemSize,
      props.warehouseId,
      transactionId          // ✅ string - 12th parameter
    )
