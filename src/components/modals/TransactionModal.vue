    ...
    <div v-if="isOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center z-50 p-4" @click.self="close">
      <div class="bg-white dark:bg-gray-800 rounded-t-2xl sm:rounded-2xl shadow-xl w-full sm:max-w-md md:max-w-lg transition-all duration-300 transform animate-slide-up" :class="isMobile ? 'max-h-[90vh]' : 'max-h-[85vh]'">
        ...
        <div class="overflow-y-auto p-5" :class="isMobile ? 'max-h-[calc(90vh-120px)]' : 'max-h-[calc(85vh-120px)]'">
          ...
          <div v-if="alreadyProcessed" class="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3 mb-4">
            <p class="text-sm font-bold text-yellow-700 dark:text-yellow-300">⚠️ تم معالجة هذه العملية مسبقاً</p>
            <p class="text-xs text-yellow-600 dark:text-yellow-400">الكمية تم إضافتها/صرفها بالفعل</p>
          </div>
          ...
          <form @submit.prevent="submit" class="space-y-4">
            ...
            <div class="flex gap-3 pt-4 sticky bottom-0 bg-white dark:bg-gray-800 pb-2">
              ...
              <button 
                type="submit" 
                :disabled="!canSubmit"
                class="flex-1 px-4 py-3 bg-amber-700 hover:bg-amber-800 text-white rounded-xl font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-md min-h-[48px] flex items-center justify-center"
              >
...
// Focus quantity when modal opens
...
const submit = async () => {
  ...
  try {
    ...
    if (result.alreadyProcessed) {
      ...
      showToast(`⚠️ تم معالجة هذه العملية مسبقاً (تم ${form.value.type === 'IN' ? 'إضافة' : 'صرف'} ${form.value.quantity} وحدة بالفعل)`, 'error')
    ...
    if (result.success) {
      showToast(`✅ تم ${form.value.type === 'IN' ? 'إضافة' : 'صرف'} ${form.value.quantity} وحدة بنجاح (الإذن: ${autoVoucher.value})`, 'success')
    ...
    if (error.message?.includes('network') || error.message?.includes('timeout') || error.message?.includes('Failed to fetch')) {
      ...
      if (retryCount < MAX_RETRIES) {
        ...
        showToast(`⚠️ انقطع الاتصال بالشبكة. جاري إعادة المحاولة (${retryCount}/${MAX_RETRIES})...`, 'error')
