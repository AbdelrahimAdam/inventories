          ...
          <div v-else-if="result">
            ...
            <div class="grid grid-cols-2 gap-3 mb-4">
              <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-3 text-center">
                ...
                <p class="text-sm font-bold text-gray-900 dark:text-white mt-0.5">{{ lastTransactionDate || '—' }}</p>
            ...
            <div class="space-y-3 bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4">
              ...
              <div class="border-t border-gray-200 dark:border-gray-600 pt-3">
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    <p class="text-xs text-gray-500 dark:text-gray-400">الكمية المضافة الحالية</p>
                    <p class="text-sm font-semibold">{{ formatNumber(result.current_added) }}</p>
                  </div>
                  <div>
                    <p class="text-xs text-gray-500 dark:text-gray-400">الكمية المضافة المحسوبة</p>
                    <p class="text-sm font-semibold">{{ formatNumber(result.calculated_added) }}</p>
                  </div>
                </div>
              </div>
...
const lastTransactionDate = ref<string | null>(null)
...
const loadItemDetails = async () => {
  try {
    // Find item in store to get photo
    ...
    const transactions = inventoryStore.transactions.filter(tx => 
      tx.itemCode === props.itemCode
    )
    ...
    if (transactions.length > 0) {
      ...
      lastTransactionDate.value = new Date(lastTx.createdAt).toLocaleDateString('ar-EG', {
        year: 'numeric',
        month: 'short',
        day: 'numeric'
      })
      ...
      const days = Math.floor((Date.now() - new Date(lastTx.createdAt).getTime()) / (1000 * 60 * 60 * 24))
      daysSinceLastTransaction.value = days
    } else {
      lastTransactionDate.value = 'لا توجد حركات'
      daysSinceLastTransaction.value = 999
    }

    // Calculate movement percentage
    // Movement = total_in + total_out / (total_in + total_out + current_balance) * 100
    if (result.value) {
      const totalMovement = result.value.total_in + result.value.total_out
      const totalAvailable = totalMovement + result.value.current_balance
...
watch(() => props.isOpen, (open) => {
  if (open) {
    ...
    lastTransactionDate.value = null
    daysSinceLastTransaction.value = 0
    movementPercentage.value = 0
    loadVerification()
  } else {
    toastVisible.value = false
    if (toastTimer) {
