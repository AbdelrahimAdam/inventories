<template>
  <header 
    class="bg-amber-700 dark:bg-amber-800 shadow-md sticky top-0 z-30 transition-colors duration-200 safe-top" 
  ...
  >
    <div class="px-2 sm:px-4 py-1.5 sm:py-3 flex justify-between items-center">
      ...
      <div class="flex items-center gap-1 sm:gap-3 flex-shrink-0">
        ...
        <!-- Language Switcher - Inline with language store -->
        <div ref="languageMenuRef" class="relative">
          ...
          >
            <span class="flex items-center gap-1">
              <span>{{ languageStore.current === 'en' ? '🇬🇧' : '🇸🇦' }}</span>
              <span>{{ languageStore.current === 'en' ? 'EN' : 'ع' }}</span>
            </span>
          </button>
          <div 
          ...
          >
            <button 
              ...
              class="w-full px-4 py-2 text-sm text-right hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors text-gray-700 dark:text-gray-300 flex items-center gap-2"
              :class="isRTL ? 'justify-end' : 'justify-start'"
            >
              <span>🇬🇧</span>
              <span>English</span>
              <svg v-if="languageStore.current === 'en'" class="w-4 h-4 text-amber-600 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            >
              <span>🇸🇦</span>
              <span>العربية</span>
              <svg v-if="languageStore.current === 'ar'" class="w-4 h-4 text-amber-600 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
              </svg>
            </button>
          </div>
