...
import { supabaseService as authStore } from '@/stores/auth'
...
class SupabaseService {
  ...
  private subscriptionCheckInterval: number = 300000 // 5 minutes
  ...
  async checkSubscriptionStatus(force: boolean = false): Promise<SubscriptionStatus | null> {
    ...
    // Use cached status if within TTL and not forced
  ...
  private handleQueryError(error: any): void {
    ...
    // Check for subscription/permission errors
    if (
      code === '42501' ||
      ...
      // Check if actually expired or just a permission issue
  ...
  // ✅ NEW: Force refresh subscription status for current user
