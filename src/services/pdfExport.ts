...
export class PDFExportService {
  static async generateInvoicePDF(
    ...
    try {
      // Give the browser enough time to render the inserted HTML.
      ...
      // Wait for browser fonts when supported.
      ...
      // Wait for every image inside the invoice.
      ...
      // Make sure the browser has painted the final layout.
      await new Promise<void>((resolve) => {
        requestAnimationFrame(() => {
          requestAnimationFrame(() => {
            resolve()
      ...
      if (error instanceof Error) {
        console.error('PDF generation details:', error.message)
      }
  ...
  static generateInvoiceHTML(
    ...
    const itemsRows = (invoice.items || [])
      .map((item: any, idx: number) => {
        const codeHtml = item.code
          ? `<div style="font-size:9px;color:#6b7280;margin-top:2px;">الكود: ${escapeHtml(item.code)}</div>`
          : ''

        const sizeHtml = item.size
          ? `<div style="font-size:9px;color:#6b7280;margin-top:2px;">المقاس: ${escapeHtml(item.size)}</div>`
          : ''
    ...
    const discountHtml =
      Number(invoice.discount_value) > 0
        ? `
          <tr>
            <td>الخصم (${escapeHtml(invoice.discount_value)} ${
      ...
      >
        ...
        <div class="pdf-header avoid-break">
          ...
          <div class="pdf-header-info">
            ...
            <div class="pdf-header-subtitle">
              رقم الفاتورة:
              <span dir="ltr">
                ${escapeHtml(invoice.invoice_number || 'INV-001')}
              </span>
        ...
        <div class="info-grid avoid-break">
          ...
          <div class="info-box">
            ...
            <p>
              الرقم الضريبي:
              ${escapeHtml(invoice.customer?.tax_number || '—')}
            </p>
          ...
          <table>
            ...
            <thead>
              ...
              <tr>
                ...
                <th style="width:38%;text-align:right;">
                  الصنف
                </th>
        ...
        <div class="totals avoid-break">
          ...
          <table class="totals-table">
            ...
            <tr>
              <td>
                الضريبة (${escapeHtml(invoice.vat_rate || 0)}%)
              </td>
        ...
        <div class="footer">
          ...
          <p>
            تم الإنشاء في:
            ${escapeHtml(new Date().toLocaleString('ar-EG'))}
          </p>
