...
export class SingleInvoiceExportService {
  static async exportSingleInvoice(invoice: any): Promise<void> {
    ...
    // Add company logo to workbook - as a square image
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // Set column widths - adjusted for better layout
    worksheet.columns = [
      { width: 8 },   // #
      { width: 40 },  // Item name (wider)
      { width: 15 },  // Code
      { width: 12 },  // Quantity
      { width: 18 },  // Unit price
      { width: 18 }   // Total
    ...
    // Logo + Header - Logo placed with proper square sizing
    if (logoImageId !== null) {
      try {
        // Add logo as a square (1x1 ratio) in the top-left corner
        // Using editAs: 'absolute' to maintain aspect ratio
        worksheet.addImage(logoImageId, {
          tl: { col: 0, row: currentRow - 1 } as any,
          ext: { width: 80, height: 80 } as any,
          editAs: 'absolute'
        } as any)
        // Add extra rows for logo spacing
        currentRow += 3
      } catch (e) {
    ...
    // Company & Customer Cards - Two columns side by side
    // Company Card
    worksheet.mergeCells(currentRow, 1, currentRow, 3)
    const companyCardRow = worksheet.getRow(currentRow)
    companyCardRow.height = 24
    const companyCardHeader = companyCardRow.getCell(1)
    companyCardHeader.value = 'بيانات الشركة'
    companyCardHeader.font = headerFont
    companyCardHeader.fill = headerFill
    companyCardHeader.alignment = { horizontal: 'center', vertical: 'middle' }
    companyCardHeader.border = thinBorder
    currentRow++
    ...
    // Customer Card
    worksheet.mergeCells(currentRow, 4, currentRow, 6)
    const customerCardRow = worksheet.getRow(currentRow)
    customerCardRow.height = 24
    const customerCardHeader = customerCardRow.getCell(4)
    customerCardHeader.value = 'بيانات العميل'
    customerCardHeader.font = headerFont
    customerCardHeader.fill = headerFill
    customerCardHeader.alignment = { horizontal: 'center', vertical: 'middle' }
    customerCardHeader.border = thinBorder
    currentRow++
    ...
    for (const data of customerData) {
      ...
      worksheet.mergeCells(currentRow, 6, currentRow, 6)
