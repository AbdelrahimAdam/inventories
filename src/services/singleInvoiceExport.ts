...
export class SingleInvoiceExportService {
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    worksheet.columns = [
      { width: 12 },  // A - # (wider for better text visibility)
      { width: 28 },  // B - الصنف / Customer value
      { width: 14 },  // C - الكود
      { width: 12 },  // D - الكمية
      { width: 18 },  // E - سعر الوحدة / Company value
      { width: 18 }   // F - الإجمالي
    ]
    ...
    // ============================================================
    // 1. HEADER SECTION - Logo + Title in the SAME ROW
    // ============================================================
    ...
    // Add the logo in the SAME ROW, far right corner (column 5-6)
    if (logoImageId !== null) {
      try {
        worksheet.addImage(logoImageId, {
          tl: { col: 5.4, row: currentRow - 1 + 0.05 } as any,
          ext: { width: 70, height: 70 } as any,
          editAs: 'absolute'
        } as any)
      } catch (e) {
    ...
    // ============================================================
    // 3. COMPANY & CUSTOMER CARDS - Equal widths side by side
    // ============================================================
