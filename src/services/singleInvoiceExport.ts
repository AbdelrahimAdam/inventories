...
export class SingleInvoiceExportService {
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // Professional color scheme - softer, more elegant blue
    const primaryColor = 'FF4A7BB7'  // Softer professional blue
    const secondaryColor = 'FFD4E2F0' // Light blue for accents
    const accentColor = 'FFF0F4F8'    // Very light blue for alternating rows
    ...
    const thickBorder: Partial<ExcelJS.Borders> = {
      top: { style: 'thick', color: { argb: 'FF4A7BB7' } },
      left: { style: 'thick', color: { argb: 'FF4A7BB7' } },
      bottom: { style: 'thick', color: { argb: 'FF4A7BB7' } },
      right: { style: 'thick', color: { argb: 'FF4A7BB7' } }
    }
    ...
    const evenRowFill: ExcelJS.Fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: 'FFF9FAFC' }
    }

    // Professional column widths - balanced for RTL
    worksheet.columns = [
      { width: 6 },   // A - #
      { width: 35 },  // B - الصنف (item name)
      { width: 15 },  // C - الكود (code)
      { width: 10 },  // D - الكمية (quantity)
      { width: 16 },  // E - سعر الوحدة (unit price)
      { width: 16 }   // F - الإجمالي (total)
    ]

    let currentRow = 1

    // ============================================================
    // 1. HEADER SECTION - Logo (top right) + Title + Company Name
    // ============================================================
    ...
    // Merge all columns for the header background
    worksheet.mergeCells(currentRow, 1, currentRow, 6)
    const headerBgCell = headerRow1.getCell(1)
    headerBgCell.fill = headerFill
    headerBgCell.border = thickBorder

    // Title in the center (columns 2-5)
    worksheet.mergeCells(currentRow, 2, currentRow, 5)
    const titleCell = headerRow1.getCell(2)
    ...
    // Logo on the RIGHT side (column 6, since RTL this is visually LEFT)
    if (logoImageId !== null) {
      try {
        // Position logo in the top-right corner (column 5-6)
        worksheet.addImage(logoImageId, {
          tl: { col: 4.8, row: currentRow - 1 + 0.1 } as any,
          ext: { width: 55, height: 55 } as any,
          editAs: 'absolute'
        } as any)

        // Add border around logo area
        worksheet.mergeCells(currentRow, 5, currentRow, 6)
        const logoCell = headerRow1.getCell(5)
        logoCell.border = thickBorder
    ...
    companyCell.font = { name: 'Cairo', size: 13, bold: true, color: { argb: 'FFFFFFFF' } }
    companyCell.fill = headerFill
    companyCell.alignment = {
      horizontal: 'center',
      vertical: 'middle'
    }
    companyCell.border = thickBorder
    ...
    // ============================================================
    // 3. COMPANY & CUSTOMER CARDS - Equal width side by side
    // ============================================================

    const detailsStartRow = currentRow

    // Headers - equal width columns (3 columns each)
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      // Customer block (Columns 1-3) - Right side
      ...
      // Company block (Columns 4-6) - Left side
    ...
    totalCell5.font = { ...totalFont, color: { argb: 'FF2E7D32' } }
