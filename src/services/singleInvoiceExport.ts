...
export class SingleInvoiceExportService {
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // ============================================================
    // 1. LOGO AREA - Left side with proper aspect ratio
    // ============================================================
    ...
    if (logoImageId !== null) {
      try {
        // Use ext to maintain aspect ratio instead of stretching
        // Position the logo on the left side (column 1-2)
        worksheet.addImage(logoImageId, {
          tl: { col: 0.3, row: logoStartRow - 0.1 } as any,
          ext: { width: 80, height: 80 } as any,
          editAs: 'absolute'
        } as any)

        // Apply thick border to the logo area
        for (let r = logoStartRow; r <= logoEndRow; r++) {
          for (let c = 1; c <= 2; c++) {
