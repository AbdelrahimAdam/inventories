...
export class SingleInvoiceExportService {
  static async exportSingleInvoice(invoice: any): Promise<void> {
      ...
      if (tenantId) {
        ...
        if (!error && data) {
          ...
          companyInfo = {
            ...
            logoUrl: data?.logo_url || settings?.logo_url || settings?.logo || null
          }
        }
      }
    } catch (error) {
    ...
    // Enable RTL for the entire workbook
    workbook.views = [{ rightToLeft: true as any }]
    ...
    // Enable RTL for the worksheet - this is the key for true RTL support
    worksheet.views = [{ rightToLeft: true as any }]

    this.createProfessionalInvoiceWorksheet(worksheet, invoice, companyInfo, logoImageId)
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // RTL is already enabled via worksheet.views above
    // This ensures the entire sheet is RTL
    ...
    // Column widths optimized for RTL
    worksheet.columns = [
      { width: 8 },   // # - narrow
      { width: 40 },  // Item name - wide (most important)
      { width: 15 },  // Code - medium
      { width: 12 },  // Quantity - narrow
      { width: 18 },  // Unit price - medium
      { width: 18 }   // Total - medium
    ]
    ...
    if (logoImageId !== null) {
      try {
        worksheet.addImage(logoImageId, {
          tl: { col: 4, row: currentRow - 1 } as any, // Position on right side in RTL
          ext: { width: 80, height: 80 } as any,
          editAs: 'absolute'
        } as any)
        currentRow += 3
      } catch (e) {
    ...
    // ============================================
    // 2. INVOICE INFO - Both on same row, RTL positioned
    // ============================================
    worksheet.mergeCells(currentRow, 1, currentRow, 3) // Right side in RTL
    worksheet.mergeCells(currentRow, 4, currentRow, 6) // Left side in RTL
    ...
    // ============================================
    // 3. COMPANY & CUSTOMER - SIDE BY SIDE (SAME ROW START)
    // ============================================
    const detailsStartRow = currentRow
    
    // Headers on the same row
    worksheet.mergeCells(detailsStartRow, 1, detailsStartRow, 3) // Customer card (RIGHT in RTL)
    worksheet.mergeCells(detailsStartRow, 4, detailsStartRow, 6) // Company card (LEFT in RTL)
    const cardHeaderRow = worksheet.getRow(detailsStartRow)
    cardHeaderRow.height = 24
    
    // Customer header (RIGHT side in RTL)
    ...
    // Company header (LEFT side in RTL)
    ...
    // Data rows - both cards progress together
    const companyData = [
    ...
    // Process 5 rows (max of both arrays)
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      // Customer data (RIGHT side in RTL - columns 1-3)
      if (i < customerData.length) {
        ...
        worksheet.mergeCells(rowIndex, 3, rowIndex, 3)
      ...
      // Company data (LEFT side in RTL - columns 4-6)
      if (i < companyData.length) {
        ...
        worksheet.mergeCells(rowIndex, 6, rowIndex, 6)
    ...
    currentRow = detailsStartRow + maxRows + 2 // Add spacing
    ...
    // Table headers - in logical RTL order
    ...
    // ============================================
    // 5. TOTALS - PROPER RTL LAYOUT
    // ============================================
    // Labels in column 1-4 (right side), values in column 5-6 (left side)
    // With RTL, column 1 is visually on the right
    ...
    footerCell.font = { name: 'Cairo', size: 10, italic: true, color: { argb: 'FF444444' } }
