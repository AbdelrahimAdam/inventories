...
export class SingleInvoiceExportService {
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // Professional column widths for invoice
    worksheet.columns = [
      { width: 6 },   // A - #
      { width: 38 },  // B - الصنف (item name - wide)
    ...
    // ============================================================
    // 1. HEADER SECTION - Logo + Title + Company Name
    // ============================================================

    // Set header height
    const headerHeight = 75
    const headerRow1 = worksheet.getRow(currentRow)
    headerRow1.height = headerHeight

    // Merge all 6 columns for the header background
    worksheet.mergeCells(currentRow, 1, currentRow, 6)

    // Create the header cell with background
    const headerCell = headerRow1.getCell(1)
    headerCell.value = ''
    headerCell.fill = headerFill
    headerCell.border = thickBorder

    // Add the title text in the center
    // We'll use a separate merge for the title text
    ...
    // Add the company name below the title (on the same row? No, we'll add it on the next row)
    // But we want the logo and title on the same row

    // Place the logo on the RIGHT side (since RTL, this is column 5-6)
    if (logoImageId !== null) {
      try {
        // Logo positioned on the right side (columns 5-6) with proper size
        // Using ext to maintain aspect ratio
        worksheet.addImage(logoImageId, {
          tl: { col: 4.5, row: currentRow - 1 + 0.1 } as any,
          ext: { width: 60, height: 60 } as any,
          editAs: 'absolute'
        } as any)

        // Add a circular border effect by using a border around the logo area
        // We'll merge cells for the logo area and add border
        worksheet.mergeCells(currentRow, 5, currentRow, 6)
        const logoAreaCell = headerRow1.getCell(5)
        logoAreaCell.border = thickBorder
        logoAreaCell.fill = headerFill
      } catch (e) {
        console.warn('Failed to add logo:', e)
      }
    }

    // Add the company name on the next row
    currentRow++
    ...
    companyCell.font = { name: 'Cairo', size: 14, bold: true, color: { argb: 'FFFFFFFF' } }
    ...
    // ============================================================
    // 3. COMPANY & CUSTOMER CARDS - same start row
    // ============================================================
