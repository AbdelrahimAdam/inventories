...
export class SingleInvoiceExportService {
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // Balanced column widths for better appearance
    worksheet.columns = [
      { width: 6 },   // A - #
      { width: 38 },  // B - الصنف
      { width: 15 },  // C - الكود
    ...
    // Logo area - positioned on the right side aligned with title
    if (logoImageId !== null) {
      try {
        // Set heights for logo rows
        worksheet.getRow(1).height = 30
        worksheet.getRow(2).height = 30
        worksheet.getRow(3).height = 30
        
        // Position logo on the right side (columns 5-6)
        worksheet.addImage(logoImageId, {
          tl: { col: 4.5, row: 0.15 } as any,
    ...
    // Title row - company name and invoice title
    ...
    // Company & Customer Cards - same start row
