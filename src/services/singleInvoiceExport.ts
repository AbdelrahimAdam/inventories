...
export class SingleInvoiceExportService {
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // Softer, more professional blue color scheme
    const primaryColor = 'FF5B8DB8'
    const secondaryColor = 'FFDCE8F1'
    ...
    // Balanced column widths - equal for customer and company sections
    worksheet.columns = [
      { width: 10 },  // A - # (wider for better text visibility)
      { width: 30 },  // B - الصنف (balanced)
      { width: 14 },  // C - الكود
      { width: 10 },  // D - الكمية
      { width: 16 },  // E - سعر الوحدة
      { width: 16 }   // F - الإجمالي
    ]

    let currentRow = 1

    // Logo positioned in the far right corner of the header row
    if (logoImageId !== null) {
      try {
        // Set row height for the header
        worksheet.getRow(1).height = 70
        
        // Position logo in the far right corner (column 6, at the edge)
        // The logo fills the height of the cell (top to bottom)
        worksheet.addImage(logoImageId, {
          tl: { col: 5.6, row: 0.05 } as any,
          ext: { width: 65, height: 65 } as any,
          editAs: 'absolute'
        } as any)
      } catch (e) {
    ...
    // Title row - company name and invoice title (logo is in this row)
    ...
    // Company & Customer Cards - same start row with equal sizes
