...
export class SingleInvoiceExportService {
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    const primaryColor = 'FF5B8DB8'  // Lighter, softer blue
    const secondaryColor = 'FFDCE8F1' // Very light blue for subheaders
    ...
    // Logo positioned in the same row as title, far right corner
    if (logoImageId !== null) {
      try {
        // Set row height for the header
        worksheet.getRow(1).height = 70
        
        // Position logo in the far right corner (column 5-6)
        worksheet.addImage(logoImageId, {
          tl: { col: 4.8, row: 0.1 } as any,
          ext: { width: 55, height: 55 } as any,
          editAs: 'absolute'
        } as any)
        
        // Don't add extra rows for logo - it's in the same row as title
      } catch (e) {
