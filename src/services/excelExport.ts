...
export class ExcelExportService {
  static async exportSingleCard(
    ...
    // @ts-ignore - rightToLeft is supported at runtime
  ...
  private static async createProfessionalWorksheet(
    ...
    // Item details - label in first column, value spans next columns (RTL friendly)
    ...
    for (let rowIdx = imageStartRow; rowIdx <= originalImageEndRow && detailIndex < details.length; rowIdx++) {
      ...
      // First detail in this row (columns 3-4) - label in 3, value in 4
      if (detailIndex < details.length) {
        ...
        valueCell1.alignment = { horizontal: 'right', vertical: 'middle' }
        valueCell1.border = thinBorder
      }
      
      // Second detail in this row (columns 5-6) - label in 5, value in 6
      if (detailIndex < details.length) {
        ...
        valueCell2.alignment = { horizontal: 'right', vertical: 'middle' }
        valueCell2.border = thinBorder
      }
      
      // Third detail in this row (columns 7-8) - label in 7, value in 8
      if (detailIndex < details.length) {
        ...
        valueCell3.alignment = { horizontal: 'right', vertical: 'middle' }
