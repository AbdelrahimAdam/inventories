...
export class InvoiceExportService {
  static async exportToExcel(invoices: any[], options: ExportOptions = {}): Promise<void> {
    ...
    // Add company logo to the workbook
    let logoImageId: number | null = null
    if (options.companyInfo?.logoUrl) {
      try {
        const logoBuffer = await fetchImageAsBuffer(options.companyInfo.logoUrl)
        if (logoBuffer) {
        ...
        console.warn('Failed to add company logo to workbook:', e)
      }
    }

    const summarySheet = workbook.addWorksheet('ملخص الفواتير')
    this.createProfessionalSummarySheet(summarySheet, invoicesToExport, options, logoImageId)
    ...
    for (let i = 0; i < Math.min(invoicesToExport.length, MAX_INDIVIDUAL_SHEETS); i++) {
      ...
      const sheetName = this.createSafeSheetName(invoice.invoice_number || `فاتورة_${i + 1}`, i + 1)
      const worksheet = workbook.addWorksheet(sheetName)
      this.createProfessionalInvoiceWorksheet(worksheet, invoice, options, logoImageId)
    }

    if (options.companyInfo) {
      const companySheet = workbook.addWorksheet('معلومات الشركة')
      this.createCompanyInfoSheet(companySheet, options.companyInfo, logoImageId)
    }
  ...
  private static createProfessionalSummarySheet(
    ...
    const totalRowFill: ExcelJS.Fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFC5E0B4' } }
    ...
    // Logo (if available) - positioned in RTL correctly
    if (logoImageId !== null) {
      try {
        worksheet.addImage(logoImageId, {
          tl: { col: 0, row: currentRow - 1 } as any,
          br: { col: 2, row: currentRow + 1 } as any,
          editAs: 'oneCell'
        } as any)
        currentRow += 2
      } catch (e) {
        console.warn('Failed to add logo to summary sheet:', e)
      }
    }
    ...
    subtitleCell.alignment = { horizontal: 'center', vertical: 'middle', readingOrder: 2 }
    subtitleCell.border = thickBorder
    currentRow++
    currentRow++

    // Statistics - RTL aligned
    ...
    statsCell1.value = `إجمالي الفواتير: ${invoices.length} | الإجمالي: ${totalAmount.toLocaleString('ar-EG')} ج.م`
    statsCell1.font = { name: 'Arial', size: 12, bold: true }
    statsCell1.fill = subheaderFill
    statsCell1.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    statsCell1.border = thinBorder
    ...
    statsCell5.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    statsCell5.border = thinBorder
    currentRow++
    currentRow++

    // Table headers - RTL aligned
    ...
    for (let i = 0; i < invoices.length; i++) {
      ...
      for (let col = 1; col <= 8; col++) {
        ...
        if (col === 7) {
          cell.numFmt = '#,##0.00'
          cell.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
        }
      }
      currentRow++
    }
    ...
    footerCell.alignment = { horizontal: 'center', vertical: 'middle', readingOrder: 2 }
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // Set column widths - RTL oriented
    worksheet.columns = [
      { width: 8 }, { width: 35 }, { width: 15 }, { width: 12 }, { width: 15 }, { width: 18 }
    ]
    ...
    if (logoImageId !== null) {
      try {
        ...
        console.warn('Failed to add logo to invoice sheet:', e)
      }
    }

    // Header with company name - RTL aligned
    ...
    headerCell.alignment = { horizontal: 'center', vertical: 'middle', readingOrder: 2 }
    headerCell.border = thickBorder
    currentRow++

    // Invoice info - RTL aligned
    ...
    infoCell1.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    infoCell4.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    infoCell4.border = thinBorder
    currentRow++
    currentRow++

    // Customer section - RTL aligned
    worksheet.mergeCells(currentRow, 1, currentRow, 6)
    ...
    customerHeaderCell.alignment = { horizontal: 'center', vertical: 'middle', readingOrder: 2 }
    ...
    itemsHeaderCell.alignment = { horizontal: 'center', vertical: 'middle', readingOrder: 2 }
    itemsHeaderCell.border = thinBorder
    currentRow++

    // Table headers - RTL oriented
    ...
    for (let i = 0; i < headers.length; i++) {
      ...
      headerCellItem.alignment = { horizontal: 'center', vertical: 'middle', readingOrder: 2 }
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      for (let col = 1; col <= 6; col++) {
        ...
        dataCell.alignment = { horizontal: 'center', vertical: 'middle', readingOrder: 2 }
        dataCell.border = thinBorder
        if (col === 5 || col === 6) {
          dataCell.numFmt = '#,##0.00'
          dataCell.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
        }
        if (col === 2) {
          dataCell.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
        }
    ...
    subtotalCell1.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    subtotalCell5.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    discountCell1.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    discountCell5.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    shippingCell1.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    shippingCell5.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    vatCell1.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    vatCell1.border = thinBorder

    const vatCell5 = vatRow.getCell(5)
    vatCell5.value = invoice.vat_amount || 0
    vatCell5.font = valueFont
    vatCell5.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    totalCell1.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    ...
    totalCell5.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
    totalCell5.border = thickBorder
    totalCell5.numFmt = '#,##0.00'
    currentRow++
    currentRow++

    // Signature section - RTL aligned
    ...
    sigCell1.alignment = { horizontal: 'center', vertical: 'bottom', readingOrder: 2 }
    ...
    sigCell4.alignment = { horizontal: 'center', vertical: 'bottom', readingOrder: 2 }
  ...
  private static createCompanyInfoSheet(
    worksheet: ExcelJS.Worksheet, 
    companyInfo: any, 
    logoImageId: number | null
  ): void {
    ...
    if (logoImageId !== null) {
      try {
        ...
        console.warn('Failed to add logo to company info sheet:', e)
      }
    }
    ...
    for (let i = 0; i < companyData.length; i++) {
      ...
      labelCell.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
      ...
      valueCell.alignment = { horizontal: 'right', vertical: 'middle', readingOrder: 2 }
