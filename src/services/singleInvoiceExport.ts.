...
export class SingleInvoiceExportService {
  ...
  private static createProfessionalInvoiceWorksheet(
    ...
    // Wider client block and a slightly wider item-name area
    worksheet.columns = [
      { width: 8 },   // A - #
    ...
    // ============================================================
    // 1. TITLE ROW (Company Name + Invoice Title)
    // ============================================================
    ...
    // ============================================================
    // 2. LOGO (positioned to the right of the title, like excelExport)
    // ============================================================

    if (logoImageId !== null) {
      try {
        // Logo spans from column 1 to 2 (3 columns wide) 
        // and takes the full height from the current row
        const logoStartRow = currentRow - 1 // Use the title row as the starting row
        const logoEndRow = currentRow + 3   // Extend down 4 rows total
        ...
        worksheet.addImage(logoImageId, {
          ...
          br: { col: 2.5, row: logoEndRow + 0.3 } as any,
          editAs: 'absolute'
        } as any)

        // Apply border to the logo area like excelExport does
        for (let r = logoStartRow; r <= logoEndRow; r++) {
          for (let c = 1; c <= 2; c++) {
    ...
    // ============================================================
    // 4. COMPANY & CUSTOMER CARDS - same start row
    // ============================================================
